...
$$
	\sum_i \left[\sup_{\omega \in A_i} f(\omega)\right] \mu(A_i) < \infty
$$
Then $a_i = \sup_{A_i} f < \infty$ for each $i$ in the set of indices for which $\mu(A_i) > 0$. If $a = \max_i a_i$, then $\mu[f > a] = \sum_I \mu(A_i \cap [f > a]) \leq \sum_I \mu(A_i \cap [f > a_i]) = 0$, because we know that $a_i$ is the supremum of $f$ on $A_i$. 
\\\\
...
$$
	...
	\int |f| d\mu
$$
Where the second equality comes from $f^+$ and $f^-$ both being nonnegative. We also can apply to integrable $f$ and $g$, in which case $f - g$ is integrable and making use of Theorem 16.1 gives us:
$$
...
$$
	...
	\mu(\Omega - A) = 0
$$
A technical point arises here - since $\fcal$ doesn't need to necessarily contain $A$. Consider the following example. Let $\fcal$ consist of the Borel subsets of $\Omega = [0,1)$, and let $H$ be a nonmeasurable set - a subset of $\Omega$ that does not lie in $\fcal$ (such as the Vitali Sets).
