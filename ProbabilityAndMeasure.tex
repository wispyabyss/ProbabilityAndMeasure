...
\paragraph{Definition - Distribution/Law of a Random Variable} The \textit{distribution} or \textit{law} of a random variable $X$ is t he probability measure on $(\R^1, \rcal^1)$ \textit{induced} by the random variable, defined by:
$$
...
\begin{enumerate}
	...
	\item Note that $\mu(-\infty,x] = F(x)$ and $\mu(\R) = 1$
	\item Thus, $\mu$ can be a probability measure for $(\R,\rcal,\mu)$.
