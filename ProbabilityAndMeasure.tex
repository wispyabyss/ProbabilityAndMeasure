...
$$
	...
	\bigcap_{j=0}^\infty \bigcup_n [\omega : f_n(\omega) \geq j] \in \fcal
$$
$$
	\left[\omega : \sup_n f_n(\omega) = -\infty\right] =
	\bigcap_{j = 0}^{-\infty} \bigcup_n [\omega : f_n(\omega) \leq j] \in \fcal
$$
Note, I had to switch the order of the intersection/union above. For the supremum, we want $\omega$ such that there is some $f_n(\omega) \geq j$ for each $j$ - as then the supremum for that $\omega$ would be infinite. Similar case for $-\infty$. So, $\sup_n f_n$ is measurable. Measurability of $\inf_n f_n$ follows in much the same way. We thus have:
$$
...
\paragraph{Distribution Weak Convergence} If $F_n$ and $F$ are distribution functions, then by definition, $F_n$ \textit{converges weakly} to $G$, written $F \Rightarrow F$, if:
$$
	\lim_n F_n(x) = F(x)
$$
...
\begin{enumerate}
	...
	$$
		P[F(x) \leq u] = P[F(X) < u] = F(\varphi(u)-) = F(\varphi(u))
	$$
\end{enumerate}
...
$$
	...
	\leq y_n + z_n
$$
So, we must have $d(F,H) \leq y + z$. Thus, we have proved the triangle inequality. Now, we want to show that a necessary and sufficient condition for $F_n \Rightarrow F$ is that $d(F_n,F) \to 0$. Recall, $F_n \Rightarrow F$ just means that for all points $x$ where $F$ is continuous:
$$
	\lim_n F_n(x) = F(x)
$$
...
\\\\
\textbf{From the book} We are proving necessity. Choose continuity points $x_i$ of $F$ in such a way that:
$$
	x_0 < x_1 < \cdots < x_k
$$
Where:
...
$$
And so all together, we must have:
$$
...
\\\\
So, we have almost proved that $F_n \Rightarrow F$ implies $d(F,F_n) \to 0$. However, what about $x \leq x_0$, or $x \geq x_k$? Consider $x \leq x_0$. In that case, we have:
$$
	0 \leq F_n(x) \leq F_n(x_0) \leq F(x_i) + \epsilon/2 \leq F(x + \epsilon) + \epsilon/2 \leq F(x + \epsilon) + \epsilon
$$
