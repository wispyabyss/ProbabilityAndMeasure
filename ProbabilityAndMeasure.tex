...
\\\\
From now on, we will fix a chain $X_0, X_1, \cdots$ satisfying $\alpha_i > 0$ for all $i$. Denote by $P_i$ probabilities conditional on $[X_0 = i]$ - ie, we have $P_i(A) = P[A_i | X_0 = i]$. thus, we have:
$$
...
$$
	= f_{ij}f_{jj}^{k - 1}
$$
And so now, taking $k \to \infty$, and making use of monotone convergence theorem, we have:
$$
...
$$
And as $P_i$ is a probability measure on $\fcal$, we indeed have that if the sum of the above events converges, we have that:
$$
	P_i\left[X_n = i \;\io\right] = 0
$$
...
$$
	2 \implies 1, 3 \implies 2
$$
If we prove $1 \implies 3$, then we have the first three conditions are equivalent. Note, it will also prove the final three conditions are equivalent, as we have $\not 1 = 4, \not 2 = 5, \not 3 = 6$. And so, as $1, 2, 3$ are equivalent, we will also have $\not 1, \not 2, \not 3$ are equivalent, which means the second line of the theorem is true as well. So now, we just try and prove that transience of $i$, ie $f_{ii} < 1$, implies $\sum_n p_{ii}^{(n)} < \infty$. Note that:
$$
...
$$
$$
	= \sum_{s = 0}^{n-1} P_i\left[X_1 \neq j, \cdots, X_{n - s - 1} \neq j, X_{n - s} = j\right]P_j\left[X_n = j\right]
$$
