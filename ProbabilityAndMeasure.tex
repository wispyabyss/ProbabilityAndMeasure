...
\\\\
First, one direction is easy. Suppose that $\varphi: \Omega' \to \rcal^1$ is measurable $\fcal'/\rcal$, and $f = \varphi \circ T$. Note, $T$ is measurable $T^{-1}\fcal'/\fcal'$. By Theorem 13.1.ii, we directly have that $f$ must be measurable $T^{-1}\fcal'/\rcal^1$.
\\\\
...
\\\\
We follow the hint. We first consider $f$ as a simple function. And so, we must haveL
$$
