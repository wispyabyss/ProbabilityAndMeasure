...
$$
	\left|r - \sum_i \sup_{x \in I_i} f(x) \cdot \lambda(I_i)\right| < \epsilon
$$
...
$$
Where the second sum comes from noting, that $g$ and $g'$ only differ by $x_i$ on the portions where $A_i$ and $B_i$ don't meet up exactly (added up w/ all the differences for the other $x_i$). Thus, we have found a $g'$ that satisfies the first part.
\\\\
