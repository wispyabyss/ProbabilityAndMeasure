...
\\\\
The problem notes to also make use of problem 18.1, done above, where we proved if $A \times B$ is nonnempty then $A \in \xcal$ and $B \in \ycal$.
\\\\
Well, we consider $A \times B$, where $A$ is a single point, and $B$ is outside the completion of $\rcal$ with respect to $\lambda$. Note that a single point is in $\rcal$ via a countable intersection $(a_n, b]$. 
