...
\\\\
\textbf{Proof} We first try and prove 1. Define $\tau_n = \min\left\{\tau,N\right\}$. Note that $\tau_N$ is a stopping time as well (we could do it rigorously, but it seems intuitive that $P[\tau_n < \infty] = 1$, as it won't be greater than $1$, and $\tau_N = k \in \fcal_k$, which we can take from $\tau$). Note that:
$$
...
$$
We break this up into cases: $\tau < \sigma + N$, and $\tau \geq \sigma + N$. We first note that we can break this up across the infinite values that $\sigma$ can take on:
$$
	= \sum_{m=0}^\infty \sum_{n=0}^{\infty} \sum_k P_i\left[\sigma = m, \tau = m + n, X_{\tau_n} = k\right]\varphi(k)
$$
We now note, that if $m + n \geq m + N$, we have that $\tau_n = m + N$. Otherwise, $\tau_n = m + n$. So, we can break the expectation up as:
$$
	E_i\left[\varphi(X_{\tau_N})\right] =
...
\paragraph{Lemma 8.6 If an excessive function $\varphi$ dominates the payoff function $g$, then it dominates the value function $v$ as well} Note, $g$ dominates $h$ means for all $i$, $g(i) \geq h(i)$.
\\\\
...
qed.
\\\\
As $\tau = 0$ is a stopping time, we also have that $v$ dominates $f$. Lemmas 4 and 6 thus characterize $v$:

\paragraph{Theorem 8.10} The value function $v$ is the minimal excessive function dominating $f$. Note how this stems from our lemmas 8.4, 8.5, and 8.6.
