...
\\\\
So, I think a lot of the properties we can just forward from $\lambda$ - rotation invariance via translation invariance, arc generation via interval generation. However, we need to first show that $\ccal$ is the sets in $\rcal^2$ within $C$.
\\\\
Realize that we proved $\ccal$ is a $\sigma$ algebra in question 13.2. First, note that $C \in \rcal^2$. We can define $f: \R^2 \to \R^2$ as $f(\theta,r) = (\theta, r-1)$. This is a continuous function. And so Theorem 13.2 implies that $f$ is measurable, and $C = f^{-1}\left(\left\{(t,0) : t \in [0,2\pi]\right\}\right) \in \rcal^2$. Thus, the sets in $\rcal^2$ that are contained in $C$ can be defined as:
$$
