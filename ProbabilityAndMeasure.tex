...
\begin{enumerate}
	...
	$$
	Well - first, I will examine $[f > 1]$. Clearly, as $(1, \infty) \in \rcal$, we have that $[f > 1] \in \fcal$. We want to show that $[f > 1] \times (0,x] \in \sigma(\acal_0)$, and so $\nu$ will be well defined on that set.
	\\\\
	First, we will show that $[f > 1] \in \fcal_0$, as only this will allow $\mu_0[f > 1]$ to be well defined. This follows if $[f > 1] \times (0,1] \in \sigma(\acal_0)$. So, if $[f > 1] \times (0,1]$ can be expressed as the union or intersection of sets in $\sigma(\acal_0)$, we are good! For $x \in \R$, look at:
	$$
		(1, f \wedge 1] = 
		\left\{(\omega, t) : 1 < t \leq f \vee 1\right\}
	$$
