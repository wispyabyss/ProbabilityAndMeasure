...
\begin{enumerate}
	...
	$$
	Now, consider the case where $(f,g] \subseteq \bigcup_i (f_i,g_i]$. Then, we know for all $\omega$:
	$$
...
\begin{enumerate}
	...
	\item Let $\fcal$ be the smallest $\sigma$-field with respect to which every $f$ in $\lcal$ is measurable: $\fcal = \sigma\left[f^{-1}H : f \in \lcal, H \in \rcal^1\right]$. Note, $f$ is measurable on a $\sigma$ field if $f^{-1}(H)$ is within that sigma field, for all $H \in \rcal^1$. Note, we didn't worry about $\rcal^1$ earlier with Simple Random Variables, because we only really had to consider finite element $H \subseteq \R$.
	\\\\
	Let $\fcal_0$ be the class of $A$ in $\fcal$ for which $A \times (0,1] \in \sigma(\acal_0)$. Show that $\fcal_0$ is a semiring and that $\fcal = \sigma(\fcal_0)$. 
	\\\\
	First note that $\fcal_0$ is a $\sigma$ algebra on $\Omega$. Functions $f: \Omega \to \R$, and so $f^{-1}(H)$ for $H \in \rcal^1$ are subsets of $\Omega$. Recall, $\acal_0$ is the sets of the form $(f,g]$, for $f,g \in \lcal$ and $f \leq g$. So, $\fcal_0$ is sets of $\fcal$ $A$ (where $A \subseteq \Omega$) such that:
	$$
	...
	$$
	As that will imply $[\omega : f(\omega) > 1] \in \fcal_0$. Well, $[\omega : f(\omega) > 1] \times (0,1]$ is the union of sets $(0,f_n] \in \acal_0$, and so yes, we do have the above fact. And so, we can conclude for all $f \in \lcal$, we have:
	$$
		[\omega : f(\omega) > 1] \in \fcal_0
	$$
	Now, we note that $xf \in \lcal$ and $-xf \in \lcal$ for $x > 0$. This is because $\lcal$ is a vector space, and $f + \cdots + f \in \lcal$. Thus, we can also conclude:
	$$
		[f > x] \in \fcal_0 \quad\quad\quad [f > -x] \in \fcal_0 \quad\quad\quad
		[f < x] \in \fcal_0 \quad\quad\quad [f < -x] \in \fcal_0
	$$
	Note that the sets $(-\infty, -x)$ and $(x, \infty)$ generate $\rcal^1$. Any finite interval of the form $(x, y]$ can be formed by a finite amount of intersections, unions, and complements of the given form - and those intervals generate $\rcal^1$. So note - we can take those finite amount of intersections, unions, and complements to find that:
	$$
		f^{-1}(a,b] \in \sigma(\fcal_0)
	$$
	Note, as $(a,b]$ generate $\rcal^1$, we have that $f^{-1}(a,b]$ generates $\fcal$. I believe this is a fact that we will see again and again, so going over the proof of that here. 
	
\end{enumerate}

\subsubsection{Pullback Lemma}
