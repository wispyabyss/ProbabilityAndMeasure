...
\subsubsection{13.13} Suppose that the circular Lebesgue measure of $A$ satisfies $\mu(A) > 1 - n^{-1}$ and that $B$ contains at most $n$ points. Show that some rotation carries $B$ into $A$ - ie, $\theta B \subset A$ for some $\theta \in C$.
\\\\
