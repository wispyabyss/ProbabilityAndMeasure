...
\subsubsection{4.14} Suppose that there are in $(\Omega, \fcal, P)$ independent events $A_1, A_2, \cdots$ such that, if:
$$
...
$$
	C_x = \bigcup_{k=1}^\infty C^k_x
$$
Clearly, $\omega \in C^k_x \implies \omega \in C_x$, as we can just take $\omega \in A_i^c$ for $i > k$. Now, take $\omega \in C_x$. Note, if the $\omega$ sum is less than $x$, we can find a terminating expansion (with infinite tail of $\omega \in A_i^c$ such that $\omega \in C^k_x$ for some $k$ large enough. If the $\omega$ sum equals $x$ - and $x$ has only a non terminating expansion - we might run into some trouble. But, I think at this point, we should rely on the measurable mappings argument, which we will get to later.
\\\\
The above argument is actually correct, as $\omega$ can belong to one of the $C^k_x$ with finite sequences, as the partial sum will be less than $x$. So yeah, the above equality does hold, and we have $C_x \in \fcal$, which proves that $(\Omega, \fcal, P)$ is \textit{nonatomic}. qed.
