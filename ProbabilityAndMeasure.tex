...
$$
	...
	P_n\left[\bigcap_{i=1}^u \alpha_{p_i} \geq k_i - \alpha_{p_i} \geq k_i + 1\right] =
	1 - P_n\left[\bigcup_{i=1}^u [\alpha_{p_i} \geq k_i]^c \cup [\alpha_{p_i} \geq k_i + 1]\right]
$$
Note, the inner union on the right is a union of two disjoint sets. Making use of the inclusion-exclusion principle, the above is:
$$
	= 1 -
	\sum_{i=1}^u P_n\left[[\alpha_{p_i} \geq k_i]^c \cup [\alpha_{p_i} \geq k_i + 1]\right]
$$
$$
	+ 
	\sum_{1 \leq i < j \leq u} P_n\left[
	\left([\alpha_{p_i} \geq k_i]^c \cup [\alpha_{p_i} \geq k_i + 1]\right) \cap 
	\left([\alpha_{p_j} \geq k_j]^c \cup [\alpha_{p_j} \geq k_j + 1]\right)
	\right]
	+ \cdots
$$
We replace the above probabilities with their limits, derived from the above. We define:
$$
	A_i = \left([\alpha_{p_i} \geq k_i]^c \cup [\alpha_{p_i} \geq k_i + 1]\right)
$$
And we note independence of $A_i$ and $A_j$?
