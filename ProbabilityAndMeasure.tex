...
$$
Ie, $\rcal^1$, restricted to the unit interval (the LHS) is equal to the generated sigma algebra from the half open intervals in $(0,1]$ (the RHS), which is $\bcal$. Ie, we have:
$$
	\bcal = \rcal^1 \cap (0,1]
$$
...
$$
Is finite. If it wasn't - we could make a contradiction proving $\infty \leq \mu(A)/\epsilon$. If we take a union over positive rational $\epsilon$, we muts have:
$$
...
\paragraph{Theorem 10.3 - Uniqueness of General Measure Extension} Suppose that $\mu_1$ and $\mu_2$ are measures on $\sigma(\pcal)$, where $\pcal$ is a $\pi$ system, and suppose they are $\sigma$ finite on $\pcal$. If $\mu_1$ and $\mu_2$ agree on $\pcal$, then they agree on $\sigma(\pcal)$.
\\\\
I guess one question I would have before the proof - is what does $\sigma$ finite mean on $\pcal$? Does $\pcal$ contain $\Omega$? \textbf{Proof} Suppose that $B \in \pcal$ and $\mu_1(B) = \mu_2(B) < \infty$, and let $\lcal_B$ be the class of sets $A$ in $\sigma(\pcal)$ for which $\mu_1(B \cap A) = \mu_2(B \cap A)$. Then $\lcal_B$ is a $\lambda$ system (clearly contains $\omega$, complements by subtraction and $\mu(B) < \infty$, and countable disjoint unions via additivity of $\mu$) containing $\pcal$ ($\pi$ system contains intersections) and hence containing $\sigma(\pcal)$ via the $\pi-\lambda$ theorem.
\\\\
...
\\\\
I would first say that $\mu$ is finitely additive. If you have the finite disjoint union of finite sets - then we will just get out the sum. If you add a countably additive set in there - both sides will be infinite. Note, a finite amount of finite sets won't get to countable additivity.
\\\\
...
\begin{enumerate}
	...
	\\\\
	Now, we consider the case where $\fcal$ is uncountable. Let $\ical$ index every countable union of non-infinite measure. Ie, for $i \in \ical$, we have that:
	$$
...
\begin{enumerate}
	...
	$$
\end{enumerate}
Some simple examples - the class of finite intervals in $\Omega = \R^1$. Clearly the intersection of two such finite intervals is a finite interval. The set minus is also at most a pair of disjoint intervals. Also, the class of subintervals of $\Omega = (0,1]$ is a semiring as well, with the same reasoning.
...
$$
Thus, our set function $\mu$ is monotone. Take our outer measure $\mu^*$ - recall, defined as:
$$
...
$$
We now conclude (via countable subadditivity):
$$
	\mu^*(A \cap E) + \mu^*(A^c \cap E) \leq
	\sum_n \mu(B_n) + \sum_n \sum_{k=1}^{m_n} \mu(C_{nk}) =
	\sum_n \mu(A_n) < \mu^*(E) + \epsilon
$$
Take $\epsilon \to 0$, and yes, we can conclude that $\acal \subset \mcal(\mu^*)$.
\\\\
...
\\\\
Now, we go onto 11.4.2. Consider the $A_k$ of part 1 for $B$ with finite measure. For $A = \bigcup_k A_k$, we have $\mu(A) < \mu(B)$, and so $A$ has finite measure, and we can apply continuity from above to see:
$$
