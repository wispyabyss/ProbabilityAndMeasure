...
\subsubsection{11.4 - Riesz Representation Part 1} Let $\Lambda$ be a real linear functional on a vector lattice $\lcal$ of (finite) real functions on a space $\Omega$.
\\\\
...
$$
Assume further of $\lcal$ that $f \in \lcal$ implies $f \wedge 1 \in \lcal$ - note, I think that comes from the lattice definition. Assume further of $\Lambda$ that it is positive in the sense that $f \geq 0$ (pointwise) implies $\Lambda(f) \geq 0$ and continuous from above at $0$ in the sense that $f_n \downarrow 0$ (pointwise) implies $\Lambda(f_n) \to 0$. Those are indeed extra conditions on $\Lambda$.
