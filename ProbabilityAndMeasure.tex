...
$$
Satisfying $\pi_i \geq 0$ and $\sum_i \pi_i = 1$ (note, we could always rescale the sum if the previous two properties applied). Then, the chain is persistent:
$$
	\lim_n p_{ij}^{(n)} = \pi_j
$$
For all $i$ and $j$, the $\pi_j$ are all positive, and the stationary distribution is unique.
\\\\
...
$$
	\lim_n p_{ij}^{(n)} = \pi_j
$$
Note, that this equality would hold for any stationary distribution - and so the stationary distribution is unique, as we could define it by the above limit. All that remains is to show that the stationary distribution is positive - ie, $\pi_j > 0$. Choose $r$ and $s$ so that $p_{ij}^{(r)}$ and $p_{ji}^{(s)}$ are positive. Assume that $\pi_j = 0$. Then we note:
$$
	0 = \pi_i = \lim_{n \to \infty} p_{ji}^{(n)} \geq \lim_{m \to \infty} p_{ji}^{(s)}p_{ij}^{(m)}p_{ji}^{(s)} =
	p_{ji}^{(s)}p_{ji}^{(s)} \pi_j 
$$
We can go the other way as well, to find that if one stationary distribution is zero, they all are. This would contradict them summing to one. qed.
