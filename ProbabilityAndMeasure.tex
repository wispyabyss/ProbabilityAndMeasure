...
\subsubsection{14.6} A borel function satisfying Cauch's equation: $f(x + y) = f(x) + f(y)$ and defined on $(0,\infty)$ is automatically bounded in some interval, and hence satisfies $f(x) = xf(1)$. Note, we used this in proving the exponential distribution from the memoryless property.
\\\\
Prove that a borel function is automatically bounded in some interval to the right of $0$. Recall, a borel function $f: \R \to \R$ is measurable.
\\\\
We make use of problem 12.3. If $A \in \rcal^1$, and $\lambda(A) > 0$, and $0 < \theta < 1$, there is a bounded open interval $I$ such that:
$$
	\lambda(A \cap I) \geq \theta\lambda(I)
$$
Note, we must have a $K$ large enough such that:
$$
	\lambda\left[x : x > s, |f(x)| \leq K\right] > 0
$$
