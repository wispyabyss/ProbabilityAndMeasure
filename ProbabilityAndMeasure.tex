...
\\\\
Just as an aside. I was thinking of something like:
$$
	f(x) = \begin{cases}
	5x & \text{ if $x \geq 0$}\\
	0.5x & \text{ if $x \leq 0$}
	\end{cases}
$$
Note, the integral across $(\R, \rcal, \lambda)$ would not be defined. I think it is clear that both $f^+$ and $f-$ have infinite area. However, consider something like:
$$
	\int_{-n}^n f d\lambda =
	\int_0^n 5x d\lambda - \int_{-n}^0 0.5x = \frac{5}{2}n^2 - \frac{1}{4}n^2 = \frac{9}{4}n^2
$$
Ie, the positive area is larger than the negative area. As $n \to \infty$, the above value goes to $+\infty$. Is there some kind of theory for this? I think this is called something like the \textit{extended improper integral}.
...
$$
	\int_* f d\mu = \sup \sum_i \left[\inf_{\omega \in A_i} f(\omega)\right] \mu(A_i)
$$
...
$$
	\int^* f d\mu = \infty
$$
Now, assume that $\mu[\omega : f(\omega) > a] > 0$ for all $a$. We need to again show for any partition $A_1, \cdots, A_n$ of $\Omega$ into $\fcal$ sets, the integral sum is infinite. Assume:
$$
	\sum_i \left[\sup_{\omega \in A_i} f(\omega)\right] \mu(A_i) < \infty
$$
Then $a_i = \sup_{A_i} f < \infty$ for each $i$ in the set of indices for which $\mu(A_i) > 0$. If $a = \max_i a_i$, then $\mu[f > a] = \sum_I \mu(A_i \cap [f > a]) \leq \sum_I \mu(A_i \cap [f > a_i]) = \sum_I 0 = 0$, because we know that $a_i$ is the supremum of $f$ on $A_i$ (and while $f(\omega)$ may equal $a_i$, we have $f(\omega) > a_i$ must be false).
\\\\
So, we have $\mu[f > a] = 0$. This contradicts $\mu[\omega : f(\omega) > a] > 0$ for all $a$, and so we must have for any partition $A_1, \cdots, A_n$:
$$
	\sum_i \left[\sup_{\omega \in A_i} f(\omega)\right] \mu(A_i) = \infty \implies
	\int^* f d\mu = \infty
$$
qed.
\\\\
Note - there are many familiar functions that ought to be integrable, but are of the preceding types and thus have infinite upper integral. Consider $f = x^{-2}\1{x \geq 1}$. Here, we have that the area that $f$ is nonnegative in is infinite. Also consider $f = x^{-1/2}\1{x \in (0,1)}$. Here, we have that the measure of $f > a$ for any $a$ is  positive. Thus, both functions would have infinite upper integral. However, we generally think they should have finite integrals, as their contained area is finite, even though they go to infinity in the range/domain. These are the classic examples - having an infinite height over a very small width, or a very small height over an infinite width. Also fun to note - these two graphs are essentially symmetrical, so they should both have the same area:

\begin{center}
	\includegraphics[width=0.8\textwidth]{integral_examples.png}
\end{center}

\subsubsection{15.2 - Upper and Lower Integral Inequalities}
\begin{enumerate}
	\item Show that:
	$$
		\sum_i \left[\inf_{\omega \in A_i} f(\omega)\right]\mu(A_i) \leq
		\sum_j \left[\inf_{\omega \in B_j} f(\omega)\right]\mu(B_j)
	$$
	If $\{B_j\}$ is a partition that refines $\{A_i\}$. Note, this should be clear, as for $B_j \subseteq A_i$, we have $\sum_j \mu(B_j) = \mu(A_i)$, and $\inf_{\omega \in A_i} f(\omega) \leq \inf_{\omega \in B_j} f(\omega)$. We similarly can conclude that:
	$$
		\sum_j \left[\sup_{\omega \in B_j} f(\omega)\right]\mu(B_j) \leq 
		\sum_i \left[\sup_{\omega \in A_i} f(\omega)\right]\mu(A_i)
	$$
	If $\{B_j\}$ refines $\{A_i\}$. These two facts allow us to conclude that:
	$$
		\int f d\mu = \int_* f d\mu \leq \int^* f d\mu
	$$
	Note, the supremum the left is smaller than the infimum on the right, because every element in the left infimum is upper bounded by every element on the right infimum. Indeed, we will compare:
	$$
		\sup \sum_i \left[\inf_{\omega \in A_i} f(\omega)\right] \mu(A_i) 
		\quad\quad\text{ versus }\quad\quad
		\inf \sum_i \left[\sup_{\omega \in A_i} f(\omega)\right] \mu(A_i)
	$$
	Take an arbitrary sum on the left and right, for partitions $A_i$ and $C_i$. Note, we can find a partition $B_{ij}$ that refines both $A_i$ and $C_i$. Define $B_{ij} = A_i \cap C_j$. We can take disjoint unions across $j$ for a fixed $i$ to recover $A_i$, and disjoint unions across $i$ for a fixed $j$ to recover $C_j$. Note that:
	$$
		\sum_i \left[\inf_{\omega \in A_i} f(\omega)\right] \mu(A_i) \leq
		\sum_{ij} \left[\inf_{\omega \in B_{ij}} f(\omega)\right] \mu(B_{ij}) 
	$$
	$$
		\leq
		\sum_{ij} \left[\sup_{\omega \in B_{ij}} f(\omega)\right] \mu(B_{ij}) \leq
		\sum_j \left[\sup_{\omega \in C_j} f(\omega)\right] \mu(C_j)
	$$
	Where the second inequality comes from comparing infimums and supremums. And so, we can indeed conclude that:
	$$
		\int_* f d\mu \leq \int^* f d\mu
	$$
	
	\item Assume $f$ is bounded and that $\mu(\Omega) < \infty$. Now assume that $f$ is measurable $\fcal$ and let $M$ be a bound for $|f|$. Consider the partition $A_i = \left[\omega : i\epsilon < f(\omega) \leq (i + 1)\epsilon\right]$, where $i$ ranges from $-N$ to $N$ and $N$ is large enough such that $N\epsilon > M$. Show that:
	$$
		\sum_i \left[\sup_{\omega \in A_i} f(\omega)\right] \mu(A_i) -
		\sum_i \left[\inf_{\omega \in A_i} f(\omega)\right] \mu(A_i) < \epsilon \mu(\Omega)
	$$
	I think this is clear. We have that:
	$$
		= 
		\sum_i \left[\sup_{\omega \in A_i} f(\omega) - \inf_{\omega \in A_i} f(\omega)\right] \mu(A_i)
	$$
	And the difference between the inf and sup on $A_i$ must be less than $\epsilon$:
	$$
		\leq \sum_i \epsilon \mu(A_i) = \epsilon \mu(\Omega)
	$$
	Conclude that:
	$$
		\int_* f d\mu = \int^* f d\mu
	$$
	Note that the difference goes to zero as $\epsilon \to 0$, as $\mu(\Omega) < \infty$. Note that the limits as $\epsilon \to \infty$ equal the values for the lower and upper integral. If they didn't - then we would find that the upper integral is smaller than the lower integral, which would contradict:
	$$
		\int_* f d\mu \leq \int^* f d\mu
	$$
	And so we must have equality. 
\end{enumerate}

\section{Section 16 - Properties of the Integral}
\subsection{Notes}
\subsubsection{Equalities and Inequalities}
\paragraph{Integrable Definition 2} Recall, being integrable requires both $\int f^+ d\mu$ and $\int f^- d\mu$ to be finite. Via additivity of nonnegative functions from Theorem 15.1, that implies:
$$
	\int f^+ d\mu + \int f^- d\mu < \infty \quad\Leftrightarrow\quad
	\int f^+ + f^- d\mu < \infty \quad\Leftrightarrow\quad
	\int |f| d\mu < \infty
$$
By Theorem 15.2 for almost everywhere monotonicity, this implies that if $|f| \leq |g|$ almost everywhere and $g$ is integrable, then $f$ is as well. If $\mu(\Omega) < \infty$ and $f$ is bounded, $f$ is also integrable ($\Omega$ itself is a partition).

\paragraph{Theorem 16.1 - Integral Monotonicity and Linearity}
\begin{enumerate}
	\item Monotonicity: If $f$ and $g$ are integrable and $f \leq g$ almost everywhere, then:
	$$
		\int f d\mu \leq \int g d\mu
	$$
	
	\item Linearity: If $f$ and $g$ are integrable and $\alpha,\beta$ are finite real numbers, then $\alpha f + \beta g$ is integrable and:
	$$
		\int (\alpha f + \beta g) d\mu =
		\alpha \int f d\mu + \beta \int g d\mu
	$$
...
\textbf{Proof}
\begin{enumerate}
	\item Monotonicity. For nonnegative $f$ and $g$ such that $f \leq g$ almost everywhere, the Theorem follows directly from Theorem 15.2.4. For general integrable $f$ and $g$, if $f \leq g$ almost everywhere, then $f^+ \leq g^+$ and $g^- \leq f^-$ almost everywhere. So, the theorem follows directly by definition:
	$$
		\int f d\mu =
		\int f^+ d\mu - \int f^- d\mu \leq
		\int g^+ d\mu - \int g^- d\mu =
		\int g d\mu
	$$
	
	\item Linearity. First, note that $\alpha f + \beta g$ is integrable via Theorem 15.1:
	$$
		\int |\alpha f + \beta g|d\mu \leq
		\int |\alpha||f| + |\beta||g| d\mu =
		|\alpha| \int |f| d\mu + |\beta| \int |g| d\mu < \infty
	$$
	The first step is directly Theorem 15.1.2, and the second step is directly Theorem 15.1.4. Now, we consider $g = 0$. It is clear that for $a \geq 0$:
	$$
		\int af d\mu = \int af^+ d\mu - \int af^- d\mu = a\int f^+ d\mu - a\int f^- d\mu = a\int f d\mu
	$$
	Flipping the subtraction gives us the same conclusion for $a < 0$. Therefore, we just need to prove the general case for $\alpha = \beta = 1$ - if we do, then we can treat $f = af$ and $g = bf$, split the addition, and then pull out the constants. We have $(f + g)^+ - (f + g)^- = f + g = f^+ - f^- + g^+ - g^-$ Rearranging gives us:
	$$
		(f + g)^+ + f^- + g^- = (f + g)^- + f^+ + g^+
	$$
	Being nonnegative sums on both sides, Theorem 15.1.4 gives us:
	$$
...
\end{enumerate}
We have thus proven both monotonicity and linearity of $f$ and $g$ integrable. qed.
\\\\
We note that:
$$
	...
	\leq
	\left|\int f^+ d\mu\right| + \left|\int f^- d\mu\right| =
	\int f^+d\mu + \int f^- d\mu =
	\int |f| d\mu
$$
Where the first inequality comes from the triangle theorem, and the second equality comes from $f^+$ and $f^-$ both being nonnegative. We also can apply to integrable $f$ and $g$, in which case $f - g$ is integrable and making use of Theorem 16.1 gives us:
$$
	\left|\int f d\mu - \int g d\mu\right| =
	\left|\int f - g d\mu\right| \leq
	\int \left|f - g\right| d\mu
$$
Not sure why this would be useful.

\paragraph{Example 16.1 - Counting Measure and Comparisons With Series Theory} Suppose that $\Omega$ is countable - $\Omega = \{1, 2, \cdots\}$ to be definite. Let $\fcal$ consist of all subsets of $\Omega$, and $\mu$ is the counting measure - each singleton has measure $1$.
\\\\
A function is thus a sequence, essentially, where $x_i = f(i)$. If $x_{nm}$ is $x_m$ if $m \leq n$ or $0$ if $m > n$, the function $f_n$ corresponding to $x_{n1}, x_{n2}, \cdots$ has integral $\sum_{m=1}^n x_m$ by Theorem 15.1.1, as it is a simple function essentially on the decomposition $\{1\}, \cdots, \{n\}, \{n+1, n+2, \cdots\}$. It follows by Theorem 15.1.3, that for nonnegative $f$, the integral of the function $f(m) = x_m$ is the sum:
$$
	\sum_m x_m
$$
Finite or infinite, of the corresponding infinite series. In the general case - we have $f^+$ and $f-$ are nonnegative, with integrals that can be similarly expressed as $\sum_m x_m$. Thus, general $f$ is \textit{integrable} iff both of these sums are finite, which follows iff $\sum_m |x_m|$ is a convergent infinite series (ie sum is less than infinity). In this case, the integral is $\sum_{m=1}^\infty x_m^+ - \sum_{m=1}^\infty x_m^-$.
\\\\
The function $x_m = (-1)^{m+1}m^{-1}$ is not integrable by this definition. That is because the absolute value sum is the non-convergent harmonic series. It fails to even have a definite integral, as $\sum_{m=1}^\infty x_m^+ = \sum_{m=1}^\infty x_m^- = \infty$ (every other sum on the harmonic series is still infinite).
\\\\
Compare this with infinite series theory. We have that the alternating harmonic series does converge in the sense that:
$$
	\lim_M \sum_{m=1}^M (-1)^{m+1}m^{-1} = \log(2)
$$
However - note that this sum is for the \textit{first $M$ elements}. It requires an order on the space $\Omega$. Consider instead, for example the 3d integer coordinates - the integer lattice points in 3 space. It has no canonical linear ordering. If $\sum_m x_m$ is to have the same finite value no matter the order of summation, the series must be absolutely convergent, which is standard real analysis theory. This helps explain why $f$ is defined to be integrable if and only if both $\int f^+ d\mu$ and $\int f^- d\mu$ are both finite - then, we have absolute convergence (at least in the counting measure example), which implies the sum is the same, no matter the reordering. 

\paragraph{Example 16.2 - Principal Value} I've included this example because it was something I was wondering about in section 15, defining integrals for which $\int f^+ d\mu = \int f^- d\mu = \infty$ by examining $\lim_n \int_{-n}^n f$.
\\\\
Consider the function $f(x) = 3\1{a \leq x} - 2\1{x < a}$. There is no natural value for $\int f d\lambda$, as it is the $\infty - \infty$ case. But, if a function $f$ is bounded on bounded intervals, then $f_n(x) = f(x)\1{-n \leq x \leq n}$ is integrable with respect to $\lambda$. Since $f = \lim_n f_n$, the limit of $\int f_n d\lambda$, it if exists, is sometimes called the \textit{principal value} of the integral $f$.
\\\\
Note, however, this is not the right definition in the context of general measure theory. The functions $g_n(x) = f(x)\1{-n \leq x \leq n+1}$ also converges to $f$ - however, we may have $\lim_n \int g_n d\lambda \neq \lim_n \int f_n d\lambda$. Take $f(x) = x$ as one such example. There is no general reason why $f_n$, which I guess is symmetrical around the origin, should take precedence over $g_n$. 

\subsubsection{Integration to the Limit}
\paragraph{Theorem 16.2 - Monotone Convergence Theorem} If $0 \leq f_n \uparrow f$ almost everywhere, then $\int f_n d\mu \uparrow \int f d\mu$.
\\\\
\textbf{Proof:} This is essentially Theorem 15.1.3, except we have added an almost everywhere. We have $0 \leq f_n \uparrow f$ on a set $A$ with $\mu(A^c) = 0$. So, we can use an indicator, and make use of Theorem 15.2.5:
$$
	\int f_n d\mu = \int f_n\1{A} d\mu \uparrow \int f\1{A} d\mu = \int f d\mu
$$

\paragraph{Theorem 16.3 - Fatou's Lemma} For nonnegative $f_n$:
$$
	\int \liminf_n f_n d\mu \leq \liminf_n \int f_n d\mu
$$
\textbf{Proof:} If $g_n = \inf_{k \geq n} f_k$, then $0 \leq g_n \uparrow g = \liminf_n f_n$ (this is by definition, the limit exists for each $x$ as it is monotone increasing). Theorem 16.1 gives monotonicity for $g_n \leq f_n$, and Theorem 16.2 monotone convergence theorem gives:
$$
	\int f_n d\mu \geq \int g_n d\mu \uparrow \int g d\mu
$$
As this is true for all $n$, we have:
$$
	\int g d\mu = \int \liminf_n f_n d\mu \leq \liminf_n \int f_n d\mu
$$
And thus we have concluded the theorem. I think it might be better to note that:
$$
	\liminf_n \int f_n d\mu \geq \liminf_n \int g_n d\mu = \int g d\mu =
	\int \liminf_n f_n d\mu
$$
We start with liminfs on both sides, as we only have the equality for fixed $n$, $\int f_n d\mu \geq \int g_n d\mu$. We can take liminfs of both side, which exist, and then note that the liminf equals the limit if it exists, which is our integral of $g$. That gives us the theorem for nonnegative $f_n$. qed.

\paragraph{Example 16.6 - Fatou's Lemma Strict Inequality} On $(\R^1, \rcal^1, \lambda)$, the functions $f_n = n^2\1{(0,n^{-1})}$ have integral value $n^2 * n^{-1} = n$. Note that for $f = 0$ though, $f_n(x) \to f(x)$ for each $x$.
\\\\
Thus, $\liminf_n f_n = f$, so:
$$
	0 = \int f d\mu = \int \liminf_n f d\mu \quad\quad\quad
	\infty = \liminf_n n = \liminf_n \int f_n d\mu
$$
In this case, the inequality is strict. Note, it also is not always possible ot ingrate to the limit.

\paragraph{Theorem 16.4 - Dominated Convergence Theorem} If $|f_n| \leq g$ almost everywhere, where $g$ is integrable, and if $f_n \to f$ almost everywhere, then $f$ and the $f_n$ are integrable and $\int f_n d\mu \to \int f d\mu$.
\\\\
\textbf{Proof:} Assume at the outset only that the $f_n$ are dominated by an integrable $g$. Note, $|f_n| \leq g$ implies $g$ is nonnegative, and so $|f_n| \leq |g|$ almost everywhere implies that $f_n$ is integrable given that $g$ is, by the note at the start of the chapter. Note, we also must have:
$$
	|\liminf_n f_n| \leq |g|
	\quad\quad\quad
	|\limsup_n f_n| \leq |g|
$$
This again is clear via liminf and limsup maintaining inequalities. Thus, both $f^* = \limsup_n f_n$ and $f_* = \liminf f_n$ are integrable as well. Recall, we already have $f^*$ and $f_*$ are measurable by Theorem 13.4. As $g$ is measurable and nonnegative, we have $g + f_n$ and $g - f_n$ are nonnegative. And so, we can make use of Fatou's lemma to find:
$$
	\int g d\mu + \int f_* d\mu = \int \liminf_n (g + f_n) d\mu 
	\leq
	\liminf_n \int (g + f_n) d\mu = \int g d\mu + \liminf_n \int f_n d\mu
$$
Note, we had to pass to $g + f_*$ as Fatou's lemma can't be applied to not necessarily non-negative $f_n$. Similarly, we find:
$$
	\int g d\mu - \int f^* d\mu = \int g - \limsup_n f_n d\mu =
	\int \liminf (g - f_n) d\mu
$$
Note, the equality is valid at each $x$. By Fatou's lemma, we have:
$$
	\leq \liminf_n \int g - f_n d\mu = \int g d\mu - \limsup_n \int f_n d\mu
$$
Subtract $\int g d\mu$ from both sides of the inequalities we have derived above, and reorder to find:
$$
	\int \liminf_n f_n d\mu \leq
	\liminf_n \int f_n d\mu \leq
	\limsup_n \int f_n d\mu \leq
	\int \limsup_n f_n d\mu
$$
Note, we had something similar in Theorem 4.1. If we assume that $f_n \to f$ almost everywhere, then note that $\limsup_n f_n(x) = \liminf_n f_n(x)$ almost everywhere. Both the upper and lower values of the inequality are equal, to the common value of $\int f d\mu$, which exists as $f$ must be similarly dominated by $g$. Thus, the limit of the integrals exist, as the inner liminf and limsup are equal, and we can conclude:
$$
	\lim_n \int f_n d\mu = \int f d\mu
$$
Thus, we have proven the Dominated Convergence Theorem. qed.
\\\\
Note - the Dominated Convergence Theorem could be considered just like a corollary to Fatou's lemma. Essentially, if $|f_n| \leq g$ almost everywhere, and $g$ is integrable, then we can conclude:
$$
	\int \liminf_n f_n d\mu \leq
	\liminf_n \int f_n d\mu \leq
	\limsup_n \int f_n d\mu \leq
	\int \limsup_n f_n d\mu
$$
Note, the left hand side is given to us by Fatou's Lemma, but the right hand side is derived from Fatou's lemma, making use of our dominating $g$ to create the integrable function $g - \limsup_n f_n = \liminf_n g - f_n$ to which we can apply Fatou's lemma.

\paragraph{Theorem 16.5 - Bounded Convergence Theorem} If $\mu(\Omega) < \infty$ and the $f_n$ are uniformly bounded, then $f_n \to f$ almost everywhere implies $\int f_n d\mu \to \int f d\mu$.
\\\\
\textbf{Proof:} This is just a special case of the dominated convergence theorem. As the $f_n$ are uniformly bounded, this means there is a finite $K$ such that $|f_n(\omega)| \leq K$ for all $\omega$ and $n$ (recall, $f_n$ are all measurable on a measurable space $(\Omega,\fcal,\mu)$). We let $g = \1{\Omega}K$. Theorem 15.1.i tells us that $\int g d\mu = \mu(\Omega)K < \infty$, and so $g$ is integrable. As $|f_n| \leq g = K$ almost everywhere, where $g$ is integrable, and $f_n \to f$ almost everywhere, then Dominated Convergence Theorem tells us that $f$ and $f_n$ are integrable, and:
$$
	\int f_n d\mu \to \int f d\mu
$$

\paragraph{Theorem 16.6 - Monotone Convergence Theorem For Series} If $f_n \geq 0$, then $\int \sum_n f_n d\mu = \sum_n \int f_n d\mu$. This is clear via monotone convergence theorem for $g_n = \sum_{k=1}^n f_k$ and $g = \sum_n f_n$. qed.

\paragraph{Theorem 16.7 - Dominated Convergence Theorem For Series} If $\sum_n f_n$ converges almost everywhere and $\left|\sum_{k=1}^n F_k\right| \leq g$ almost everywhere, where $g$ is integrable, then $\sum_n f_n$ and $f_n$ are integrable, and $\int \sum_n f_n d\mu = \sum_n \int f_n d\mu$. Again, same proof as before. qed.

\paragraph{Corollary 16.7.1} If $\sum_n \int |f_n| d\mu < \infty$, then $\sum_n f_n$ converges absolutely almost everywhere and is integrable, and $\int \sum_n f_n d\mu = \sum_n \int f_n d\mu$.
\\\\
\textbf{Proof:} The function $g = \sum_n |f_n|$ is integrable be Theorem 16.6, and is finite almost everywhere by Theorem 15.2.iii. Hence, we can apply Theorem 16.7, the Dominated Convergence Theorem For Series. qed.

\subsubsection{Integration to Continuous Limits} This is a pretty technical section. In place of a sequence $\{f_n\}$ of real measurable functions on $(\Omega,\fcal,\mu)$, consider a family $[f_t: t \geq 0]$ indexed by a continuous parameter $t$. Suppose of a measurable $f$ that:
$$
	\lim_{t \to \infty} f_t(\omega) = f(\omega)
$$
on a set $A$, where:
$$
	A \in \fcal \quad\quad\quad
	\mu(\Omega - A) = 0
$$
A technical point arises here - since $\fcal$ doesn't need to necessarily contain $A$. Consider the following example. Let $\fcal$ consist of the Borel subsets of $\Omega = [0,1)$, and let $H$ be a nonmeasurable set - a subset of $\Omega$ that does not lie in $\fcal$ (such as the Vitali Sets). Define:
$$
	f_t(\omega) =
	\begin{cases}
	1 & \text{ if $\omega = t - \floor{t}$, the fractional part of $t$, AND $\omega = t - \floor{t} \in H^c$}\\
	0 & \text{ otherwise}
	\end{cases}
$$
Each $f_t$ is measurable $\fcal$. That is what the book states at least - I think this would rely on the set where $f_t(\omega) = 1$ being measurable, which is a subset of $H^c$. I think it is measurable - there should only be one point $\omega = t - \floor{t} \in [0,1)$ - so $f_t$ is either identically equal to 0, or a one hot function, which is measurable (single points are an infinite intersection of $[\omega,\omega+\epsilon)$). However, if $f = 0$, the set where $\lim_{t \to \infty} f_t(\omega) = f(\omega)$ holds is $H$ exactly - indeed, if $\omega \in H^c$, then the limit does not exist, as there will always be points $t > \infty$ such that $\omega = t - \floor{t}$, and if $\omega \in H$, we have $f_t(\omega) = 0$ for all $t$. 
\\\\
Because of the above example, we actually need to assume that $A$ is measurable. Now, assume that $f$ and $f_t$ are integrable. If $I_t = \int f_t d\mu$ converges to $I = \int f d\mu$ as $t \to \infty$, we have convergence for $I_{t_n} \to I$ for $t_n \to \infty$ a sequence. If $I_t \not\to I$, this similarly implies there is a sequence $t_n \to \infty$ such that $|I_{t_n} - I| > \epsilon$. The question of whether or not $I_{t_n}$ converges to $I$ can be answered with our previous theorems, which have to do with convergence on sequences.
\\\\
Suppose that $|f_t(\omega)| \leq g(\omega)$ holds for $\omega \in A$, where $A \in \fcal$ and $\mu(A^c) = 0$, and $g$ is integrable. Also, suppose that $\lim_{t \to \infty} f_t(\omega) = f(\omega)$ on $A$ as well. We can make use of the Dominated Convergence Theorem to 1. Find that both $f$ and $f_t$ are integrable and 2. $I_{t_n} \to I$ for every sequence $t_n \to \infty$. Note, as this is true for all sequences $t_n$, by definition, $I_t \to I$. Note, this result would follow if $t$ went continuously to $0$ or some other value. Also note, Dominated Convergence Theorem applies as we have $|f_t| \leq g$ almost everywhere.

\paragraph{Theorem 16.8 - Leibnitz's Rule For Lebesgue Integrals} suppose that $f(\omega,t)$ is a measurable and integrable function of $\omega$ for each $t \in (a,b)$. Let $\varphi(t) = \int f(\omega, t) \mu(d\omega)$ (note, the notation here helps specify that we are integrating across $\omega$). 
\begin{enumerate}
	\item Suppose that for $\omega \in A$, where $A$ satisfies $A \in \fcal$ and $\mu(\Omega - A) = 0$, $f(\omega, t)$ is continuous in $t$ at $t_0 \in (a,b)$ - ie, $\lim_{t \to t_0} f(\omega, t) = f(\omega, t_0)$. Suppose further that $|f(\omega, t)| \leq g(\omega)$ for $\omega \in A$ and $|t - t_0| < \delta$, where $\delta$ is independent of $\omega$ and $g$ is integrable. Then $\varphi(t)$ is continuous at $t_0$.
	
	\item Suppose that for $\omega \in A$, where $A$ satisfies $A \in \fcal$ and $\mu(\Omega - A) = 0$, $f(\omega, t)$ has in $(a,b)$ a derivative $f'(\omega,t)$; suppose further that $|f'(\omega, t)| \leq g(\omega)$ for $\omega \in A$ and $t \in (a,b)$, where $g$ is integrable. Then $\varphi(t)$ has a derivative on $(a,b)$:
	$$
		\varphi'(t) = \int f'(\omega, t) \mu(d\omega)
	$$
\end{enumerate}
\textbf{Proof:}
\begin{enumerate}
	\item Note, this follows directly from our previous discussion. We have for each sequence $t_n \to t$, eventually $|f(\omega, t_n)| \leq g(\omega)$ once we have $|t - t_0| < \delta$ for $\omega \in A$, at which point we can switch over to the dominated convergence theorem, as we have $f_{t_n}(\omega) = f(\omega, t_n) \to f(\omega, t_0)$ via continuity (note, the domination of $g$ and convergence all occur on the same almost everywhere set $A$). Thus, $I_{t_n} \to I_{t_0}$ for all sequences $t_n \to t_0$, and so:
	$$
		\lim_{t_n \to t_0} \varphi(t_n) = \lim_{t_n \to t_0} I_{t_n} = I_{t_0} = \varphi(t_0)
	$$
	
	\item Note, this is essentially the same proof through the Dominated Convergence Theorem. Consider a fixed $t$. If $\omega \in A$, then by the mean value theorem:
	$$
		\frac{f(\omega, t + h) - f(\omega, t)}{h} = f'(\omega, s)
	$$
	For some $s \in (t, t+h)$. By definition of the derivative, the ratio on the left goes to $f'(\omega, t)$ as $h \to 0$ (note, this can be done through a sequence). By hypothesis, we have that each entry in the sequence is dominated by $g$ on $A$ - ie, $|f'(\omega, s)| \leq g(\omega)$. Therefore, via the dominated convergence theorem:
	$$
		\frac{\varphi(t + h) - \varphi(t)}{h} =
		\int \frac{f(\omega, t + h) - f(\omega, t)}{h} \mu(d\omega) \to
		\int f'(\omega, t) \mu(d\omega)
	$$
	Where the above follows for any sequence of $h \to 0$, and the dominated convergence theorem applies. Thus, we have:
	$$
		\varphi'(t) = \int f'(\omega, t) \mu(d\omega)
	$$
	For $t \in (a,b)$. Note, the condition involving $g$ can be weakened. It suffices to assume that for each $t$, there is an integrable $g(\omega, t)$ such that $|f'(\omega, s)| \leq g(\omega, t)$ for $\omega \in A$ and $s$ in some neighborhood of $t$ - note, this is essentially what we needed to be able to apply the Dominated Convergence Theorem. qed.
\end{enumerate}

\subsubsection{Integration Over Sets}
The integral of $f$ over a set $A$ in $\fcal$ is defined by:
$$
	\int_A f d\mu = \int \1{A} f d\mu
$$
Note that the integral equals zero if $\mu(A) = 0$. All the concepts and theorems previously stated carry over in an obvious way to integrals over $A$.

\paragraph{Theorem 16.9 - Sum Of Integrals Over Sets} If $A_1, A_2, \cdots$ are disjoint, and if $f$ is either nonnegative or integrable, then:
$$
	\int_{\bigcup_n A_n} f d\mu =
	\sum_n \int_{A_n} f d\mu
$$
\textbf{Proof:} This follows directly from Theorem 16.6, the series version of monotone convergence, and Theorem 16.7, the series version of dominated convergence (as each $\|\1{A_n}f\| \leq f$, and $f$ is integrable). Just express the union as a summation. qed.

\paragraph{Theorem 16.10 - Integral Values Determine $f$}
\begin{enumerate}
	\item If $f$ and $g$ are nonnegative and $\int_A f d\mu = \int_A gd\mu$ for all $A$ in $\fcal$, and if $\mu$ is $\sigma$-finite, then $f = g$ almost everywhere.
	
	\item If $f$ and $g$ are integrable and $\int_A f d\mu = \int_A g d\mu$ for all $A$ in $\fcal$, then $f = g$ almost everywhere.
	
	\item If $f$ and $g$ are integrable and $\int_A f d\mu = \int_A g d\mu$ for all $A$ in $\pcal$, where $\pcal$ is a $\pi$ system generating $\fcal$ and $\Omega$ is a finite or countable union of $\pcal$ sets, then $f = g$ almost everywhere.
\end{enumerate} 
Note, we need different cases, as nonnegative does not necessarily imply integrable. The first case will deal with infinite values.
\\\\
\textbf{Proof:} \begin{enumerate}
	\item Suppose that $f$ and $g$ are nonnegative and that $\int_A f d\mu \leq \int_A g d\mu$ for all $A \in \fcal$. If $\mu$ is $\sigma$ finite, we can find $A_n \in \fcal$ such that $A_n \uparrow \Omega$ and $\mu(A_n) < \infty$. Note, take the $B_1, B_1 \cup B_2, B_1 \cup B_2 \cup B_3$ and so on, and note that each union measure is bounded by a finite sum of finite values. Define now:
	$$
		B_n = \left[0 \leq g < f, g \leq n\right]
	$$
	$B_n \in \fcal$ via $g, f$ measurable. Note, the hypothesis applies to $A_n \cap B_n$, so:
	$$
		\int_{A_n \cap B_n} f d\mu \leq \int_{A_n \cap B_n} g d\mu < \infty
	$$
	Where the final equality comes from $\mu(A_n \cap B_n) \leq \mu(A_n) < \infty$ and $g \leq n$ on that set, so the integral is less than $\mu(A_n) * n$. Hence, we have that the subtraction is well defined:
	$$
		\int_{A_n \cap B_n} f - g d\mu = 0
	$$
	Where $= 0$ comes from the initial hypothesis. By Theorem 15.2(ii), if the measure where the function $\1{A_n \cap B_n} f - g$ was greater than 0, the integral would be greater than 0. So, we must have:
	$$
		\mu(\1{A_n \cap B_n}(f - g) > 0) = 0
	$$
	Now, note that:
	$$
		A_n \cap B_n \subseteq \left\{\1{A_n \cap B_n}(f - g) > 0\right\}
	$$
	As if $\omega \in A_n \cap B_n$, we have $\omega \in B_n \implies g(\omega) < f(\omega) \implies f(\omega) - g(\omega) > 0$, and $\1{A_n \cap B_n}(\omega) = 1$. And so, we can conclude that $\mu(A_n \cap B_n) = 0$ via the monotonicity of the measure $\mu$. As $A_n \cap B_m \uparrow \Omega \cap B_m$, this implies:
	$$
		\mu(B_m) = \lim_{n \to \infty} \mu(A_n \cap B_m) = 0
	$$
	Note, everything above would have followed for $m \neq n$. Now, note that $B_m \uparrow \{0 \leq g < f, g < \infty\}$, and so:
	$$
		\mu\left[0 \leq g < f, g < \infty\right] = 0
	$$
	This implies that $f \leq g$ almost everywhere. Indeed, we find:
	$$
		\left\{f \leq g\right\}^c = \left\{g < f\right\} =
		\left\{0 \leq g < f, g < \infty\right\}
	$$
	The first equality is clear. As for the second - note first that $0 \leq g$, so we can add it easily. Note second that if $g = \infty$, $g < f$ is not possible, as there is no number on our extended real line that exceeds infinity. So, we can add that extra condition without losing any information. And so, we have $f \leq g$ almost everywhere, and $g \leq f$ almost everywhere symmetrically. The intersection of almost everywhere events is still almost everywhere, and so:
	$$
		g = f \text{ almost everywhere}
	$$
	
	\item If $f$ and $g$ are integrable, we have a simpler argument, as we can take the integral subtraction without any added $A_n \cap B_n$ headache. We have $\int_A f d\mu \leq \int_A g d\mu$ for all $A \in F$, and so:
	$$
		\int \1{g < f} (f - g) d\mu = 0 \implies \mu(g < f) = 0
	$$
	Where the implication comes from the same argument we made for $\mu(A_n \cap B_n) = 0$ above with Theorem 15.2(ii). And so, $g = f$ almost everywhere, as $\mu((g = f)^c) \leq \mu(g < f) + \mu(f < g) = 0$.
	
	\item First, assume that $f$ and $g$ are nonnegative. We have that the following are measures on $\fcal$:
	$$
		\nu_f(A) = \int_A f d\mu \quad\quad\quad
		\nu_g(A) = \int_A g d\mu
	$$
	Recall, to be a measure, we must have the measure of the emptyset is zero, the value of the measure is nonnegative for all $A \in \fcal$, and countable additivity. As $f$ and $g$ are nonnegative, the integrals and thus measures are always nonnegative; clearly, $\nu_f(\emptyset) = 0 = \nu_g(\emptyset)$ as well. Also just a note, $f$ and $g$ being nonnegative imply that $\1{A}f$ always has an integral value. Finally, note that countable additivity comes from Theorem 16.9. So, we indeed have measures. Our condition implies equality of the measures for all $A \in \pcal$.
	\\\\
	Further, we have for all $A \in \fcal$, $\nu_f(A) < \infty, \nu_g(A) < \infty$, as $f$ and $g$ are integrable, and so the corresponding integrals must be finite. So, together with the assumption that $\Omega$ is a finite or countable union of $\pcal$ sets, we can make use of Theorem 10.4 to conclude that:
	$$
		\nu_f(A) = \nu_g(A) \text{ for all $A \in \fcal$} \implies
		\int_A f d\mu = \int_A g d\mu \text{ for all $A \in \fcal$}
	$$
	So now, we need to extend to the general $f$ and $g$ integrable. Well, note for $A \in \pcal$, we still have:
	$$
		\int_A f d\mu = \int_A g d\mu \implies
		\int_A f^+ d\mu - \int_A f^- d\mu = \int_A g^+ d\mu - \int_A g^- d\mu
	$$
	$$
		\implies
		\int_A f^+ + g^- d\mu = \int_A g^+ + f^- d\mu
	$$
	Note, as $f^+ + g^-$ and $g^+ + f^-$ are nonnegative, we make use of the same argument to find $f^+ + g^- = g^+ + f^-$ almost everywhere, which implies $f = g$ almost everywhere in the general case. qed.
	
\end{enumerate}

\subsubsection{Densities}
Suppose that $\delta$ is a nonnegative measurable function and defined a measure $\nu$ by:
$$
	\nu(A) = \int_A \delta d\mu
$$
In the previous Theorem 16.10.3, we noted that $\nu$ is indeed a measure. You don't even need to assume that $\nu$ is integrable - just nonnegative, in which case the integral can be finite or infinite. Note that $\mu(A) = 0 \implies \nu(A) = 0$ via Theorem 15.2ii. Further, $\nu$ is finite if and only if $\delta$ is integrable $\mu$ - this should be clear as well.
\\\\
Another function $\delta'$ gives rise to the same $\nu$ if $\delta = \delta'$ almost everywhere. On the other hand, both having the same $\nu$ imply the functions are equal via Theorem 16.10.1 (the first assumption of densities is that $\delta$ are nonnegative).
\\\\
The measure $\nu$ defined by above is said to have \textit{density} $\delta$ with respect to $\mu$. Note - this is a good name - as density times volume (given by $A$ here) should give you a mass (given by $\nu(A)$). 

\paragraph{Theorem 16.11 - Integral Across Measures With Density} If $\nu$ has density $\delta$ with respect to $\mu$, then:
$$
	\int f d\nu = \int f\delta d\mu
$$
Holds for nonnegative $f$. Moreover, $f$ (not necessarily nonnegative) is integrable with respect to $\nu$ if and only if $f\delta$ is integrable with respect to $\mu$, in which case:
$$
	\int f d\nu = \int f\delta d\mu \text{ and }
	\int_A f d\nu = \int_A f\delta d\mu
$$
For nonnegative $f$, the above always holds as well.
\\\\
\textbf{Proof:} First, take $f = \1{A}$ for $A \in \fcal$. Note that both $\nu$ and $\mu$ are measures on $\fcal$. We have:
$$
	\int f d\nu = \nu(A) = \int_A \delta d\mu = \int f\delta d\mu
$$
Where the first equality makes use of Theorem 15.1.1 and the remaining inequalities are definitions. If $f$ is a simple nonnegative function, then equality follows via linearity of the integral. Finally, for $f$ nonnegative, then:
$$
	\int f_n d\nu = \int f_n\delta d\mu
$$
For $f_n \uparrow f$ simple, which exists by Theorem 13.5. Then, we can pass to the integral via the monotone convergence theorem - Theorem 16.2. So, we have proved the first statement for nonnegative $f$.
\\\\
Now, if $f$ is not nonnegative, we can apply the argument to $|f|$, and so if $f$ is integrable with respect to $\nu$, we have:
$$
	\int |f| d\nu = \int |f|\delta d\mu = \int |f\delta| d\mu
$$
The second equality follows by definition of a density, which must be nonnegative, and so it is clear that $f$ is integrable wrt $\nu$ iff $f\delta$ is integrable wrt $\mu$. And if $f$ is integrable, we have that:
$$
	\int f d\nu = \int f^+ d\nu - \int f^- d\nu = 
	\int (f\delta)^+ d\mu - \int (f\delta)^- d\mu = \int f\delta d\mu
$$
Replacing $f$ by $f\1{A}$ gives us the proofs for integrals across subsets. qed.

\paragraph{Theorem 16.12 - Convergence of Densities Implies Convergence Of Measures} Suppose that $\nu_n(A) = \int_A \delta_n d\mu$ and $\nu(A) = \int_A \delta d\mu$ for densities $\delta_n$ and $\delta$. If:
$$
	\nu_n(\Omega) = \nu(\Omega) < \infty \text{ for $n = 1, 2, \cdots$}
$$
And if $\delta_n \to \delta$ except on a set of $\mu$ measure 0, then:
$$
	\sup_{A \in \fcal} |\nu(A) - \nu_n(A)| \leq \int_\Omega |\delta - \delta_n| d\mu \to 0
$$
\textbf{Proof:} Earlier, we proved via monotonicity, that $\left|\int f d\mu \right| \leq \int |f| d\mu$. And so, we can immediately conclude our inequality:
$$
	\sup_{A \in \fcal} |\nu(A) - \nu_n(A)| =
	\sup_{A \in \fcal} \left|\int_A \delta - \delta_n d\mu\right| \leq
	\sup_{A \in \fcal} \int_A |\delta - \delta_n| d\mu \leq
	\int_\Omega |\delta - \delta_n| d\mu
$$
Note, the subtraction is well defined by monotonicity, and $\nu(\Omega) < \infty$. Let $g_n = \delta - \delta_n$. The positive part $g_n^+$ of $g_n$ converges to $0$ except on a set of $\mu$ measure 0. As $\delta$ and $\delta_n$ are nonnegative densities, we also have $0 \leq g_n^+ \leq \delta$ and $\delta$ is integrable across $\mu$ (again, via $\nu(\Omega) < \infty$ and everything is nonnegative), and so the dominated convergence theorem applies:
$$
	\int g_n^+ d\mu \to 0
$$
Also note that:
...
$$
	\int_\Omega |g_n| d\mu =
	\int_{[g_n \geq 0]} g_n d\mu - \int_{[g_n < 0]} g_n d\mu =
	2\int_{[g_n \geq 0]} g_n d\mu = 2\int_\Omega g_n^+ d\mu \to 0
$$
The second equality coming from the fact that as the complete integral of $g_n$ equals zero, the negative and positive integral portions must be equal. And thus, we have found that:
$$
	\int_\Omega |\delta - \delta_n| d\mu = \int_\Omega |g_n| d\mu \to 0
$$
Note, we had to go through the above fuss because we were dealing with the absolute value of the difference, not just the difference. The difference equaling zero came directly from our assumptions. qed.

\paragraph{Corollary 16.12 - Infinite Series} If $\sum_m x_{nm} = \sum_m x_m < \infty$, the terms being nonnegative, and if $\lim_n x_{nm} = x_m$ for each $m$, then:
$$
	\lim_n \sum_m |x_{nm} - x_m| = 0
$$
If $y_m$ is bounded, then $\lim_n \sum_m y_m x_{nm} = \sum_m y_m x_m$.
\\\\
Note, this is similar to the Weierstrauss $M$ test. Note that it follows from taking $\mu$ as the counting measure on $\Omega = \{1, 2, \cdots\}$, and using Theorem 16.12.

\subsubsection{Change of Variable} Let $(\Omega, \fcal)$ and $(\Omega', \fcal')$ be measurable spaces, and suppose that the mapping $T: \Omega \to \Omega'$ is measurable by $\fcal/\fcal'$. For a measure $\mu$ on $\fcal$, define measure $\mu T^{-1}$ on $\fcal'$ by:
$$
	\mu T^{-1}(A') = \mu(T^{-1}(A')) \text{ where } A' \in \fcal'
$$
Recall, we already discussed this at the end of section 13. This is a pullback measure, as we pull $A'$ back into $\fcal$ via $T^{-1}$.
\\\\
Suppose $f$ is a real function on $\Omega'$ that is measurable $\fcal'$, so that the composition $fT$ is a real function on $\Omega$ that is measurable $\fcal$. Composition of measurable functions is measurable, this is a pushforward I believe. 

\paragraph{Theorem 16.13 - Change Of Variables} If $f$ is nonnegative real measurable function on $\Omega'$, then:
$$
	\int_\Omega f(T\omega) \mu(d\omega) = \int_{\Omega'} f(\omega') \mu T^{-1}(d\omega')
$$
A function $f$ (not necessarily nonnegative) is integrable with respect to $\mu T^{-1}$ if and only if $fT$ is integrable with respect to $\mu$, in which case the above holds, as well as:
$$
	\int_{T^{-1}A'} f(T\omega) \mu(d\omega) = \int_{A'} f(\omega') \mu T^{-1}(d\omega')
$$
For nonnegative $f$, the above always holds as well.
\\\\
Note, this is the same problem as in Theorem 16.13 - it is badly stated. I think we just have both expressions hold for nonnegative $f$, and as well for general $f$ integrable, where we also have the if and only if statement. Note also - the $'$ and $\mu(d\omega)$ help us understand what we are integrating over. On $\fcal'$, we use the pull back measure, along with $f$ as defined, whereas on $\fcal$ we use the original measure, and the pushforward $fT$. Oh, I think the Theorem is stated as is, as we first have to prove the expression for nonnegative $f$, which gives us the iff, which gives us the expression for integrable $f$, along with the integral over subsets expression, which we can then conclude for nonnegative $f$ as well.
\\\\
\textbf{Proof:} If $f = \1{A'}$, then $fT = \1{T^{-1}A'}$, and so we have that:
$$
	\int_\Omega f(T\omega) \mu(d\omega) =
	\mu(T^{-1}(A')) =
	\int_{\Omega'} f(\omega') \mu T^{-1}(d\omega')
$$
By linearity, the equation holds for nonnegative simple functions as well. If $f_n$ are simple functions for which $0 \leq f_n \uparrow f$, then $0 \leq f_nT \uparrow fT$ as well. Indeed, for $\omega$, we have:
$$
	\lim_n f_nT(\omega) = \lim_n f_n(T\Omega)) = f(T(\Omega)) = fT(\Omega)
$$
And so, we have for nonnegative $f$, via the Monotone Convergence Theorem:
$$
	\int_\Omega f(T\omega) \mu(d\omega) =
	\lim_n \int_\Omega f_nT(\omega) \mu(d\omega) =
	\lim_n \int_{\Omega'} f_n(\omega') \mu T^{-1}(d\omega') =
	\int_{\Omega'} f(\omega') \mu T^{-1}(d\omega')
$$
Now, we just have for any $f$, that:
$$
	\int_\Omega |f|(T\omega) \mu(d\omega) = \int_{\Omega'} |f|(\omega') \mu T^{-1}(d\omega')
$$
So, it is clear that $|f|T = |fT|$ is integrable with respect to $\mu$ if and only if $|f|$ is integrable with respect to $\mu T^{-1}$. Note, the first expression holds for integrable $f$ if we decompose into $f = f^+ - f^-$, and finally, we get integrals over subsets by replacing $f$ with $f\1{A}$. qed.

\paragraph{Example 16.10} Suppose that $(\Omega', \fcal') = (\R^1, \rcal^1)$ and $T = \varphi$ is an ordinary real function, measurable $\fcal$. If $f(x) = x$, the above becomes:
$$
	\int_\Omega \varphi(x) \mu(d\omega) = \int_{\R^1} x\mu\varphi^{-1}(dx)
$$
Actually, I think this example isn't great. However, what we can do is note that - we can get the general Change of Variables Theorem from multivariable calculus out of this Theorem. Let our measurable spaces be $(\R^n, \rcal^n)$ and $(\R^n, \rcal^n)$, and let $T^{-1}$ be nonsingular and linear. Let $\mu$ be the Lebesgue measure $\lambda$.  We can just replace $T$ with $T^{-1}$, and so for integrable/nonnegative $f$, Theorem 16.13 gives us for $A \in \rcal^n$:
$$
	\int_{T(A)} (f \circ T^{-1}) d\lambda = \int_A f(\omega') d(\lambda \circ T)
$$
Now, recall, Theorem 12.2 tells us that:
$$
	\lambda \circ T = |\det T|\lambda
$$
So, we can conclude (note, this can be done with the similar simple function, nonnegative function, integrable function argument):
$$
	\int_{T(A)} (f \circ T^{-1}) d\lambda = \int_A f(\omega') |\det T| d\lambda
$$
And we can just replace $f$ with $f \circ T$ to give us:
$$
	\int_{T(A)} f d\lambda = \int_A (f \circ T) |\det T| d\lambda
$$
Now, the only difference here is that in calculus, we can assume $g$ is 1-1 continuously differentiable, not just linear. I think there is a way to extend the above for such $g$ as well (not just $T$), but I'll leave it there for now.

\subsubsection{Uniform Integrability}
This section essentially introduces another criteria which can be used to find the convergence of integrals. \\\\
If $f$ is integrable, then $|f|\1{|f| \geq a}$ goes to $0$ almost everywhere as $a \to \infty$. If that was not the case, then there is a set of positive measure where $|f| = \infty$, which would contradict $\int |f| d\mu < \infty$.
\\\\
Hence, as $|f|\1{|f| \geq a}$ is dominated by $|f|$, the dominated convergence theorem gives us for integrable $f$:
$$
	\lim_{a \to \infty} \int_{[|f| \geq a]} |f| d\mu = 0
$$
We can use this notion to define a property of a sequence of functions $f_n$.

\paragraph{Definition - Uniformly Integrable} A sequence $\{f_n\}$ is \textit{uniformly integrable} if:
$$
	\lim_{a \to \infty} \sup_n \int_{[|f_n| \geq a]} |f_n| d\mu = 0
$$
Assume the property holds for $f_n$, and $\mu(\Omega) < \infty$. If $a$ is large enough such that the supremum is less than $1$, then:
$$
	\int |f_n| d\mu = \int_{[|f_n| \geq a]} |f_n| d\mu + \int_{[|f_n| < a]} |f_n| d\mu
	\leq 1 + \int_{[|f_n| < a]} |f_n| d\mu \leq 1 + a\mu(\Omega)
$$
Which implies that the $f_n$ are integrable.
\\\\
Now, assume that the $f_n$ are uniformly bounded, ie that $f_n(\omega) \leq K$ for each $n, \omega$. In this case, it is clear that uniform integrability always holds, as we just need $a \geq K$. In this case, the $f_n$ don't even need to be integrable, which can happen if $\mu(\Omega) = \infty$.
\\\\
If $h$ is the maximum of $|f|$ and $|g|$, then:
$$
	\int_{|f + g| \geq 2a} |f + g| d\mu \leq 2\int_{h \geq a} h d\mu =
	2\int_{|f| \geq a, |g| \geq a} h d\mu + 2\int_{|f| \geq a, |g| < a} h d\mu +
	2\int_{|f| < a, |g| \geq a} h d\mu
$$
$$
	=
	2\int_{|f| \geq a, |g| \geq a} h d\mu + 2\int_{|f| \geq a, |g| < a} f d\mu +
	2\int_{|f| < a, |g| \geq a} g d\mu
$$
$$
	=
	2\int_{|f| \geq a, |g| \geq a, |f| \geq |g|} |f| d\mu + 
	2\int_{|f| \geq a, |g| \geq a, |f| < |g|} |g| d\mu +
	2\int_{|f| \geq a, |g| < a} f d\mu +
	2\int_{|f| < a, |g| \geq a} g d\mu
$$
$$
	= 
	2\int_{|f| \geq a, |f| \geq |g|} |f| d\mu +
	2\int_{|g| \geq a, |f| \leq |g|} |g| d\mu
$$
$$
	\leq
	2\int_{|f| \geq a} |f| d\mu +
	2\int_{|g| \geq a} |g| d\mu	
$$
The first inequality holds as $2h \geq f + g$, and if $f + g \geq 2a$ then $2h \geq f + g \geq 2a \implies h \geq a$. So, the domain in the right integral is larger, and the function $2h \geq |f| + |g| \geq |f + g|$ is larger as well. The remaining equalities/inequalities are self-explanatory.
\\\\
This implies that if $\{f_n\}$ and $\{g_n\}$ are uniformly integrable, so is $\{f_n + g_n\}$. This is because:
$$
	\lim_{2a \to \infty} \sup_n \int_{[|f_n + g_n| \geq 2a]} |f_n + g_n| d\mu \leq
	\lim_{2a \to \infty} \sup_n 
	2\int_{|f_n| \geq a} |f_n| d\mu +
	2\int_{|g_n| \geq a} |g_n| d\mu	= 0
$$
Where the right side goes to zero via uniform integrability of $f_n$ and $g_n$.

\paragraph{Theorem 16.14 - Uniform Integrability Convergence} Suppose that $\mu(\Omega) < \infty$ and $f_n \to f$ almost everywhere.
\begin{enumerate}
	\item If the $f_n$ are uniformly integrable, then $f$ is integrable and:
	$$
		\int f_n d\mu \to \int f d\mu
	$$
	
	\item If $f$ and the $f_n$ are nonnegative and integrable, then:
	$$
		\int f_n d\mu \to \int f d\mu
	$$
	Implies that the $f_n$ are uniformly integrable.
\end{enumerate}
\textbf{Proof} We first prove that if the $f_n$ are uniformly integrable, then $f$ is integrable. We want to show:
$$
	\int |f| d\mu < \infty
$$
We have via our starting notes that give $\mu(\Omega) < \infty$, for some $a$ finite and large enough:
$$
	\int |f_n| d\mu \leq a\mu(\Omega) + 1 < \infty
$$
As $f_n \to f$ almost everywhere, we have $|f_n| \to |f|$ almost everywhere, ie on a set $A$ where $\mu(A^c) = 0$, and so:
$$
	\int |f| d\mu = \int_A |f| d\mu + \int_{A^c} |f| d\mu =
	\int_A \liminf |f_n| d\mu + \int_{A^c} |f| d\mu
$$
Where the second equality just comes from the limit. Now, via Fatou's Lemma, we have:
$$
	\leq 
	\liminf_{n \to \infty} \int_A |f_n| d\mu + \int_{A^c} |f| d\mu \leq
	a\mu(\Omega) + 1 + \int_{A^c} |f| d\mu =
	a\mu(\Omega) + 1 < \infty
$$
Where we note that $f \1{A^c} = 0$ almost everywhere. And so, with uniform integrability of $f_n$, $\mu(\Omega) < \infty$, $f_n \to f$ almost everywhere, and Fatou's Lemma, we can conclude that $f$ is integrable. Define:
$$
	f_n^{(a)} = 
	\begin{cases}
	f_n & \text{ if $|f_n| < a$}\\
	...
	\end{cases}
$$
If $\mu[|f| = a] = 0$, then $f_n^{(a)} \to f^{(a)}$ almost everywhere - we need the condition, because at $|f| = a$, for the $\omega$ that satisfy the condition, $|f_n^{(a)}|(\omega)$ could be bouncing from $0$ to the $|f_n|(\omega) \to |f|(\omega) \neq 0$ value. But, if this is the case, we can make use of the Bounded Convergence Theorem to find:
$$
	\int f_n^{(a)} d\mu \to \int f^{(a)} d\mu
$$
Since:
$$
	\int f_n d\mu - \int f_n^{(a)} d\mu =
	\int_{|f_n| \geq a} f_n d\mu
$$
Which can be found by using indicator definitions for $f_n^{(a)}$, and similarly:
$$
	\int f d\mu - \int f^{(a)} d\mu =
	\int_{|f| \geq a} f d\mu
$$
It follows from the above equations that:
$$
	\limsup_n \left|\int f_n d\mu - \int f d\mu\right| \leq
	\sup_n \int_{|f_n| \geq a} |f_n| d\mu + \int_{|f| \geq a} |f| d\mu
$$
As for why that is:
$$
	\limsup_n \left|\int f_n d\mu - \int f d\mu\right| =
	\limsup_n \left|\int f_n d\mu - \int f_n^{(a)} d\mu + \int f^{(a)} d\mu - \int f d\mu\right|
$$
Where the middle portion goes to zero, as the limit between the two values exists. This becomes via our above equalities:
$$
	=
	\limsup_n \left|\int_{|f_n| \geq a} f_n d\mu - \int_{|f| \geq a} f d\mu\right| \leq
	\sup_n \left|\int_{|f_n| \geq a} f_n d\mu - \int_{|f| \geq a} f d\mu\right|
$$
Where the above comes from the supremum always being greater than the lim sup. Continuing with the triangle inequality and our note at the start of the chapter:
$$
	\leq \sup_n \left|\int_{|f_n| \geq a} f_n d\mu\right| + \left|\int_{|f| \geq a} f d\mu\right| \leq
	\sup_n \int_{|f_n| \geq a} |f_n| d\mu + \int_{|f| \geq a} |f| d\mu
$$
Now, we just take $a \to \infty$ on the RHS. Note, as $f$ is measurable, there are only countably many points where $\mu[|f| = a] \neq 0$ - if there were more than countably many, we would contradict Theorem 10.2.iv. We take a sequence $a_n \to \infty$ that avoids these points, which allows us to conclude:
$$
	\limsup_n \left|\int f_n d\mu - \int f d\mu\right| \leq \lim_{a_n}\sup_n \int_{|f_n| \geq a_n} |f_n| d\mu + \int_{|f| \geq a_n} |f| d\mu
$$
$$
	\leq \lim_{a_n} 0 + \int_{|f| \geq a_n} |f| d\mu
$$
Where the 0 comes from uniform integrability. As for the remaining portion, $a_n$ being integrable implies that the right integral goes to 0 as well, and so:
$$
	\limsup_n \left|\int f_n d\mu - \int f d\mu\right| = 0
	\implies
	\lim_n \left|\int f_n d\mu - \int f d\mu\right| = 0
$$
$$
	\implies \int f_n d\mu \to \int f d\mu
$$
Now, suppose that the limit holds, where $f_n$ and $f$ are nonnegative and integrable. We want to show that $f_n$ is uniformly integrable. If $\mu[|f| = a] = 0$ holds, like before, we can find:
$$
	\int f_n^{(a)} d\mu \to \int f^{(a)} d\mu
$$
And from our subtractions, and $\int f_n d\mu \to \int f d\mu$ as well, we can find:
$$
	\int_{f_n \geq a} f_n d\mu \to \int_{f \geq a} f d\mu
$$
Absolute values in the domains removed by non-negativity. As $f$ is integrable, there is, for given $\epsilon$, an $a$ such that the right hand side above is less than $\epsilon$ and $\mu[f = a] = 0$. But then, the integral on the left is similarly less than $\epsilon$ for all $n$ exceeding some $n_0$. Thus:
$$
	\sup_{n \geq n_0} \int_{f_n \geq a} f_n d\mu < \epsilon
$$
For $n_0$ corresponding to our $a$. Now, if we let $a$ be large enough, for the first $n_0 - 1$ functions, they can be found to be less than $\epsilon$ as well, given integrability implies there is always some $a_1, \cdots, a_{n_0-1}$ for each of the first $n_0 - 1$ functions for which the integrals are less than $\epsilon$. So, for $\epsilon$ chosen, and corresponding $a$, and then corresponding $n_0$, and then $a = \max(a, a_1, \cdots, a_{n_0 - 1})$, we have:
$$
	\sup_n \int_{f_n \geq a} f_n d\mu < \epsilon
$$
And this implies:
$$
	\lim_{a \to \infty} \sup_n \int_{f_n \geq a} f_n d\mu = 0
$$
For nonnegative $f_n$, this is the uniform integrable condition. qed.

\paragraph{Corollary to Uniform Integrability Convergence} Suppose that $\mu(\Omega) < \infty$. If $f$ and the $f_n$ are integrable, and if $f_n \to f$ almost everywhere, then these conditions are equivalent:
\begin{enumerate}
	\item $f_n$ are uniformly integrable
	\item $\int |f - f_n| d\mu \to 0$
	\item $\int |f_n| d\mu \to \int |f| d\mu$
\end{enumerate}
\textbf{Proof:} Assume that 1. holds. Then, the differences $|f - f_n|$ are uniformly integrable as well. Make use of our previous note - if $f_n$ and $g_n$ are uniformly integrable, so is $\{f_n + g_n\}$. Let $g_n = -f$, and note integrability of $-f$ clearly implies uniform integrability of $g_n$. So, $f_n - f$ are uniformly integrable. Note that the absolute value $|f_n - f|$ is uniformly integrable as well - this is because we already take the absolute value in the definition of uniformly integrable. And so, $|f - f_n| = |f_n - f|$ is uniformly integrable.
\\\\
Now, note that $|f - f_n| \to 0$ almost everywhere via $f_n \to f$ almost everywhere. Thus, Theorem 16.14 implies that $\int |f - f_n| d\mu \to 0$.
\\\\
Now, note that (2) implies (3), because $||f| - |f_n|| \leq |f - f_n|$. This follows, because the triangle inequality implies $|a| \leq |a - b| + |b| \implies |a| - |b| \leq |a - b|$, and with symmetry, this implies:
$$
	-|a - b| \leq |a| - |b| \leq |a - b| \implies
	||a| - |b|| \leq |a - b|
$$
And so, monotonicity of the integral implies:
$$
	\left|\int |f| - |f_n| d\mu\right| \leq 
	\int ||f| - |f_n|| d\mu \leq \int |f - f_n| \to 0
	\implies
	\lim_n \left|\int |f| - |f_n| d\mu\right| = 0
$$
$$
	\implies \int |f_n| d\mu \to \int |f| d\mu
$$
Now, note that (3) implies (1), as Theorem 16.14 part 2 with (3) directly implies the $|f_n|$ are uniformly integrable. Note, this implies $f_n$ are uniformly integrable as well, from what we noted above. qed.

\subsubsection{Complex Functions}
A complex-valued function on $\Omega$ has the form $f(\omega) = g(\omega) + ih(\omega)$, where $g$ and $h$ are ordinary finite valued real functions on $\Omega$. We defined it as measurable $f$ if both $h$ and $g$ are. I guess, essentially, we have the borel sets on $\R^2$ - $\rcal^2$ - generated from the rectangles. I don't think if $A \in \rcal^2$, we have the projection $\pi_i(A)$ is a borel set in $\rcal$.
\\\\
If $g$ and $h$ are integrable, then $f$ is by definition integrable, and its integral is taken as:
$$
	\int (g + ih) d\mu = \int g d\mu + i \int h d\mu
$$
Now, we note that:
$$
	\max\{|g|, |h|\} \leq |f| \leq |g| + |h|
$$
Both come from $|f| = \sqrt{|g^2| + |h|^2}$. We have $|g|^2 + |h|^2 \leq (|g| + |h|)^2$, which is clear, and so the second inequality comes through. For the first one, note that the square of the max is $\max\{|g|^2, |h|^2\}$, which is less than $|g|^2 + |h|^2 = |g^2| + |h^2| = g^2 + h^2$. So, $f$ is integrable if and only if $\int |f| d\mu < \infty$, just as in the real case.
\\\\
Note that pretty much all of our theorems on real integral linearity and limits extend to the complex case as well. 

\subsection{Problems}
\subsubsection{16.1 Alternate Bounded Convergence Theorem Proof} Note, this problem says to make use of problem 13.9, in which we proved Egoroff's Theorem: Suppose that $f_n$ and $f$ are finite valued, $\fcal$ measurable functions such that $f_n(\omega) \to f(\omega)$ for $\omega \in A$, where $\mu(A) < \infty$ and $\mu$ is a measure on $\sigma$ field $\fcal$. For each $\epsilon > 0$, there exists a subset $B$ of $A$ such that $\mu(B) < \epsilon$, and $f_n(\omega) \to f(\omega)$ uniformly on $A - B$.
\\\\
Now, for the problem. Suppose that $\mu(\Omega) < \infty$ and the $f_n$ are uniformly bounded.
\begin{enumerate}
	\item Assume $f_n \to f$ uniformly and deduce $\int f_n d\mu \to \int f d\mu$ from what we proved earlier, that for integrable $f$ and $g$:
	$$
		\left|\int f d\mu - \int g d\mu\right| \leq \int |f - g| d\mu
	$$
	Recall, $f_n$ uniformly bounded means that $|f_n(\omega)| \leq K$ for all $\omega, n$. 
	\\\\
	Can't we just apply the bounded convergence theorem to automatically prove this? I think maybe we want to prove convergence without the theorem. Note, the next part of the problem wants us to use this part to give another proof for the bounded convergence theorem, so yes, we need to go ahead without it.
	\\\\
	It is clear that uniformly bounded and $\mu(\Omega) < \infty$ implies that $f_n$ are integrable. Can we also conclude that $f$ is integrable? Yes. For $\epsilon > 0$, there is an $n$ such that $|f_n(\omega) - f(\omega)| < \epsilon$ for all $\omega$, via uniform convergence. And so, $|f(\omega)| < K + 2\epsilon$, which implies that $\int |f| d\mu < \mu(\Omega)(K + 2\epsilon) < \infty$, so we have integrability. Thus, we can find:
	$$
		\left|\int f_n d\mu - \int f d\mu\right| \leq \int |f_n - f| d\mu
	$$
	For each $\epsilon$, there is an $N$ large enough such that the difference $|f_N - f| < \epsilon$. So, for $N$ large enough, for all $n \geq N$, we have via monotonicity of the integral:
	$$
		\int |f_n - f| d\mu < \epsilon \mu(\Omega)
	$$
	As $\epsilon$ can be arbitrarily small, we must have that:
	$$
		\lim_{n \to \infty} \int |f_n -f| d\mu = 0 \implies
		\int f_n d\mu \to \int f d\mu
	$$
	
	\item Use part 1 and Egoroff's theorem to give another proof of Theorem 16.5.
	\\\\
	So, we just assume that $\mu(\Omega) < \infty$, the $f_n$ are uniformly bounded, and $f_n \to f$ almost everywhere. By Egoroff's Theorem, for $\epsilon > 0$, there is a $B \subseteq \Omega$ such that $\mu(B) < \epsilon$, for which $f_n \to f$ uniformly on $\Omega - B$ (union $B$ with the measure 0 set on which $f_n \not \to f$). Thus, we have:
	$$
		\int_{\Omega - B} f_n d\mu \to \int_{\Omega - B} f d\mu
	$$
	Now, examine:
	$$
		\int_\Omega f_n d\mu =
		\int_{\Omega - B_n} f_n d\mu + \int_{B_n} f_n d\mu
	$$
	Where $\mu(B_n) < 1/n$. We have that:
	$$
		\int_{B_n} f_n d\mu \leq 2K\mu(B_n)
	$$
	Where $K$ is the uniform bound. This clearly goes to zero. So, we have:
	$$
		\lim_n \int_\Omega f_n d\mu =
		\lim_n \int_{\Omega - B_n} f_n d\mu + \int_{B_n} f_n d\mu \leq
		\lim_n \int_{\Omega - B_n} f_n d\mu + \lim_n 2k\mu(B_n)
	$$
	$$
		= \lim_n \int_{\Omega - B_n} f_n d\mu = \int_\Omega f d\mu
	$$
	And so we have the bounded convergence theorem. qed.
\end{enumerate}

\subsubsection{16.2 - Alternate Fatou's Lemma} Prove that if $0 \leq f_n \to f$ almost everywhere and $\int f_n d\mu \leq A < \infty$, then $f$ is integrable and $\int f d\mu \leq A$.
\\\\
Note, the book says this is essentially the same as Fatou's Lemma, which was just a consequence of monotone convergence theorem. We can use Fatou's Lemma fo prove it. We have that $\lim_n f_n = f$, which implies $\liminf_n f_n = f$ (almost everywhere). And so:
$$
	\int f = \int \liminf_n f_n
$$
As to functions that are equal almost everywhere have the same integral. By Fatou's lemma, as the $f_n$ are nonnegative:
$$
	\leq \liminf \int f_n \leq \liminf A = A
$$

\subsubsection{16.3 Beppo Levi's Theorem} Suppose that $f_n$ are integrable and $\sup_n \int f_n d\mu < \infty$. Show that, if $f_n \uparrow f$, then $f$ is integrable and $\int f_n d\mu \to \int f d\mu$.
\\\\
Note that we can't make automatic use of Monotone Convergence Theorem because the $f_n$ are not nonnegative. Note that $0 \leq f_n - f_1 \uparrow f - f_1$. Note, non-negativity comes from the $f_n$ getting larger. Thus, we have via Monotone convergence theorem:
$$
	\int f_n - f_1 d\mu \uparrow \int f - f_1 d\mu
$$
If we find that $f$ is integrable, then we should be able to make use of addition/subtraction to find the limit. 
\\\\
As $f_n \uparrow f$, we have $f_n \to f$ everywhere. As the limit exists, we have that:
$$
	\liminf_n f_n = \lim_n f_n = f
$$
And so, we can make use of Fatou's lemma, as well as our supremum bound, to find:
$$
	\int f d\mu = \int \lim_n f_n d\mu = \int \liminf_n f_n d\mu \leq
	\liminf_n \int f_n d\mu \leq \limsup_n \int f_n d\mu \leq \sup_n \int f_n d\mu < \infty
$$
However, this doesn't prove that $f$ is integrable. I think, we can instead show that $f - f_1$ is integrable. As $f - f_1 \geq 0$, we just need to show it has a finite integral, which would imply integrable, as $|f - f_1| = f - f_1$. We note:
$$
	\int f - f_1 d\mu = \lim_n \int f_n - f_1 d\mu =
	\lim_n \int f_n d\mu - \int f_1 d\mu \leq
	- \int f_1 d\mu + \sup_n \int f_n d\mu < \infty
$$
Where we have the supremum is finite via our assumption, and $-\int f_1 d\mu < \infty$ by integrable, and we can split the subtraction in the limit by Theorem 16.1. As $f - f_1$ is integrable, Theorem 16.1 tells us that $f - f_1 + f_1 = f$ is integrable as well, and we can again make use of Theorem 16.1 to find:
$$
	\lim_n \int f_n d\mu =
	\lim_n \int f_n - f_1 + f_1 d\mu =
	\int f_1 d\mu + \lim_n \int f_n - f_1 d\mu =
	\int f_1 d\mu + \int f - f_1 d\mu 
$$
$$
	= \int f - f_1 + f_1 d\mu = \int f d\mu
$$
So, we have proven Beppo Levi's Theorem. qed.

\subsubsection{16.8 Integrable Functions Bounded Integral On Small Enough Set} Show that if $f$ is integrable, then for each $\epsilon$ there is a $\delta$ such that $\mu(A) < \delta$ implies:
$$
	\int_A |f| d\mu < \epsilon
$$
As $f$ is integrable, we have that $\int |f| d\mu < \infty$. Take $\epsilon > 0$. Note, we can't necessarily assume that $f$ is bounded (think of going to infinity slowly on the unit interval). I think, we have that this is true vacuously? If $\delta = 0$, we have:
$$
	\int_A |f| d\mu = 0 < \epsilon
$$
I think, we need to assume that $\delta > 0$ as well. Let $f_n = |f|\1{|f| \leq n}$. As $f_n \uparrow |f|$, by monotone convergence, we have:
$$
	\lim_n \int f_n d\mu \uparrow \int |f| d\mu
$$
As both sides are integrable (and finite), we have this implies:
$$
	\lim_n \int_{|f| > n} |f| d\mu = \lim_n \int |f| - f_n d\mu = 0
$$
So, there is some $n$ large enough such that:
$$
	\int_{|f| > n} |f| d\mu < \epsilon/2
$$
Now, note that for any measurable $A$, we have:
$$
	\int_A |f| d\mu = \int_{A \cap \{|f| \leq n\}} |f| d\mu + \int_{A \cap \{|f| > n\}} |f| d\mu
$$
Note, the RHS is bounded by:
$$
	\leq \int_A n d\mu + \int_{\{|f| > n\}} |f| d\mu \leq \mu(A)n + \epsilon/2
$$
So, if $\delta = \epsilon/2n$ (note, $n$ depends on $\epsilon$, but that is fine) and $\mu(A) < \delta$, we have:
$$
	\int_A |f| d\mu \leq \epsilon/2 + \epsilon/2 = \epsilon
$$
And so we have proved the theorem. qed.


\subsubsection{16.9 Uniform Integrability Equivalent Condition} Suppose that $\mu(\Omega) < \infty$. Show that $\{f_n\}$ is uniformly integrable if and only if the following two conditions apply:
\begin{enumerate}
	\item $\int |f_n| d\mu$ is bounded by some $M$ for each $n$
	\item For each $\epsilon$ there is a $\delta$ such that $\mu(A) < \delta$ implies $\int_A |f_n| d\mu < \epsilon$ for all $n$.
\end{enumerate}
$\Rightarrow$ First, we assume that $\{f_n\}$ are uniformly integrable. Note, in the chapter, we found that $\mu(\Omega) < \infty$ together with uniform integrability implies that $f_n$ are integrable. Hence, we do have:
$$
	\int |f_n| d\mu \leq \infty
$$
Note, I think maybe it wants a bound for all $n$. Well, this also comes from the chapter - if $\alpha$ is large enough such that the supremum:
$$
	\sup_n \int_{|f_n| \geq \alpha} |f_n| d\mu < 1
$$
Note, such an $\alpha$ exists, as uniform integrability implies that the limit is 0 - we have that for all $n$:
$$
	\int |f_n| d\mu =
	\int_{|f_n| \geq \alpha} |f_n| d\mu +
	\int_{|f_n| < \alpha} |f_n| d\mu \leq
	1 + \int \alpha d\mu = 1 + \alpha \mu(\Omega)
$$
Now, for the second point. By the previous problem, as each $n$ is integrable, the for $\epsilon$, there is a $\delta_n$ such that $\mu(A) < \delta_n$ implies:
$$
	\int_A |f_n| d\mu < \epsilon
$$
However, we need an argument for each $n$. Well, we do have:
$$
	\int_A |f_n| d\mu =
	\int_{A \cap |f_n| \geq \alpha} |f_n| d\mu +
	\int_{A \cap |f_n| < \alpha} |f_n| d\mu \leq
	\int_{|f_n| \geq \alpha} |f_n| d\mu + \alpha\mu(A)
$$
This is true for all $n$. So, choose our $\epsilon > 0$. Now, choose $\alpha$ large enough such that for all $n$:
$$
	\int_{|f_n| \geq \alpha} |f_n| d\mu \leq \frac{\epsilon}{2}
$$
Now, choose $\delta = \frac{\epsilon}{2\alpha}$, and we have for $\mu(A) < \delta$:
$$
	\int_A |f_n| d\mu \leq
	\int_{|f_n| \geq \alpha} |f_n| d\mu + \alpha\mu(A) \leq
	\frac{\epsilon}{2} + \alpha \frac{\epsilon}{2\alpha} = \epsilon
$$
And os we have proven the first direction.
\\\\
$\Leftarrow$ We assume the two conditions hold, and we want to prove that $\{f_n\}$ are uniformly integrable. Note, for $\epsilon > 0$, there is a $\delta$ such that $\mu(A) < \delta$ implies:
$$
	\sup_n \int_A |f_n| d\mu < \epsilon
$$
As if it is true for all $n$, it is true for the supremum by (2). Now, as each $\int |f_n| d\mu < M$, there is an $\alpha$ large enough such that:
$$
	\alpha^{-1} \int |f_n| d\mu < \delta
$$
This is by (1). Then, we have:
$$
	\mu\left[|f_n| \geq \alpha\right] = \int \1{|f_n| \geq \alpha} d\mu =
	\alpha^{-1} \int \alpha \1{|f_n| \geq \alpha} d\mu
$$
$$
	\leq \alpha^{-1} \int |f_n|\1{|f_n| \geq \alpha} d\mu
	\leq \alpha^{-1} \int |f_n| d\mu < \delta
$$
Note, the first equality makes use of Theorem 15.1, and then the following are just standard from moving scalars through a nonnegative function. Thus, we have $\mu\left[|f_n| \geq \alpha\right] < \delta$, and we can make use of our first fact that for each $n$:
$$
	\int_{|f_n| \geq \alpha} |f_n| d\mu \leq \epsilon
$$
This directly implies uniform integrability - namely, there is an $\alpha$ large enough such that:
$$
	\sup_n \int_{|f_n| \geq \alpha} |f_n| d\mu \leq \epsilon \implies
	\lim_{\alpha \to \infty} \sup_n \int_{|f_n| \geq \alpha} |f_n| d\mu = 0
$$
Thus, we have proven that for $\mu(\Omega) < \infty$, the two conditions taken together are equivalent to uniform integrability. qed.

\subsubsection{16.10 Uniform Integrability Equivalent Condition Properties} Assume $\mu(\Omega)$
\begin{enumerate}
	\item Show by examples that neither of the conditions (1) and (2) in the preceding problem implies the other.
	\\\\
	Define $f_n: (0,1] \to \R$ as:
	$$
		f_n = \1{(0,1/n)}n
	$$
	We have that:
	$$
		\int f_n d\lambda = \lambda(0,1/n)n = 1 
	$$
	And so the $f_n$ are all bounded. Now, I will show that $f_n$ as defined do not satisfy property 2. Take $0 < \epsilon < 1$ and assume there is a $\delta$ such that $\mu(A) < \delta$ implies:
	$$
		\int_A |f_n| d\mu < \epsilon \text{ for all $n$}
	$$
	We will provide a contradiction to this argument. Indeed, take $1/n < \delta$, we have $\mu(0,1/n) = 1/n < \delta$. Now, we note:
	$$
		\int_{(0,1/n)} f_n d\lambda = \int \1{(0,1/n)}^2n d\lambda = 1 > \epsilon
	$$
	So, we have our contradiction, and for the given example, (1) does not imply (2).
	\\\\
	Now, we go the other direction. Define $f_n: \R \to \R$:
	$$
		f_n = \1{(0,n)}
	$$
	Take $\epsilon > 0$. Let $\delta = \epsilon$, and assume that $\mu(A) < \delta = \epsilon$. We have:
	$$
		\int_A f_n d\lambda = \int_A \1{(0,n)} d\lambda \leq
		\int_A 1 d\lambda = \mu(A) = \epsilon
	$$
	So, the $f_n$ as defined do satisfy (2). However, we find that:
	$$
		\int f_n d\lambda = n
	$$
	And so the integrals of $f_n$ are not bounded. Thus, for this example, (2) does not imply (1).
	\\\\
	Note, in the above second example, I let $\mu(\Omega) = \infty$, but we need to assume $\mu(\Omega) < \infty$. Considering the second problem, we also need $\mu$ to be nonatomic. If we did the counting measure on $\{1, 2, \cdots\}$, we would still have $\mu(\Omega) = \infty$. I guess we could give weights to each entry - say $2^{-n}$, and so $\mu(\Omega) = 1$... but I'm not sure we could have a non bounded $f_n$? maybe, we can define:
	$$
		f_n(\omega) = \1{\omega = n} n2^n
	$$
	So then, we should have:
	$$
		\int f_n d\mu = \int \1{\omega = n} n2^n d\mu = \mu(\omega = n) n 2^n = n
	$$
	Which is not bounded. I think this is approaching what we need, but not quite right.
	\\\\
	Instead, define $f_1(\omega) = \1{\omega = 1} \cdot \infty$, and $f_n(\omega) = \1{\omega = n}$ otherwise. The $f_n$ are not bounded via $f_1$ - but if you have $\epsilon > 0$, set $\delta = \min(1/2, \epsilon)$, which should exclude the point mass at $1$, and otherwise $\int f_n d\mu = 2^{-n} < \epsilon$.
	
	\item Show that (ii) implies (i) for all sequences $\{f_n\}$ if and only if $\mu$ is nonatomic.
	\\\\
	First - I need to think - does this not contradict my example above? I defined $f_n$ for the Lebesgue measure on $\R$, which should be nonatomic. Oh - I let $\mu(\Omega) = \infty$, which is incorrect.
	\\\\
	For the rest, just look at the solutions.
\end{enumerate}

\subsubsection{16.12 Daniell-Stone Representation Theorem} Note - I am just going to follow with the solutions for this problem. That is because I don't really want to do it, but I do think it is a pretty important theorem to understand, at least.

\paragraph{Review} Consider the vector lattice $\lcal$ and the functional $\Lambda$ of problems 11.4 and 11.5. $\lcal$ is thus a vector space of finite real valued functions $f: \Omega \to \R$, with $f(\omega) < \infty$, and the functions are closed under addition, scalar multiplication. Also, as $\lcal$ is a lattice, it is closed under join and meet, essentially maximum and minimum:
$$
	f \vee g: \Omega \to \R \quad\quad
	(f \vee g)(\omega) = \max(f(\omega), g(\omega)) \quad\quad
	f \vee g \in \lcal
...
$$
	...
	f \wedge g \in \lcal
$$
And $\Lambda$ is a real linear functional, which is a map from a vector space $V$ to its field, which is also linear. So, $\Lambda: \lcal \to \R$, and:
$$
	\Lambda(af + bg) = a\Lambda(f) + b\Lambda(g)
$$
We further assumed of $\Lambda$ that it is positive in the sense that $f \geq 0$ (pointwise) implies $\Lambda(f) \geq 0$ and continuous from above at $0$ in the sense that $f_n \downarrow 0$ (pointwise) implies $\Lambda(f_n) \to 0$.
\\\\
For $\lcal$ and $\Lambda$ as defined, we were able to show a couple of properties in problems 11.4 and 11.5. I will list those properties here:
\begin{enumerate}
	\item For $f \leq g$ (with $f,g \in \lcal$), we defined an "interval" in $\Omega \times \R^1$ as:
	$$
		(f,g]:= \left\{(\omega, t) : f(\omega) < t \leq g(\omega)\right\}
	$$
	We found that these sets formed a semi ring $\acal_0$, with:
	$$
		(f_1,g_1] \cap (f_2,g_2] = (f_1 \vee f_2, g_1 \wedge g_2]
	$$
	$$
		(f_2,g_2] - (f_1,g_1] = (f_2, g_2 \wedge f_1] \cup (f_2 \vee g_1, g_2]
	$$
	
	\item For a set function $\nu_0$ on $\acal_0$ defined as:
	$$
		\nu_0(f,g] = \Lambda(g - f)
	$$
	We found that $\nu_0$ was finitely additive, and countably subadditive. Via Theorem 11.3, this enabled us to extend $\nu_0$ to a measure on $\sigma(\acal_0)$.
	
	\item We were able to conclude a property - that for $f \in \lcal$, with $f_n = (n(f - f \wedge 1)) \wedge 1$, for $x > 0$, we had:
	$$
		(0, xf_n] \uparrow [\omega : f(\omega) > 1] \times (0,x]
	$$
	Note, this means the unions of the sets in $\acal_0$ go up to the set on the RHS (note, the set on the RHS might not necessarily be in $\acal_0$).
	
	\item We defined $\fcal$ as the smallest $\sigma$ field with respect to which every $f$ in $\lcal$ is measurable:
	$$
		\fcal = \sigma\left[f^{-1}H : f \in \lcal, H \in \rcal^1\right]
	$$
	We defined $\fcal_0$ as the class of $A$ in $\fcal$ for which $A \times (0,1] \in \sigma(\acal_0)$. We were able to show that $\fcal_0$ is a semiring, and $\fcal = \sigma(\fcal_0)$. This was proved using the above property.
	
	\item Finally - we let $\nu$ be the extension of $\nu_0$ to $\sigma(\acal_0)$, and for $A \in \fcal_0$, we defined:
	$$
		\mu_0(A) = \nu(A \times (0,1]) \text{ well defined, as $A \in \fcal_0 \implies A \times (0,1] \in \sigma(\acal_0)$}
	$$
	We showed the $\mu_0$ is finitely additive and countably subadditive on the semiring $\fcal_0$ - and thus, via Theorem 11.3, has a measure extension $\mu$ on $\fcal$.
\end{enumerate}

So, these are a lot of properties. But, what it gives us, at the end of the day, is a measure $\mu$ on $\fcal$, the smallest $\sigma$ field with respect to which every $f \in \lcal$ is measurable.

\paragraph{Problems}

\begin{enumerate}
	\item Show by (11.7) (our property 3 above) that for positive $x,y_1,y_2$ one has:
	$$
		\nu([f > 1] \times (0,x]) =
		x\mu_0[f > 1] = x\mu[f > 1]
	$$
	...
	$$
	Well - first, I will examine $[f > 1]$. Clearly, as $(1, \infty) \in \rcal$, we have that $[f > 1] \in \fcal$. We want to show that $[f > 1] \times (0,x] \in \sigma(\acal_0)$, and so $\nu$ will be well defined on that set. Recall:
	$$
		f_n = (n(f - f \wedge 1)) \wedge 1
	$$
	We have that if $f(\omega) > 1$ - then, for some $n$, we have that $(n(f - f \wedge 1)) > 1$, and so $f_n(\omega) = 1$ for all sufficiently large $n$. If $f(\omega) \leq 1$, $f - f \wedge 1 = 0$, and so $f_n(\omega) = 0$. In all cases, $f_n \geq 0$, and so $(0, xf_n]$ is well defined for $x > 0$. Recall, we also proved that:
	$$
		(0,xf_n] \uparrow [f > 1] \times (0,x]
	$$ 
	And so, at the very least, $\nu([f > 1] \times (0,x])$ is well defined, given that the set is a union of $\acal_0$ sets. We have that via continuity from below:
	$$
		\nu([f > 1] \times (0,x]) = \lim_{n \to \infty} \nu(0,xf_n] =
		\lim_{n \to \infty} \nu_0(0,xf_n]
	$$
	Anyway. We have that $[f > 1] \times (0,x] \in \sigma(\acal_0)$ for all $x > 0$. This implies that $[f > 1] \times (0,1] \in \sigma(\acal_0)$, which implies that the following is well defined:
	$$
		\mu_0[f>1] = \nu((f > 1] \times (0,1])
	$$
	...
	$$
		\nu([f > 1] \times (0,x]) =
		\lim_{n \to \infty} \nu_0(0,xf_n] =
		\lim_{n \to \infty} \Lambda(xf_n) =
		\lim_{n \to \infty} x\Lambda(f_n)
	$$
	As $\Lambda$ as a linear functional. Thus, we have:
	$$
		\nu([f > 1] \times (0,x]) = \lim_{n \to \infty} x\nu(0,f_n] = x\nu([f > 1] \times (0,x]) =
		x\mu_0[f>1]
	$$
	And we have concluded the first expression. The second one we want to prove is:
	$$
		\nu([y_1 < f \leq y_2] \times (0,x]) = x\mu[y_1 < f \leq y_2]
	$$
	This should just be a continuation of the first. We note that for positive $y$:
	$$
		\nu([f > y] \times (0,x]) =
		\nu([f/y > 1] \times (0,x]) =
		x\mu[f/y > 1]
	...
	Also:
	$$
		[y_1 < f \leq y_2] \times (0,x] =
		[f > y_1] \times (0,x] - [f > y_2] \times (0,x]
	$$
	As the first set contains the second, we can make use of measure subtraction to find:
	$$
		\nu([y_1 < f \leq y_2] \times (0,x]) =
		\nu([f/y_1 > 1] \times (0,x]) -
		\nu([f/y_1 > 1] \times (0,x])
	$$
	$$
		=
		x\left[\mu[f/y_1 > 1] - \mu[f/y_2 > 1]\right] =
		xx\mu[y_1 < f \leq y_2]
	$$
	Note, we have to use $\mu$ instead of $\mu_0$, as we don't necessarily have the subtraction property for non measure $\mu_0$. Also, we need positive $y_1,y_2$ to make the $f/y$ inequalities valid.
	
	\item Show that if $f \in \lcal$, then $f$ is integrable and:
	$$
		\Lambda(f) = \int f d\mu
	$$
	Here, I just follow the solution in the book, I don't really want to spend time trying to prove this theorem. 
	\\\\
	First, note we just need to prove that if $f \geq 0$, we have:
	$$
		\Lambda(f) = \int f d\mu
	$$
	Then, we note that $f^+ = f \vee 0$, and $f- = -f \vee 0$. In both cases, we have:
	$$
		\Lambda(f) = \Lambda(f^+ - f^-) = \Lambda(f^+) - \Lambda(f^-) =
		\int f^+ d\mu - \int f^- d\mu = \int f
	$$
	Where subtraction is well defined, because $\Lambda(f)$ is \textit{real}, and so doesn't return infinity as a value.
	\\\\
	Take $f \in \lcal$ with $f \geq 0$. If $f_n = (1 - n^{-1})f \vee 0$, then $f_n \in \lcal$ and $f_n \uparrow f$ (note, I don't think we need the $\vee 0$ as $f$ is nonnegative, but it is fine to keep). $f_n \uparrow f$ is clear, as $n \to \infty$, $(1 - n^{-1}) \to 1$. Thus, we note that:
	$$
		\nu(f_n,f] = \Lambda(f - f_n) \downarrow 0
	$$
	Via continuity from above of $\Lambda$. Note, as $\Lambda$ is finite, we have that continuity from above can be applied. We note that:
	$$
		\bigcap (f_n,f] = \left\{(\omega, t) : f(\omega) = t\right\}
	$$
	As consider $(\omega, t) \in LHS$. We have that $f_n(\omega) < t \leq f(\omega)$ for all $n$, and as $f_n(\omega) \uparrow f(\omega)$, we must have that $f(\omega) = t$, and so $(\omega, t) \in RHS$. We can similarly go the other way. And so, via continuity from above, we have that the following is well defined:
	$$
		\nu\left(\left\{(\omega, t) : f(\omega) = t\right\}\right) = 0
	$$
	Look at the disjoint union:
	$$
		B_n = \bigcup_{i=1}^{n2^n} 
		\left(\left[\frac{i}{2^n} < f \leq \frac{i+1}{2^n}\right] \times \bigg(0, \frac{i}{2^n}\bigg]\right)
	$$
	Some things to note - as $n$ increases, the total range bounding $f$ increases, and gets finer and finer. Further, the range on the RHS gets larger and larger. Note that:
	$$
		B_n \uparrow B \subseteq (0,f]
	$$
	First, note that $B_n \subseteq B_{n+1}$. Indeed, if $(\omega, t) \in B_n$, we have that for some $i$:
	$$
		\frac{i}{2^n} < f(\omega) \leq \frac{i+1}{2^n} \text{ and } 0 < t \leq \frac{i}{2^n}
	$$
	$(\omega, t) \in B_{n+1}$, as we have that for some $j = 2i, 2i+1$:
	$$
		\frac{j}{2^{n+1}} < f(\omega) \leq \frac{j+1}{2^{n+1}}
	$$
	And for both of these $j$, we have that $0 < t \leq \frac{j}{2^{n+1}}$. So, $B$ is a well defined union. Note, $B_n \in \sigma(\acal_0)$, as we found a $\nu$ measure value for each set in the disjoint union. Disjoint, as the $\omega$ are disjoint for each set in the union. Now, we need to show that:
	$$
		B \subseteq (0,f]
	$$
	Which follows if we have $B_n \subseteq (0,f]$ for each $n$. Recall, we have $f$ is nonnegative, and so:
	$$
		(0,f]:= \left\{(\omega, t) : 0 < t \leq f(\omega)\right\}
	$$
	Note that as $(\omega, t) \in B_n$, we have that $\frac{i}{2^n} < f(\omega)$ and $0 < t \leq \frac{i}{2^n}$, so we indeed have that $B_n \subseteq (0,f]$ for each $n$, and $B \subseteq (0,f]$.
	\\\\
	Now, we note that:
	$$
		(0,f] - B \subseteq [(\omega,t) : f(\omega) = t]
	$$
	Indeed, if we have $(\omega,t) \in (0,f]$, but $(\omega,t) \not\in B_n$ for each $n$, that means that $0 < t \leq f(\omega)$ is satisfied, but $0 < t \leq \frac{i}{2^n} < f(\omega)$ is not satisfied for each possible $i$ and $n$. Note, if $t < f(\omega)$, there would be an $i$ and $n$ in our range above that would satisfy $t \leq \frac{i}{2^n} < f(\omega)$, so we must conclude that $t < f(\omega)$ is not true if $(t,\omega) \not\in B$. Thus, we must indeed have that $t = f(\omega)$, which gives us our above fact. And so, pulling our facts together, we must have:
	$$
		\Lambda(f) = \Lambda(f - 0) = \nu_0(0,f] = \nu(0,f] = \nu(B)
	$$
	As for a measure, given that $B \subseteq (0,f]$, we have: 
	$$
		\nu(0,f] - \nu(B) = \nu((0,f] - B) \leq \nu\left(\left\{(\omega, t) : f(\omega) = t\right\}\right) = 0
	$$
	Using our continuity from below, we have:
	$$
		\Lambda(f) = \nu(B) = \lim_n \nu(B_n) = 
		\lim_n \sum_{i=1}^{n2^n} \frac{i}{2^n} \mu\left[\frac{i}{2^n} < f \leq \frac{i+1}{2^n}\right]
	$$
	Where we used our property from part $1$ for the last equality. Finally, we note that this final limit equals:
	$$
		= \int f d\mu
	$$
	Recall, we only defined:
	$$
		\int f d\mu = \sup \sum_i\left[\inf_{\omega \in A_i} f(\omega)\right]\mu(A_i)
	$$
	Where $A_i$ is a finite decomposition of $\Omega$. Note, our sums in the limit are of the infimum sum form (as for the final $A_i$ not expressed by the intervals decomposing the range of $f$, the infimum is zero and doesn't contribute to the sum). As for why the limit is equal to the supremum - ultimately, we have that each term is increasing, given that we are taking the lower bound of finite intervals. However, instead, think of it this way. Each term in the limit is the integral for some simple function $f_n$, as seen in Theorem 13.5. We have that $f_n \uparrow f$, and so we can actually make use of Monotone Convergence Theorem:
	$$
		\Lambda(f) = \lim_n \sum_{i=1}^{n2^n} \frac{i}{2^n} \mu\left[\frac{i}{2^n} < f \leq \frac{i+1}{2^n}\right] = \lim_n \int f_n d\mu = \int f d\mu
	$$
	And so, for nonnegative $f \in \lcal$, we have found that:
	$$
		\Lambda(f) = \int f d\mu
	$$
	For $\mu$ found in problems 11.4 + 11.5. This implies the same for all $f \in \lcal$ as noted initially. qed.
\end{enumerate}

\section{Section 17 - The Integral With Respect to Lebesgue Measure}
\subsection{Notes}
\subsubsection{The Lebesgue Integral on the Line}
A real measurable function on the line is Lebesgue integrable if it is integrable with respect to the Lebesgue measure $\lambda$, and its \textit{Lebesgue integral} $\int f d\lambda$ is denoted by $\int f(x) dx$, or in the case of integrating over an interval, by $\int_a^b f(x) dx$. 

\subsubsection{The Riemann Integral} A real function $f$ on an interval $(a,b]$ is by definition (and there are many definitions) \textit{Riemann integrable}, with integral $r$, if this condition holds: for each $\epsilon$ there exists a $\delta$ with the property that:
$$
	\left|r - \sum_i f(x_i)\lambda(I_i)\right| < \epsilon
$$
If $\{I_i\}$ is any finite partition or $(a,b]$ into subintervals satisfying $\lambda(I_i) < \delta$ and $x_i \in I_i$ for each $i$. 

\paragraph{The Riemann Integral, when it exists, coincides with the Lebesgue Integral} Suppose that $f$ is Borel measurable, and suppose that $f$ is bounded, so that it is Lebesgue integrable (on an interval). If $f$ is also Riemann integrable, the $r$ above must coincide with the Lebesgue integral:
$$
	\int_a^b f(x) dx
$$
First note that letting $x_i$ vary over $I_i$ leads to $r$ also satisfying:
$$
	\left|r - \sum_i \sup_{x \in I_i} f(x) \cdot \lambda(I_i)\right| \leq \epsilon
$$
Indeed, we could take limits for each of the finite $I_i$, to the supremum on $I_i$, and only lose the strict inequality. Now, consider a simple function $g$ with value $\sup_{x \in I_i} f(x)$ on $I_i$. We have that $f \leq g$, but also:
$$
	\int_a^b g(x) dx = \sum_i \sup_{x \in I_i} f(x) \cdot \lambda(I_i)
$$
Via Theorem 15.1. By monotonicity of the integral, this must imply that:
$$
	\int_a^b f(x) dx \leq \int_a^b g(x) dx \leq r + \epsilon
$$
The reverse inequality also follows in the same way, if we take the $\inf_{x \in I_i} f(x) \cdot \lambda(I_i)$, and so we find for all $\epsilon > 0$:
$$
	r + \epsilon \leq \int_a^b f(x) dx \leq r + \epsilon \implies
	\int_a^b f(x) dx = r
$$

\paragraph{A continuous function on a closed interval is Riemann Integrable} Suppose that $f$ is continuous on $[a,b]$. A continuous function is uniformly continuous on the closed interval, and so for each $\epsilon$ there exists a $\delta$ such that $|f(x) - f(y)| < \epsilon/(b-a)$ if $|x - y| < \delta$. If $\lambda(I_i) < \delta$ and $x_i \in I_i$, then:
$$
	g = \sum_i f(x_i) \1{I_i} \implies
	|f - g| < \epsilon/(b-a)
$$
Thus, for the \textit{Lebesgue integral}, we have:
$$
	\left|\int_a^b fdx - \int_a^b gdx\right| = \left|\int_a^b f - g dx\right| \leq 
	\left|\int_a^b \epsilon/(b-a) dx\right| \leq \epsilon
$$
Note, both $f$ and $g$ are integrable, so everything above is well defined. But note, the LHS also simplifies to:
$$
	\left|\int_a^b fdx - \sum_i \sup_{x \in I_i} f(x) \cdot \lambda(I_i)\right| \leq \epsilon
$$
Which is just the definition of Riemann integrable - where $r = \int_a^b fdx$ (and they must be equal, by our previous statement). So, a continuous function $f$ has an $r$ that satisfies the Riemann integrable definition on a closed interval. qed.

\paragraph{Example 17.1 - A Lebesgue But Not Riemann Integrable $f$} Let $f$ be the indicator of the set of rationals in $(0,1]$. Then, the Lebesgue integral is zero, because $f = 0$ almost everywhere. However, $f$ is not Riemann integrable, as every $I_i$ contains an $x_i = 0$ and $x_i = 1$, so no $r$ can satisfy the definition for $\epsilon < 1/2$. 

\paragraph{Example 17.2 - Lebesgue Theory Is Not Reducible To Riemann Theory via Restricting Measure Zero Sets} For $f$ in example 17.1, there is a $g$ such that $f = g$ almost everywhere and $g$ is Riemann integrable ($g = 0$). We want an example to show that Lebesgue Theory is not reducible to Riemann Theory by casting out sets of measure 0, and so we want to produce an $f$ (bounded and measurable) such that no $g$ exists.
\\\\
In Examples 3.1 and 3.2 in the book, we constructed a borel set $A \subseteq (0,1]$ such that $0 < \lambda(A) < 1$ and $\lambda(A \cap I) > 0$ for each subinterval $I$ of $(0,1]$. Take $f = \lambda_A$. Suppose that $f = g$ almost everywhere and that $\{I_i\}$ is a decomposition of $(0,1]$ into subintervals. Since:
$$
	\lambda(I_i \cap A \cap [f = g]) = \lambda(I_i \cap A) > 0
$$
It follows that $g(y_i) = f(y_i) = 1$ for some $y_i \in I \cap A$, and therefore:
$$
	\sum_i g(y_i)\lambda(I_i) = 1 > \lambda(A)
$$
If $g$ were Riemann integrable, its Riemann integral would coincide with the Lebesgue integrals $\int g dx = \int f dx = \lambda(A)$, in contradiction to the other Riemann integral value $1$ we found above. So, $g$ cannot be Riemann integrable, and so for $f$ as defined, there is no $f = g$ almost everywhere such that $g$ is Riemann integrable.

\paragraph{Note 17.1 + 17.2} The reason the functions in example 17.1 and 17.2 fail to be Riemann integrable, is because of their extreme oscillations (we proved in our Calculus On Manifolds notes that $f$ is integrable iff the set of its discontinuities has Lebesgue measure 0). This cannot happen in the case of the Lebesgue integral of a measurable function - recall, we have already identified the two reasons why such an $f$ would fail to be Lebesgue integrable - because its positive or negative part is too large, not because one or the other is too irregular.

\paragraph{Theorem 17.1 - Approximating Lebesgue Integrable Functions With Riemann Integrable Functions} Suppose that $\int|f|dx < \infty$ and $\epsilon > 0$.
\begin{enumerate}
	\item There is a step function $g = \sum_{i=1}^k x_i\1{A_i}$ with bounded intervals at the $A_i$, such that:
	$$
		\int |f - g| dx < \epsilon
	$$
	
	\item There is a continuous integrable $h$ with bounded support such that $\int|f - h| dx < \epsilon$.
\end{enumerate}
\textbf{Proof:} \textbf{1.} By the construction in Theorem 13.5 (sequence of simple functions approaching $f$ real and measurable) and the dominated convergence theorem (as $f$ is integrable, and dominates the simple functions), we have that there is a simple function $g$ such that $\int |f - g| dx < \epsilon$. Note here, we \textit{don't} assume that the $A_i$ are intervals. However, we do have that $\lambda(A_i) < \infty$ for each $i$ for which $x_i \neq 0$ (otherwise this would imply an infinity integral via $x_i\1{A_i}$).
\\\\
By Theorem 11.4, and because the bounded intervals $\acal$ are a semiring that generate $\bcal$ (recall, we are dealing with $f$ defined on the line here), there is a finite disjoint union $B_{ij}$ of intervals such that:
$$
	\lambda(A_i \triangle (\cup_{j=1}^n B_{ij})) < \frac{\epsilon}{k|x_i|}
$$
Note, the union $B_i = \cup_{j=1}^n B_{ij}$ is still a series of bounded intervals. Define:
$$
	g' = \sum_{i=1}^k x_i\1{B_i}
$$
Note, if any of the $B_i$ intersect, just take the addition of the $x_i$ values. In total, $g'$ is still a finite sum step function on bounded interval steps. We have:
$$
	\int |f - g'| dx \leq \int |f - g| dx + \int |g - g'| dx \leq \epsilon +
	\sum_{i=1}^k x_i\lambda(A_i \triangle (\cup_{j=1}^n B_{ij})) \leq 2\epsilon
$$
Where the second sum comes from noting, that $g$ and $g'$ only differ by $x_i$ on the portions where $A_i$ and $B_i$ don't meet up exactly (added up w/ all the differences for the other $x_i$). Thus, we have found a $g'$ that satisfies the first part. Just expanding on the sum - say that $\omega$ is in $A_i$ and in $B_i$ but also in $B_j$. Then, we have:
$$
	g(\omega) = x_i \quad\quad\quad
	g'(\omega) = x_i + x_j \implies |g(\omega) - g'(\omega)| = x_j
$$
This difference holds for all the $\omega \in A_j \triangle B_j$, and this difference is accounted for in the integral by taking the sum $x_j\lambda(A_j \triangle B_j)$. Going further, I think we just have:
$$
	|g - g'|(\omega) = \sum_{i=1}^k x_i\1{A_i \triangle B_i}
$$
Going further, in general, the difference between indicators is:
$$
	|f - g| = \sum_{i=1}^k \sum_{j=1}^k |a_i - b_j| \1{A_i \cap B_j}
$$
Where each is disjoint. But, as $a_i = b_i$ in our case, when $i = j$, the difference is zero. If we consider:
$$
	\sum_{j=1}^k |x_i - x_j|  \1{A_i \cap B_j} \leq
	\sum_{j=1}^k (x_i + x_j) \1{A_i \cap B_j}
$$
We know that this is included within the $x_i\1{A_i \triangle B_i}$ and $x_j\1{A_j \triangle B_j}$ terms, given that the $A_i$ are disjoint. 
\\\\
\textbf{2.} Recall, bounded support means the set of points where the function is not zero is bounded. Note, we need this condition, as we only proved continuous functions were Riemann integrable on a closed interval. Take the $\epsilon > 0$, and using part 1, take the $g$ step function on bounded intervals $A_i$ where:
$$
	\int |f - g| dx < \epsilon
$$
We will show there is such a continuous integrable $h$ with bounded support such that:
$$
	\int |h - g| dx < \epsilon
$$
This will imply:
$$
	\int |f - h| dx \leq \int |f - g| dx + \int |h - g| dx \leq 2\epsilon
$$
Which is enough. Suppose that $A_i = (a_i, b_i]$. Let $h_i(x)$ be 1 on $(a_i,b_i]$ and $0$ outside of $(a_i - \delta, b_i + \delta]$, and let it increase linearly from $0$ to $1$ over $(a_i - \delta, a_i]$ and decrease linearly from $1$ to $0$ over $(b_i,b_i+\delta]$. As clearly:
$$
	\int |\1{A_i} - h|dx \to 0
$$
As $\delta \to 0$, $h = \sum_i x_ih_i$ will satisfy for sufficiently small $\delta$. This is because:
$$
	\int |h - g| dx \leq \sum_i x_i \int |\1{A_i} - h|dx \to 0
$$
Thus, we have proven the Theorem. As a small note, $h$ clearly has bounded support, just taking the maximum $b_i$ and minimum $a_i$, and add $\delta$. qed.
\\\\
Thus, the Lebesgue integral is determined by its values for continuous functions with bounded support. And each of these is Riemann integrable.

\subsubsection{The Fundamental Theorem of Calculus}
Note, for all of these related theorems, I have already proven them for the Riemann integral. So, I guess I could pass to the Riemann integral whenever it exists, and get the Theorem for the Lebesgue Integral. However, I think these theorems might be more general, ie, we will be able to apply them for the cases in which the Lebesgue integral exists but the Riemann integral doesn't.
\\\\
Adopt the convention that $\int_\alpha^\beta = -\int_\beta^\alpha$ if $\alpha > \beta$. For positive $h$:
$$
	\left|\frac{1}{h}\int_x^{x+h} f(y) dy - f(x)\right| \leq
	\frac{1}{h}\int_x^{x+h} |f(y) - f(x)| dy \leq
	\sup\left[|f(y) - f(x)| : x \leq y \leq x + h\right]
$$
As the $1/h$ and $h$ factor cancel out in the supremum. The RHS goes to 0 with $h$ if $f$ is continuous at $x$. The same thing holds for negative $h$, and therefore $\int_a^x f(y) dy$ has derivative $f(x)$:
$$
	\frac{d}{dx} \int_a^x f(y)dy = f(x)
$$
If $f$ is continuous at $x$. Note, the function varies with $x$, ie $g(x) = \int_a^x f(y) dy$. The bottom limit $a$ is arbitrary - and this is because the derivative (in the one dimensional case) is the $k$ that satisfies
$$
	g'(x) = \lim_{h \to 0} \frac{g(x + h) - g(x) - k}{h} =
	\left|\frac{1}{h}\int_x^{x+h} f(y) dy - k\right| = 0
$$
And we found $k = f(x)$ is the satisfying point. Also note, this is all done with the \textit{Lebesgue Integral} with respect to the \textit{Lebesgue measure} $\lambda$. The integral calculation made use of the absolute value bound. However, note that all the subtractions require that $f$ be integrable, or at the very least $f\1{[a,x+h]}$ is integrable.
\\\\
Suppose that $F$ is a function with continuous derivative $F' = f$. That is, $F$ is the primitive (or anti-derivative) of $f$. Then:
$$
	\int_a^b f(x) dx = \int_a^b F'(x) = F(b) - F(a)
$$
As we have $F(x) - F(a) = \int_a^x f(y) dy$ if we set $x = a$, and both functions have identical derivatives, namely $f(x)$ by the above fact. And so, two functions $\R \to \R$ that agree at one point and have the same derivative everywhere are equal.

\subsubsection{Change Of Variable} For:
$$
	[a,b] \overset{T}{\to} [u,v] \overset{f}{\to} \R^1
$$
...
$$
	...
	\int_{T(a)}^{T(b)} f(y) dy
$$
If $T'$ exists and is continuous, and if $f$ is continuous, the two integrals are finite because the integrands are bounded (continuous functions on intervals are bounded). To prove the above, it is enough to let $b$ be a variable and differentiate with respect to it. This is a homework problem, but using the FTC, we have:
$$
	\frac{d}{db} \int_a^b f(Tx) T'(x) dx = f(Tb)T'(b)
$$
$$
	\frac{d}{db} \int_{T(a)}^{T(b)} f(y) dy = ...
$$
Note, I think this will just require the chain rule. Say $F' = f$. Then $(F \circ T)' = (f \circ T)T'$. We have:
$$
	\int_{T(a)}^{T(b)} f(y) dy = F(T(b)) - F(T(a))
$$
Whereas we also have:
$$
	\int_a^b f(Tx) T'(x) dx = \int_a^b (F \circ T)' dx = (F \circ T)(b) - (F \circ T)(a) = F(T(b)) - F(T(a))
$$
So we do have that both sides are equal. Also go to our calculus on manifold book to find the change-of-variable formula in one variable for the Riemann integral. Anyway, if we want to ever spend more time on this, I would say just directly use the derivative definition, with the derivative set at $f \circ T$, and I would guess the limits go to 0 on both sides, if both sides are treated as a function of $b$. They also have equal values at $b = a$.

\paragraph{Example 17.5} Put $T(x) = \tan(x)$ on $(-\pi/2,\pi/2)$. Note that $T'(x) = 1 + T^2(x)$. Via the change of variable formula, we have:
$$
	\int_{-\infty}^\infty \frac{1}{1 + y^2} dy =
	\int_{\tan(-\pi/2)}^{\tan(\pi/2)} \frac{1}{1 + y^2} dy =
	\int_{-\pi/2}^{\pi/2} \frac{1}{1 + T(x)^2} (1 + T(x)^2) dx
	\int_{-\pi/2}^{\pi/2} 1dx = \pi
$$
The theorem applies to unbounded intervals, as we can use limits to infinity. Both sides of the limit are the same. For the above, note that we can take $\int_{-t}^{t}$ as $t \to \infty$, but for each individual $t$, use the change of variable formula, and then note the RHS approaches $\pi$.

\subsubsection{The Lebesgue Integral in $\R^k$} Actually, just a note, for this section, the probability book references Spivak. That is actually the book where I learned the Change of Variables Theorem as well, so that just made me happy.
\\\\
The $k$ dimensional Lebesgue integral, the integral in $(\R^k, \rcal^k, \lambda)$, is denoted $\int f(x) dx$, it being understood that $x = (x_1, \cdots, x_k)$. In can also be denoted $\int\int_A f(x_1,x_2) dx_1dx_2$ and so on.
\\\\
Suppose that $T: U \to \R^k$ where $U$ is an open set in $\R^k$. The map has the form $Tx = (t_1(x), \cdots, t_k(x))$, which is continuously differentiable if the partial derivatives exist and are continuous in $U$. This is the definition of continuously differentiable. Let $D_x = [t_{ij}(x)]$ be the Jacobian matrix, and let $J(x) = \det D_x$ be the Jacobian determinant, and let $V = TU$.

\paragraph{Theorem 17.2 - Change of Variables} Let $T$ be a continuously differentiable map of the open set $U$ onto $V$. Suppose that $T$ is one-to-one and that $J(x) \neq 0$ for all $x$. If $f$ is nonnegative then:
$$
	\int_U f(Tx) |J(x)| dx = \int_V f(y) dy
$$
This matches the Riemann definition.
\\\\
Some things to note immediately. $V$ is open - it is equal to $T(U)$, where $T$ is the inverse image of $T^{-1}$. $T^{-1}$ exists and is continuously differentiable locally for every $x \in U$ via the Inverse Function Theorem - see Calculus on Manifolds for the proof, note it doesn't have anything to do with measurability, just the fact that $T$ is continuously differentiable and has nonzero Jacobian. Note, we also need the assumption that $T$ is one-to-one (as otherwise, we could have $T$ is invertible on disjoint neighborhoods of $x,y \in U$, but $T(x) = T(y)$ or $T(v) = T(u)$ for $v$ and $y$ in the corresponding neighborhoods).
\\\\
It is assumed that $f: V \to \R^1$ is a Borel Function, as otherwise the integral wouldn't exist definitionally. For the general $f$, the integral holds with $|f|$ in place of $f$, and if the two sides are finite, the absolute-value bars can be removed, and of course $f$ can be replaces with $f\1{B}$ or $f\1{A}$.
\\\\
We go through some examples before proving.

\paragraph{Example 17.6} Suppose that $T$ is a nonsingular linear transformation on $U = V = \R^k$. Then $D_x$ is for each $x$ the matrix of the transformation. If $T$ is identified with this matrix, then the equation becomes:
$$
	|\det T| \int_U f(Tx) dx = \int_V f(y) dy
$$
If $f = \1{TA}$, then this holds because of our earlier Theorem 12.2 for the Lebesgue measure:
$$
	\lambda_k(TA) = |\det T|\lambda_k(A)
$$
I actually went though this in Chapter 12 as well. Then, we can use the standard combine, and prove for simple $f$ and for general nonnegative $f$. Theorem 17.2 is easy in the linear case.

\paragraph{Example 17.7} In $\R^2$ take $U = \left[(\rho,\theta): \rho > 0, 0 < \theta < 2\pi\right]$ and $T(\rho, \theta) = (\rho \cos \theta, \rho \sin \theta)$. The Jacobian determinant is thus:
$$
	J(\rho, \theta) = \rho
$$
And so the Theorem gives us the formula for integrating polar coordinates:
$$
	\int\int_{\rho > 0, 0 < \theta < 2\pi} f(\rho\cos(\theta), \rho\sin(\theta)) d\rho d\theta=
	\int\int_{\R^2} f(x,y) dxdy
$$
Note, excluding $(0,0)$ and the line of $\theta = 0$ still gives us equality almost everywhere via Theorem 15.2. We have just $f = f\1{x \geq 0, y = 0}$ almost everywhere - they are not equal on the set:
$$
	A = \{\omega \in \R^2 : \omega_1 \geq 0, \omega_2 = 0\}
$$
This is a measurable set in $\rcal^2$, as it equals a countable intersection of smaller and smaller height rectangles. It is measure $0$, as we can find it has measure less than $\epsilon$ for all $\epsilon > 0$, by covering it in rectangles of the form $[n, n + 1] \times (-\epsilon/2^{n+1}, \epsilon/2^{n+1})$.
\\\\
Note, this example also shows why we need one-to-one. If we replaced the constraint on $\theta$ to $0 < \theta < 4\pi$, for example, the equation is false, as we would need a factor of 2 on the right. 

\paragraph{Proof of Theorem 17.2, Change of Variables} 
I'm just going to give the highlights. First, assume that we prove:
$$
	\int_U f(Tx)|J(x)| dx \geq \int_V f(y) dy
$$
Well, note that this symmetric, and we could let $f = f(T(x))|J(x)|$ and $T = T^{-1}$ (and we know the determinant of $T^{-1}$, and can apply the Theorem to it as the Inverse Function Theorem gives us continuous differentiability of $T^{-1}$), to show the inequality in the other direction. So, proving the above equation is enough.
\\\\
Then, consider $f$ of the form $f = \1{TA}$. Then, the equation reduces to:
$$
	\int_A |J(x)| dx \geq \lambda_k(TA)
$$
Each side is a measure on $\ucal = U \cap \rcal^k$. Note, $\ucal$ is a $\sigma$ algebra defined by Theorem 10.1, and $A \subseteq U$. $\lambda_k(TA)$ is clearly a measure, as it is easy enough to show emptyset, countable additivity, and we already proved it in Theorem 12.2. We have that $\int_A |J(x)| dx$ is a measure via our densities discussion in chapter 16, given that $|J(x)|$ is nonnegative, and continuous (via continuous differentiability) and thus measurable.
\\\\
If $\acal$ consists ot the rectangles $A$ satisfying $A^- \subseteq U$ (ie, interior is within $U$) then $\acal$ is a semiring generating $\ucal$. Semiring property is clear, and generating $\ucal$ again comes from Theorem 10.1. $U$ is also a countable union of $\acal$ sets (common fact), and the integral left side is finite for $A$ in $\acal$ (as the supremum of the determinant $J$ on a rectangle is bounded). It follows by Corollary 2 to Theorem 11.4 that if:
$$
	\int_A |J(x)| dx \geq \lambda_k(TA)
$$
Holds for $A$ in $\acal$, then it holds for $A$ in $\ucal$. But then, we can use our standard complex, and find that we can extend to $f$ simple, and $f$ nonnegative, and $f$ general to find:
$$
	\int_U f(Tx)|J(x)| dx \geq \int_V f(y) dy
$$
So, to prove the Change of Variables Theorem, we just have to prove for $A \in \acal$ (ie for rectangles in $U$):
$$
	\int_A |J(x)| dx \geq \lambda_k(TA)
$$
So we prove for such $A$. Split the rectangle $A$ into finitely many subrectangles $Q_i$ satisfying:
$$
	\diam(Q_i) < \delta
$$
Ie, the farthest distance between two points is less than $\delta$. It is obvious that such a partition is always possible, for any $\delta > 0$. We will determine $\delta$ later. Let $x_i$ be some point in $Q_i$. Given $\epsilon$, choose $\delta$ initially so that:
$$
	\left|J(x) - J(x')\right| < \epsilon \text{ if $x,x' \in A^-$ and } |x - x'| < \delta
$$
Note, $J(x)$ continuously differentiable is uniformly continuous on a closed rectangle rectangle (a small corollary to uniform continuity for continuous functions on intervals). Then, the diameter implies:
$$
	\sum_i |J(x_i)|\lambda_k(Q_i) \leq \int_A |J(x)|dx + \epsilon \lambda_k(A)
$$
Note I am reading through the proof, but it does get into the unhelpful region of like $\delta$ $\epsilon$ hell. I am already convinced via the theorem for Riemann integrals, and to be honest, the theorem here is very very similar.

\subsubsection{Stieltjes Integrals}
Suppose that $F$ is a function on $\R^k$ satisfying the hypotheses of Theorem 12.5 (continuous from above real function that satisfies $\Delta_A F \geq 0$ for bounded rectangles $A$). Then, there is a unique measure $\mu$ on $\rcal^k$ satisfying:
$$
	\mu(A) = \Delta_A F
$$
For such a $\mu$, we often notationally describe the integral across $\mu$ as:
$$
	\int_A f(x) dF(x) =
	\int_A f(x) \mu(dx)
$$
The left side is the \textit{Stieltjes Integral of $f$} with respect to $F$; since it is defined by the right side, it really isn't anything new.
\\\\
Suppose that $f$ is uniformly continuous on a rectangle $A$, and suppose that $A$ is decomposed into rectangles $A_m$ small enough such that $|f(x) - f(y)| < \epsilon/\mu(A)$ for $x,y \in A_m$. Then:
$$
	\left|\int_A f(x) dF(x) - \sum_m f(x_m) \Delta_{A_m} f\right| = 
	\left|\int_A f(x) - \sum_m f(x_m) \1{A_m} \mu(dx)\right|
$$
Note, $x_m \in A_m$ is fixed, so the $\omega$ is the $x$ in $f(x)$, and the $x$ in $\1{A_m}$ - however, when $x \in A_m$, the difference between $f(x)$ and $f(x_m)$ is less than $\epsilon/\mu(A)$. So, we find:
$$
	\leq \epsilon/\mu(A) * \mu(A) = \epsilon
$$
And so, in this case, we can define $\int_A f(x) dF(x)$ not as the Lebesgue integral, but the limit of these approximating sums without any reference to the general theory of measure. This is how Stieltjes Integrals are defined, I guess.

\subsection{Problems}
\subsubsection{17.1 - Riemann and Lebesuge Upper and Lower Integrals} So, there are a lot of prerequisite properties, just read from the book, I'm not gonna copy them. We want to prove:
\begin{enumerate}
	\item $D_f$ is a Borel set
	\item (i) implies (ii)
	\item (ii) implies (iii)
	\item (iii) implies (i)
	\item The $r$ of (i) must coincide with the $R_*f = R^*f$ of (ii).
\end{enumerate}
Just some of the notation this problem deals with, we have the upper and lower integrals as before:
$$
	L_*f = \sup \sum_i \left[\inf_{\omega \in A_i} f(\omega)\right] \mu(A_i)
$$
$$
	L^*f = \inf \sum_i \left[\sup_{\omega \in A_i} f(\omega)\right] \mu(A_i)
$$
Where the $A_i$ extend over all finite partitions $\{A_i\}$ of $\Omega$. Now, $R_*f$ and $R^*f$ are the upper and lower sums, but we only extend over finite partitions of $[0,1]$ into subintervals. Note, we are considering only $f$ as a bounded function on $[0,1]$. Clearly, we have:
$$
	R_*f \leq L_*f \leq L^*f \leq R^*f
$$
As $R$ is going over a subset of the possible partitions of $[0,1]$ into borel sets. We have the following conditions:
\begin{enumerate}
	\item There is an $r$ with the property that for each $\epsilon$ there is a $\delta$ such that:
	$$
		\left|r - \sum_i f(x_i)\lambda(I_i)\right|
	$$
	holds if $\{I_i\}$ partitions $[0,1]$ into subintervals with $\lambda(I_i) < \delta$ and if $x_i \in I_i$.
	
	\item $R_*f = R^*f$
	
	\item If $D_f$ is the set of points of discontinuity of $f$, then $\lambda(D_f) = 0$.
\end{enumerate}
So, now we begin the properties of the conditions.
\\\\
\textbf{(a)} $D_f$ is a borel set. Well, $D_f$ is the set of points of discontinuity of $f$. To be honest - I think we need more than just $f$ is bounded? In the solutions, I think there is an assumption that the first condition holds. Let $A_\epsilon$ be the set of $x$ such that for every $\delta$ there are points $y$ and $z$ satisfying $|y - x| < \delta$, $|z - x| < \delta$, and $|f(y) - f(z)| \geq \epsilon$.
\\\\
First, note that $D_f$ is the union of $A_\epsilon$. Indeed, if $f$ is not continuous as $x$, there is some $\epsilon$ such that for all $\delta$, being within $\delta$ of $x$ does not imply within $\epsilon$ of $f(x)$. So, we do have the union is $D_f$. We just need to show that each $A_\epsilon$ is a borel set, which follows if $A_\epsilon$ is closed (open sets are countable unions of rectangles).
\\\\
So, we just need to show that $A_\epsilon$ is closed. I mean, it would make sense, because if you are close enough to an $x$, you should also satisfy the $A_\epsilon$ property above. Well, take $t \not\in A_\epsilon$. This implies for $\epsilon$, there is a $\delta$ such that there are no points $y,z$ satisfying the above condition. This implies the same for all $\delta' < \delta$. So, take the open set around $t$, $|k - t| < \delta/2$. I think for all $k$ in this set, we have for $\epsilon$, $\delta/2$ doesn't satisfy the property, given that all $y,z$ in the $\delta/2$ range of $t$ should also be within the $\delta$ range of $x$. So, $A_\epsilon$ is indeed closed, as its complement is open, and so $D_f$ is a borel set, being the countable union of borel sets.
\\\\
\textbf{(b)} Prove that (i) implies (ii). Namely, the existence of such an $r$ implies $R_*f = R^*f$. 
\\\\
As noted in the chapter, as we let $x_i$ vary over the interval, we also have:
$$
	\left|r - \sum_i \sup_{x \in I_i} f(x_i)\lambda(I_i)\right| < \epsilon
$$
For $I_i$ with $\delta$ small enough. Similarly:
$$
	\left|r - \sum_i \inf_{x \in I_i} f(x_i)\lambda(I_i)\right| < \epsilon
$$
Both of these inequalities allow us to conclude that $r = R_*f = R^*f$. Indeed, consider from the infimum side. As we refine the intervals, the sum gets larger. So, for any interval consider in the sums of $R_*f$, we can refine it further, into finite interval decompositions of $[0,1]$, to get a value arbitrarily close to $\epsilon$. It is clear how an argument can be made to show this equality, and so (i) implies (ii).
\\\\
\textbf{(c)} Prove that (ii) implies (iii). Well, if we show each $\lambda(A_\epsilon)$ is arbitrarily small, we will have that $D_f$ has measure 0 as well (or, the union of measure 0 sets is measure 0). Take arbitrary $\epsilon$ and $\eta$. Choose a partition of intervals $I_i$ for which the corresponding upper and lower sums ($R_*f$ and $R^*f$) differ by at most $\epsilon\eta$. We can always find such a partition by taking two partitions arbitrarily close to $r$ by above and below and refining them. We have:
$$
	\sum_i \left[\sup_{\omega \in I_i} f(\omega) - \inf_{\Omega \in I_i} f(\omega)\right] \mu(I_i) < \epsilon\eta
$$
How many of these intervals $I_i$ have interiors that meet $A_\epsilon$? Note if $I_i$ has an interior that meets $A_\epsilon$, the difference above would be larger than $\epsilon$ (as in the definition of $A_\epsilon$, we could take $\delta$ less than half the length of the interval $I_i$). So, we have only $\eta$ of the intervals max can meet $A_\epsilon$ (rounded down) so:
$$
	\lambda(A_\epsilon) \leq \epsilon\eta \implies \lambda(A_\epsilon) = 0
$$
And the statement follows.
\\\\
\textbf{(d)} Prove that (iii) implies (i), or the set of points of discontinuities implies the existence of an $r$ such that for each $\epsilon > 0$:
$$
	\left|r - \sum_i \inf_{x \in I_i} f(x_i)\lambda(I_i)\right| < \epsilon
$$
Holds if $I_i$ are subintervals of $[0,1]$ with $\lambda(I_i) < \delta$. Let $M$ bound $|f|$ and given $\epsilon$ find an open $G$ such that $D_f \subseteq G$ and $\lambda(G) < \epsilon/M$. Such a $G$ can be found via Theorem 11.4.1 and as $D_f$ is a borel set in part a (and union of open rectangles is open). Take $C = [0,1] - G$, and note that $C$ is a closed set of a compact $[0,1]$, and so $C$ is also compact. There is a $\delta$ such that $|f(y) - f(x)| < \epsilon$ if $x$ lies in $C$ and $|y-x| < \delta$. As for how this $\delta$ can be found - for each $x \in C$, find an open interval around $x$ of $\delta_x$ small enough such that $|f(y) - f(x)| < \epsilon$ for all $y$ in the interval. Continuity around $x \not\in D_f$ implies the existence of $\delta_x$, compactness of $C$ and taking the minimum $\delta_x$ gives us the $\delta$ for each $\epsilon$.
\\\\
If $[0,1]$ is decomposed into intervals $I_i$ with $\lambda(I_i) < \delta$, and if $x_i \in I_i$, let $g$ be the function with value $f(x_i)$ on $I_i$. Let $\sum'$ denote summation over those $i$ for which $I_i$ meets $C$, and $\sum''$ for the other $i$. Note that $f(x)$ is Lebesgue integrable, as it is continuous outside a borel set of measure 0 and bounded. And so:
$$
	\left|\int_0^1 f(x) dx - \sum f(x_i)\lambda(I_i)\right| \leq
	\int_0^1 \left|f(x) - g(x)\right| dx \leq
	\sum' 2\epsilon\lambda(I_i) + \sum'' 2M\lambda(I_i) \leq 4\epsilon
$$
Note, the final portion comes from noting that $\sum'' \lambda(I_i) \leq \lambda(G) < \epsilon/M$. And so, as $f(x_i)$ was chosen arbitrarily, we found the existence of such an $r$, namely the value of the Lebesgue integral.
\\\\
\textbf{(e)} The $r$ of (i) must coincide with $R_*f = R^*f$. I think this is automatic, given that we had arbitrary $f(x_i)$, which we could go to limits with and take infimums and supremums for each specific partition.
\\\\
And so. Overall, I think this gives another proof for Riemann integrable implies Lebesgue integrable. If Riemann integrable, $R_*f = R^*f$, which bounds the Lebesgue upper and lower integrals.

\subsubsection{17.4 Limit of Integrable Function Shift Difference is Zero} Show that if $f$ is integrable, then:
$$
	\lim_{t \to 0} \int\left|f(x + t) - f(x)\right| dx = 0
$$
Use Theorem 17.1.
\\\\
I think, using Theorem 17.1, we can add a continuous $h(x+t)$ and $h(x)$ in there, somehow. But, I think we would want to conclude, for a continuous integrable $h$ with bounded support, we have:
$$
	\lim_{t \to 0} \int\left|h(x + t) - h(x)\right| dx = 0
$$
Bounded support means that $h$ is nonzero on some bounded set $A$. Note, we can just take the bounding interval $I$. Note, expand it by $+/-1$ on both sides to contain the support of $h(x+t)$ for $t$ small enough. The above becomes:
$$
	\lim_{t \to 0} \int_I\left|h(x + t) - h(x)\right| dx \leq
	\lim_{t \to 0} \lambda(I) * \sup\left[|h(x + t) - h(x)|: x \in I\right]
$$
Note, continuous $h$ is uniformly continuous on the bounded interval $I$. So, as $t$ goes to 0, the supremum goes to 0, and the above limit is 0:
$$
	\lim_{t \to 0} \int\left|h(x + t) - h(x)\right| dx = 0
$$
So, how can we make use of this for general $f$ integrable? Well, for integrable $f$ there is a continuous integrable $h$ with bounded support such that:
$$
	\int |f - h| dx < \epsilon
$$
Note, via symmetry, $f(x + t)$ is integrable as well, and so to is $f(x + t) - f(x)$, and $h(x + t) - h(x)$ will satisfy:
$$
	\int\left|f(x + t) - f(x) - h(x + t) + h(x)\right| dx
$$
$$
	\leq
	\int\left|f(x + t) - h(x + t)\right|dx + \int\left|h(x) - f(x)\right|dx < 2\epsilon
$$
Now, we note via our integral absolute value properties and the reverse triangle inequality:
$$
	\lim_{t \to 0} \left|\int\left|f(x + t) - f(x) - h(x + t) + h(x)\right| dx -
	\int\left|f(x + t) - f(x)\right|dx\right|
$$
$$
	=
	\lim_{t \to 0} \left|\int\left|f(x + t) - f(x) - h(x + t) + h(x)\right| - \left|f(x + t) - f(x)\right|dx\right|
$$
$$
	\leq
	\lim_{t \to 0} \int\left|\left|f(x + t) - f(x) - h(x + t) + h(x)\right| - \left|f(x + t) - f(x)\right|\right|dx
$$
$$
	\leq
	\lim_{t \to 0} \int\left|f(x + t) - f(x) - h(x + t) + h(x) - f(x + t) - f(x)\right|dx
$$
$$
	= \lim_{t \to 0} \int\left|h(x + t) - h(x)\right| dx = 0
$$
Note, the final inequality was the reverse triangle inequality. And so, we have:
$$
	\lim_{t \to 0} \int\left|f(x + t) - f(x)\right| dx = 
	\lim_{t \to 0} \int\left|f(x + t) - f(x) - h(x + t) + h(x)\right| dx < 2\epsilon
$$
For all $\epsilon > 0$. Thus, we have proven the theorem. qed.

\subsubsection{17.8 $1/(1+y^2)$ Change of Variables} Show that $(1 + y^2)^{-1}$ has equal integrals over $(-\infty,-1)$, $(-1,0)$, $(0,1)$, and $(1,\infty)$. Conclude that each has integral $\pi/4$ using that we have already proved:
$$
	\int_{-\infty}^\infty \frac{1}{1 + y^2} dy = \pi
$$
Expand the integrand in a geometric series and deduce Leibniz's Formula:
$$
	\frac{\pi}{4} = 1 - \frac{1}{3} + \frac{1}{5} - \frac{1}{7} + \cdots
$$
By Theorem 16.7.
\\\\
First note, $\tan(\pi/2) = \infty, \tan(-\pi/2) = -\infty, \tan(\pi/4) = 1, \tan(-\pi/4) = -1, \tan(0) = 0$. And so, we can just use change of variables to find:
$$
	\int_1^{\infty} \frac{1}{1 + y^2} dy = 
	\int_{\tan(\pi/4)}^{\tan(\pi/2)} \frac{1}{1 + y^2} dy =
	\int_{\pi/4}^{\pi/2} 1 dx = \pi/4
$$
This follows for each of the domains. Theorem 16.7 states that if $\sum_n f_n$ converges almost everywhere and $\left|\sum_{k=1}^n f_k\right| \leq g$ almost everywhere, where $g$ is integrable, then $\sum f_n$ and the $f_n$ are integrable and:
$$
...
$$
Converges via the \href{https://en.wikipedia.org/wiki/Alternating_series_test}{Alternating Series Test}. So, we have:
$$
	\sum_{n=0}^\infty \frac{(-1)^nx^{2n + 1}}{2n + 1}\1{x \in (0,1)}
$$
Converges almost everywhere, as multiplying by a factor between $0$ and $1$ won't effect the convergence. We also have this is dominated by $g = 1\1{x \in (0,1)}$ which is integrable. This is because the limit is clearly between 0 and 1, and multiplying by a factor between 0 and 1 doesn't change that. Finally, we note via the Taylor series expansion of $\arctan(x)$:
$$
	\arctan(x)\1{x \in (0,1)} = 
	\sum_{n=0}^\infty \frac{(-1)^nx^{2n + 1}}{2n + 1}\1{x \in (0,1)}
$$
Note, I don't think this is actually the right way to go about it. We have:
$$
	\pi/4 = \int_0^1 \frac{1}{1 + y^2} dy
$$
Now, we recall our geometric series which says that $\frac{1}{1 + y^2} = \sum_{n=0}^\infty (-y^2)^n$ for $y \in (0,1)$, and so the above equals:
$$
	= \int_0^1 \sum_{n=0}^\infty (-y)^{2n} dy
$$
Clearly, for $y \in (0,1)$, the function:
$$
	\sum_{n=0}^\infty (-y)^{2n} = \frac{1}{1 + y^2}
$$
Converges almost everywhere, and is bounded by integrable $g(y) = 1\1{y \in (0,1)}$. And so, we can make use of Theorem 16.7 to find:
$$
	\frac{\pi}{4} =
	\sum_{n=0}^\infty \int_0^1 (-y)^{2n} dy =
	\sum_{n=0}^\infty \frac{(-y)^{2n+1}}{2n + 1}\bigg|_{y=0}^{y=1} =
	\sum_{n=0}^\infty \frac{(-1)^{2n+1}}{2n + 1}
$$
Which proves Leibniz's formula (if there is an extra negative sign look at $(-1,0)$ instead). qed.

\subsubsection{17.12 Riesz Representation Theorem} Let $\lcal$ consist of the continuous functions on $\R^1$ with compact support. Show that $\lcal$ is a vector lattice in the sense of problem 11.4 and has the property that $f \in \lcal$ implies $f \wedge 1 \in \lcal$. Show that the $\sigma$ field $\fcal$ generated by $\lcal$ is $\rcal^1$. Suppose $\Lambda$ is a positive linear functional on $\lcal$; show that $\Lambda$ has the required continuity property if and only if $f_n(x) \downarrow 0$ uniformly in $x$ implies $\Lambda(f_n) \to 0$. Show that under this assumption on $\Lambda$ that there is a measure $\mu$ on $\rcal^1$ such that:
$$
	\Lambda(f) = \int f d\mu \quad\quad\quad f \in \lcal
$$
Show that $\mu$ is $\sigma$-finite and unique. This is a version of the \textit{Riesz Representation Theorem}.
\\\\
First, we show that the continuous functions on $\R^1$ with compact support are a vector lattice. First, they are a vector space, as addition of continuous functions is continuous, and the support of $f + g$, which is closed (by definition of being closure of nonzero points) is also bounded, clearly by the union of the two bounded supports of $f$ and $g$. Thus, the support of $f + g$ is compact. Further closed under scalar multiplication is clear, and so are all the associativity, commutativity, identity element (0), inverse element ($-f$), and distributivity properties are clear.
\\\\
We have that this vector space is also a lattice. Consider $(f \wedge g)(\omega) = \min(f(\omega), g(\omega))$. The minimum is clearly continuous, and also has compact support as well, as again the support is bounded by the support of the two original $f,g$. And so, $f \wedge g$ is a continuous compact support function on $\R^1$, and similarly so is $f \vee g$.
\\\\
And so, the continuous functions on $\R^1$ with compact support form a vector lattice. Now, we want to show $f \wedge 1 \in \lcal$ if $f \in \lcal$. We have:
$$
	(f \wedge 1)(\omega) = \min(f(\omega), 1)
$$
Clearly, the support of $f \wedge 1$ equals the support of $f$, and so $f \wedge 1$ is continuous with compact support and $f \wedge 1 \in \lcal$.
\\\\
So, the next thing to prove is the $\sigma$ field $\fcal$ generated by $\lcal$ is $\rcal^1$. Recall, $\fcal$ is the smallest $\sigma$ field with respect to which every $f \in \lcal$ is measurable. First, note that it contains the intervals, as for $(a,b)$ let $f$ be the function that is a straight line from $f(a) =
0$ to $f((a+b)/2) = 1$ to $f(b) = 0$. Clearly, $f \in \lcal$, and if $\fcal$ makes $f$ measurable, we have:
$$
	(a,b) = f^{-1}(0,1] \in \fcal
$$
Take $\bigcap (a, b + 1/n)$ implies that $(a,b] \in \fcal$ as well. As $\rcal^1$ is generated by the rectangles, we have $\rcal \subseteq \fcal$. Now, note that $f \in \lcal$, $f: \R^1 \to \R^1$ is continuous, and thus by Theorem 13.2 $f$ is measurable $\rcal$. Thus, we have $\fcal \subseteq \rcal$. And so, we must have:
$$
	\fcal = \rcal
$$
Now, onto the next part. Suppose $\Lambda$ is a positive linear functional on $\lcal$ ($f \geq 0$ pointwise implies $\Lambda(f) \geq 0$) - we want to show that $\Lambda$ has the required continuity property from above (of problems 11.4/11.5/16.12):
$$
	\text{$f_n \downarrow 0$ (pointwise) implies $\Lambda(f_n) \to 0$}
$$
If and only If:
$$
	\text{$f_n \downarrow 0$ (uniformly) implies $\Lambda(f_n) \to 0$}
$$
Clearly, the first automatically implies the second, as $f_n \downarrow 0$ uniformly means that $f_n \downarrow 0$ pointwise as well.
\\\\
So, assume that $f_n \downarrow 0$ (uniformly) implies $\Lambda(f_n) \to 0$. Take $f_n \downarrow 0$ pointwise. Examine the sets $A_n = [x : f_n(x) \geq \epsilon]$. Note that each $A_n$ is compact. As $f_n$ has compact support, the set of $x$ where $f_n$ is nonzero is bounded, and so $A_n$ being a subset of that must be bounded. Further, $A_n$ is closed, as if $y \not\in A_n$, $f(y) < \epsilon$, and continuity implies an interval around $y$ $B(y)$ for which $f(B(y)) < \epsilon$ as well. Now note:
$$
	\bigcap A_n = \emptyset
$$
This is because for each $x$, it will not be in $A_n$ for $n \geq N$ via pointwise continuity. Now, recall the finite intersection property - if a collection of compact sets has the property that every finite subcollection has nonempty intersection, then the whole intersection is nonempty. As the intersection above is empty, we must have that some finite subset of $A_n$ - say $A_{n_1}, \cdots, A_{n_k}$ - satisfy:
$$
	A_{n_1} \cap \cdots \cap A_{n_k} = \emptyset
$$
Note that as $f_1 \geq f_2 \geq \cdots$, as we are \textit{decreasing}, we must have that one of the $A_{n_k}$ is empty. And so, this implies that if $f_n \downarrow 0$ pointwise, we actually have $f_n \downarrow 0$ uniformly, which implies $\Lambda(f_n) \to 0$. And so, the second statement also implies the first.
\\\\
So, if we only just assume that if $f_n(x) \downarrow 0$ uniformly in $x$ implies $\Lambda(f_n) \to 0$, and we also have that $\Lambda$ is a positive linear functional on $\lcal$ - then, $\lcal$ and $\Lambda$ as defined satisfy all the initial properties of problem 11.4. By 11.5 and 16.2, we have a measure $\mu$ on $\fcal = \rcal^1$ which satisfies:
$$
	\Lambda(f) = \int f d\mu
$$
What remains to be proven is that $\mu$ is $\sigma$-finite, and unique. Well, first I will try and prove $\sigma$ finite. Recall, $\mu$ is sigma finite, if for some countable or finite sequence $\R^1 = A_1 \cup A_2 \cup \cdots$ satisfies $\mu(A_k) < \infty$. Recall, we have:
$$
	\int I_{A_k} d\mu = \mu(A_k)
$$
Let $[n,n+]$ be an interval. We can find a continuous and compact support $f_n$ such that $f \geq \1{[n,n+1]}$. This should be clear, just make a parabola or large enough triangle. We have by monotonicity of the integral:
$$
	\mu[n,n+1] = \int \1{[n,n+1]} d\mu \leq \int f_n d\mu = \Lambda(f_n) < \infty
$$
Where the last step comes from $\Lambda$ being \textit{real}. And so, we can just cover $\R^1$ by these countable intervals, and we have that $\mu$ is $\sigma$-finite. Now, for uniqueness. We need to find if:
$$
	\int f d\mu_1 = \Lambda(f) = \int f d\mu_2
$$
For all $f$, then $\mu_1 = \mu_2$. Well, if we wanted to use Theorem 10.3, $\sigma$ finiteness of $\mu_1, \mu_2$ means we only have to prove that they agree on the intervals. For that, I think we can find:
$$
	f_n \uparrow \1{[a,b]}
$$
By taking trapezoids. And so, we would have, for each $f_n$:
$$
	\int f_n d\mu_1 = \int f_n d\mu_2
$$
By Theorem 16.2 the Monotone Convergence Theorem, we have:
$$
	\mu_1([a,b]) = \int \1{[a,b]} d\mu_1 = \lim_n \int f_n d\mu_1 =
	\lim_n \int f_n d\mu_2 = \int \1{[a,b]} d\mu_2 = \mu_2([a,b])
$$
And so the $\mu$ is unique as well. qed.


\section{Section 18 - Product Measure and Fubini's Theorem}
\subsection{Notes} 
The overall problem of this chapter. Let $(X, \xcal)$ and $(Y, \ycal)$ be measurable spaces with measures $\mu$ and $\nu$ respectively. The problem is to construct on the Cartesian product $X \times Y$ a product measure $\pi$ such that $\pi(A \times B) = \mu(A)\nu(B)$ for $A \subseteq X$ and $B \subseteq Y$.

\subsubsection{Product Spaces} The natural $\sigma$ field to consider on $X \times Y$ is the one generated by the measurable rectangles $A \times B$ for $A \in \xcal$ and $B \in \ycal$. It is denoted $\xcal \times \ycal$.

\paragraph{Example 18.1 - $\rcal^2 = \rcal^1 \times \rcal^1$} It is clear that $\rcal^2 \subseteq \rcal^1 \times \rcal^1$, as the rectangles generate $\rcal^2$, and the measurable rectangles that generate $\rcal^1 \times \rcal^1$ contain the rectangles.
\\\\
For the other direction, if $A$ is an interval, $[B : A \times B \in \rcal^2]$ contains $\R^1$ and is clearly closed under complements and countable unions, and is thus a $\sigma$ field. It is a $\sigma$ field that contains the intervals - and hence, the borel sets. Therefore, if $B$ is a borel set, $[A : A \times B \in \rcal^2]$ is also a $\sigma$ field that contains the intervals (following the above deduction), and so it contains all the borel sets. To, if $A \in \rcal^1$, and $B \in \rcal^1$, we have $A \times B \in \rcal^2$, and so all of the \textit{measurable rectangles} are in $\rcal^2$. As $\rcal^1 \times \rcal^1$ is the smallest $\sigma$ field containing all the measurable rectangles, we have $\rcal^1 \times \rcal^1 \subseteq \rcal^2$.
\\\\
In total, we have thus found $\rcal^2 = \rcal^1 \times \rcal^1$.

\paragraph{Theorem 18.1 - $\xcal \times \ycal$ is much larger than the Measurable Rectangles} 
\begin{enumerate}
	\item If $E \in \xcal \times \ycal$, then for each $x \in X$ the set $[y : (x,y) \in E]$ lies in $\ycal$ and for each $y \in Y$ the set $[x : (x,y) \in E]$ lies in $\xcal$.
	
	\item If $f$ is measurable $\xcal \times \ycal$, then for each fixed $x \in X$ the function $f(x, \cdot)$ is measurable $\ycal$, and for each fixed $y \in Y$ the function $f(\cdot, y)$ is measurable $\xcal$.
\end{enumerate}
Note, the sets above are \textit{sections} of $E$ determined by $x/y$, and the function is a \textit{section} of $f$ determined by $x/y$.
\\\\
\textbf{Proof:} Fix $f$, and consider the mapping $T_x: Y \to X \times Y$ defined by:
$$
	T_x(y) = (x,y)
$$
If $E = A \times B$ is a measurable rectangle, $T_x^{-1}$ is $B$ or $\emptyset$, as we are taking a cartesian product here, and in either case, $T_x^{-1}E \in \ycal$. By Theorem 13.1(i), as the measurable rectangles generate $\xcal \times \ycal$, we have $T_x$ is measurable $\ycal/\xcal \times \ycal$. Hence:
$$
	[y : (x,y) \in E] = T_x^{-1}E \in \ycal \text{ for all $E \in \xcal \times \ycal$}
$$
By Theorem 13.1(ii), composition of measurable functions, if $f$ is measurable $\xcal \times \ycal/\rcal^1$, then $fT_x$ is measurable $\ycal/\rcal^1$. Hence, $f(x, \cdot) = fT_x(\cdot)$ is measurable $\ycal$. The symmetric statements are proved the same way. qed.

\subsubsection{Product Measure}
\paragraph{Product Measure on Finite Measure Spaces} Now, suppose $(X, \xcal, \mu)$ and $(Y, \ycal, \nu)$ are measure spaces, and suppose for now that $\mu$ and $\nu$ are \textit{finite}. Note that:
$$
	m(x) = \nu[y : (x,y) \in E]
$$
Is well defined for $x \in X$ and $E \in \xcal \times \ycal$ by the previous theorem. If $\lcal$ is the class of $E$ in $\xcal \times \ycal$ for which this function is measurable $\xcal$ - ie $m: X \to \R$ is measurable $\xcal/\rcal$ - then we have that $\lcal$ is actually a $\lambda$ system.
\\\\
Recall, a $\lambda$-system contains $\Omega$, is closed under complements, and closed under countable disjoint unions. Clearly, $\Omega = X \times Y \in \lcal$, as then for $A \in \rcal$, we have:
$$
	m^{-1}(A) = X \text{ or } \emptyset \text{ depending if $\nu(\rcal) \in A$}
$$
Closed under complements, as if $m_E$ is measurable, we note, recalling that our assumption is that $\mu$ and $\nu$ are finite:
$$
	m_E(x) = \nu[y : (x,y) \in E] \quad\quad\quad
	m_{E^c}(x) = \nu[y : (x,y) \in E^c]
$$
$$
	m_{E^c}^{-1}(A) =
	\left\{x : \nu[y : (x,y) \in E^c] \in A\right\} =
	\left\{x : \nu[y : (x,y) \in E] \in \nu(\rcal) - A\right\}
$$
$$
	=
	m_E^{-1}(\nu(\rcal) - A) \in \xcal
$$
Note, $\nu(\rcal) - A$ is in $\rcal$, via Theorem 12.1 for translations and a similar argument for reflection. We now note that $\lcal$ is closed under disjoint unions as well, as:
$$
	m_{\bigcup E_n}^{-1}(A) =
	\left\{x : \nu[y : (x,y) \in \bigcup E_n] \in A\right\} =
	\left\{x : \sum_n \nu[y : (x,y) \in E_n] \in A\right\}
$$
$$
	= \left[\sum_n m_{E_n}\right]^{-1}[A] \in \xcal
$$
Note, the first split into addition comes from the fact that via countable additivity:
$$
	\nu[y : (x,y) \in \bigcup E_n] =
	\nu\left[\bigcup_n y : (x,y) \in E_n\right] =
	\sum_n \nu[y : (x,y) \in E_n]
$$
And the second step notes that the sum of measurable functions is measurable, which comes directly from Theorem 13.3.
\\\\
Now, note that for $E = A \times B$, we have:
$$
	m_E(x) = \nu[y : (x,y) \in A \times B] = \1{x \in A}\nu(B)
$$
Which is clearly measurable, so $\lcal$ contains the measurable rectangles. The measurable rectangles are clearly a $\pi$ system, so $\lcal$ contains the sigma algebra generated from this $\pi$ system via the $\pi-\lambda$ theorem, and so $\lcal$ contains $\xcal \times \ycal$. As we only pulled from sets within $\xcal \times \ycal$, we have:
$$
	\xcal \times \ycal = \lcal \text{ the sets for which $m_E(x) = \nu[y : (x,y) \in E]$ is measurable $\xcal$}
$$
We now define:
$$
	\pi'(E) = \int_X m_E \mu(dx) \quad\quad\quad E \in \xcal \times \ycal
$$
We have that $\pi'$ is a finite measure on $\xcal \times \ycal$. First, note for $E \in \xcal \times \ycal$, $m_E$ is measurable, and importantly, nonnegative, so the Lebesgue integral exists. Further, as we have assumed finiteness of $\mu$ and $\nu$, we have that $\pi'$ is finite as well. Note, we have:
$$
	m_E(x) \leq m_{X \times Y}(x) = \nu(Y) < \infty \implies
	\pi'(E) \leq \int_X \nu(Y) \mu(dx) =
	\nu(Y)\mu(X) < \infty
$$
Similarly, we find:
$$
	\pi''(E) = \int_Y \mu\left[x : (x,y) \in E\right] \nu(dy) \quad\quad\quad E \in \xcal \times \ycal
$$
Is a finite measure on $\xcal \times \ycal$ as well. For measurable rectangles, $A \in \xcal, B \in \ycal$, we find:
$$
	m_{A \times B}(x) = \1{x \in A}\nu(B) \implies
	\pi'(A \times B) = \nu(B) \int_X \1{x \in A} \mu(dx) = \mu(A)\nu(B)
$$
$$
	\pi''(A \times B) = \mu(A) \int_Y \1{y \in B} \nu(dy) = \mu(A)\nu(B)
$$
The class of $E$ in $\xcal \times \ycal$ for which $\pi'(E) = \pi''(E)$ thus contains the measurable rectangles; as this class is a $\lambda$-system (clear that equality is closed under complements and disjoint unions), it contains $\xcal \times \ycal$. The common value:
$$
	\pi(E) = \pi'(E) = \pi''(E) \text{ for $E \in \xcal \times \ycal$}
$$
Is the product measure we were looking for. qed.

\paragraph{Product Measure on $\sigma$-finite Measure Spaces} So, now, we assume that $(X, \xcal, \mu)$ and $(Y, \ycal, \nu)$ are measure spaces, and $\mu$ and $\nu$ are sigma-finite. First, I want to determine that $\lcal$, the set of $E$ for which:
$$
	m_E(x) = \nu\left[y : (x,y) \in E\right]
$$
Is measurable, is a $\lambda$-system. The countable disjoint unions argument still applies. But for the complement argument, assume $m_E$ is measurable, and we note:
$$
	m_E(x) + m_{E^c}(x) = \nu(Y)
$$
As for why that is - we have via additivity of disjoint sets:
$$
	\nu\left[y : (x,y) \in E\right] +
	\nu\left[y : (x,y) \in E^c\right] =
	\nu\left[y : (x,y) \in E \cup E^c\right] =
	\nu\left[y : (x,y) \in X \times Y\right] = \nu(Y)
$$
Note, if $\nu(Y) < \infty$, we could automatically conclude that $m_E$ is measurable, via addition of measurable functions. However, it now might be infinite. Can we still conclude that $m_{E^c}$ is measurable? We have the assumption that $Y$ is $\sigma$ finite.
\\\\
Rather than closed under complements, we can instead show closed under proper differences - this is noted in the $\lambda$ system chapter 3 notes. So, assume that $A, B \in \lcal$, and $A \subset B$. We want to show $B - A \in \lcal$. Via $\sigma$-finiteness, pick $Y_1 \subseteq Y_2 \subseteq \cdots$ in $Y$, such that $\nu(Y_n) < \infty$ and $\bigcup_n Y_n = Y$. Define:
$$
	A^n = A \cap (X \times Y_n) \quad\quad\quad
	B^n = B \cap (X \times Y_n)
$$
Clearly, $A^n \subseteq B^n$, and both approach $A,B$ from below respectively. We want to show:
$$
	m_{B - A}(x) = \nu\left[y : (x,y) \in B - A\right]
$$
Is measurable. Note that:
$$
	m_{B^n - A^n}(x) =
	\nu\left[y : (x,y) \in B^n - A^n\right] =
	\nu\left[\left\{y : (x,y) \in B^n\right\} - \left\{y : (x,y) \in A^n\right\}\right]
$$
$$
	=
	\nu\left[\left\{y : (x,y) \in B^n\right\}\right] - \nu\left[\left\{y : (x,y) \in A^n\right\}\right]
	= m_{B^n}(x) - m_{A^n}(x)
$$
Note, the subtraction is well defined, as $\nu(\left\{y : (x,y) \in B^n\right\}) \leq \nu(Y_n) < \infty$. And so, we have that $m_{B^n - A^n}$ is measurable. Now, we note that:
$$
	\bigcup_{n=1}^\infty B_n - A_n \uparrow B - A
$$
This much is clear. And so:
$$
	\nu\left[y : (x,y) \in B_n - A_n\right] \uparrow
	\nu\left[y : (x,y) \in \bigcup_{n=1}^\infty B_n - A_n\right] =
	m_{B - A}(x) = \nu\left[y : (x,y) \in B - A\right]
$$
As for why the first $\uparrow$ - clearly, the fibers go up, as the sets go up as well. And so, we can take a supremum, or note that an increasing limit always exists, and so:
$$
	m_{B - A}(x) = \sup_n m_{B_n - A_n}(x) =
	\lim_n m_{B_n - A_n}(x)
$$
Note by Theorem 13.4, the supremum of measurable functions is measurable, and so $\lcal$ is closed under proper differences and thus a $\lambda$ system.
\\\\
So, continuing. If the spaces are $\sigma$ finite, we still have that $\lcal$, the class of $E \in \xcal \times \ycal$ such that $m_E(x) = \nu\left[y : (x,y) \in E\right]$ is measurable $\xcal/\rcal$, is still a $\lambda$ system, that also clearly contains the measurable rectangles, and so by the $\pi-\lambda$ theorem, coincides with $\lcal$. And so, we still have that the measures on $\xcal \times \ycal$:
$$
	\pi'(E) = \int_X m_E \mu(dx) = \int_X \nu[y : (x,y) \in E] \mu(dx) \quad\quad
	\pi''(E) = \int_Y \mu\left[x : (x,y) \in E\right] \nu(dy)
$$
Are well defined, as the integral for nonnegative measurable functions always exists (but may possibly be finite). We now want to show that they are both equal. Note, the equality arguments above may not have followed for possible infinite measures. The following equality argument works in both the $\sigma$ finite and finite cases.
\\\\
Let $\{A_m\}$ and $\{B_n\}$ be decompositions of $X$ and $Y$ into sets of finite measure (such a decomposition can be found). Put:
$$
	\mu_m(A) = \mu(A \cap A_m) \quad\quad\quad
	\nu_n(B) = \mu(B \cap B_n)
$$
Note, $\nu(V) = \sum_m \nu_m(B)$. Let $\pi'_{mn}$ and $\pi''_{mn}$ be the integrals, but for measures $\mu_m$ and $\nu_n$. Equality in our finite case allows us to find:
$$
	\sum_{mn} \pi'_{mn}(E) = \sum_{mn} \pi''_{mn}(E)
$$
Now, we also note, for the $\pi'$ defined above, which we already noted is well defined, we have:
$$
	\pi'(E) = \sum_{mn} \pi'_{mn}(E)
$$
As for why that is. We can clearly find equality on measurable rectangles, by using limits:
$$
	\pi'(A \times B) = \mu(A)\nu(B) = \nu(B)\sum_m \mu_m(A) =
	\sum_{mn} \mu_m(A)\nu_n(B) = \sum_{mn} \pi'_{mn}(E)
$$
We have that the class of $E \in \xcal \times \ycal$ where they agree is also a $\lambda$ system. Note, closed under disjoint sets is clear, and closed under proper differences follows by an argument similar to the above. And so, we find via the $\pi-\lambda$ theorem and finite measure equality:
$$
	\pi'(E) = \sum_{mn} \pi'_{mn}(E) =
	\sum_{mn} \pi''_{mn}(E) = \pi''(E)
$$
Thus, the two integrals coincide in the $\sigma$-finite case as well. Moreover:
$$
	\pi'(A \times B) = \sum_{mn} \mu_m(A)\nu_n(B) = \mu(A)\nu(B)
$$

\paragraph{Theorem 18.2 - Existence and Uniqueness of Product Measure} If $(X, \xcal, \mu)$ and $(Y, \ycal, \nu)$ are $\sigma$-finite measure spaces, $\pi(E) = \pi'(E) = \pi''(E)$ defines a $\sigma$-finite measure on $\xcal \times \ycal$; it is the only measure such that $\pi(A \times B) = \mu(A) \cdot \nu(B)$ for measurable rectangles.
\\\\
First - note that we proved existence above. We still need that $\pi$ is $\sigma$ finite on $\xcal \times \ycal$, and then uniqueness. The products $A_m \times B_n$ for $\{A_m\}$ and $\{B_n\}$ finite measure sets that decompose $X$ and $Y$ respectively, are also countable sets that decompose $X \times Y$ into measurable rectangles of finite measure. This proves $\sigma$-finiteness, and uniqueness follows directly afterwards, from making use of Theorem 10.3, as the measurable rectangles form a $\pi$-system generating $\xcal \times \ycal$. qed.

\subsubsection{Fubini's Theorem} We want to be able to express the integral with respect to the product space and product measure, in terms of the component spaces and component measures. The formulas look something like:
$$
	\int_{X \times Y} f(x,y) \pi(d(x,y)) = \int_X \left[\int_Y f(x,y) \nu(dy)\right] \mu(dx)
$$
and
$$
	\int_{X \times Y} f(x,y) \pi(d(x,y)) = \int_Y \left[\int_X f(x,y) \mu(dx)\right] \nu(dy)
$$
The left side is the \textit{double integral}, and the right side is the \textit{iterated integral}. The formulas hold very generally. Consider the first inner integral:
$$
	\int_Y f(x,y) \nu(dy)
$$
By Theorem 18.1(ii), for $f$ measurable $f(x,\cdot)$ is measurable $\ycal$. The questions we need to consider are:
\begin{enumerate}
	\item Does the integral $\int_Y f(x,y) \nu(dy)$ exist? Even though $f(x,\cdot)$ is measurable, we still might not get existence, such as in the $\infty - \infty$ case.
	
	\item If it exists for every $x$ - is the function $g(x) = \int_Y f(x,y) \nu(dy)$ measurable $\xcal$?
	
	\item And finally, if it is measurable $\xcal$, does its integral agree with the double integral?
\end{enumerate}
We do this proof in steps.

\paragraph{Nonnegative $f$} First, consider nonnegative $f$. If $f = \1{E}$, everything follows from Theorem 18.2. We have:
$$
	\int_Y f(x,y) \nu(dy) = \int_Y \1{y : (x,y) \in E} \nu(dy) = \nu\left[y : (x,y) \in E\right]
$$
And so, we have that our integral expression becomes:
$$
	\int_{X \times Y} \1{E} \pi(d(x,y)) = \pi(E)
	= \pi'(E) = \int_X \nu\left[y : (x,y) \in E\right] \mu(dx)
$$
Which is true via Theorem 18.2. So, existence of the integral follows via non-negativity, measurability and equality of iterated integrals and double integrals comes from Theorem 18.2.
\\\\
Now, consider $f$ nonnegative simple function. Then, we have:
$$
	\int_Y f(x,y) \nu(dy) = \int_Y \sum_{i=1}^n a_n \1{y : (x,y) \in E_n} \nu(dy)
$$
Again, the integral exists, as it is a non-negative measurable function. We have via additivity, the integral as a function of $x$ is:
$$
	\sum_{i=1}^n a_n \nu\left[y : (x,y) \in E_n\right]
$$
By Theorem 18.2, as $E_n \in \xcal \times \ycal$, each of the individual summands is measurable $\xcal/\rcal$, and by Theorem 13.3, the linear sum of $\xcal$ measurable functions is measurable $\xcal$. So, the iterated integral is well defined. Now, we just have to show equality of both sizes:
$$
	\int_{X \times Y} f(x,y) \pi(d(x,y)) = 
	\sum_{i=1}^n a_n \pi(E_n) =
	\sum_{i=1}^n a_n \int_X \nu\left[y : (x,y) \in E\right] \mu(dx)
$$
$$
	= \int_X \sum_{i=1}^n a_n \nu\left[y : (x,y) \in E\right] \mu(dx) =
	\int_X \left[\int_Y f(x,y) \nu(dy)\right] \mu(dx)
$$
Where the first equality comes from Theorem 15.1(i), the second equality comes from the proof of Theorem 18.2, the third equality comes from the linearity of integrals, and the fourth comes from our above argument. So, we have proven Fubini's Theorem for nonnegative simple functions.
\\\\
Finally, we consider general $f$ nonnegative. By Theorem 13.5, we have nonnegative simple $f_n \uparrow f$. We note via the monotone convergence theorem:
$$
	\int_Y f(x,y) \nu(dy) =
	\lim_n \int_Y f_n(x,y) \nu(dy)
$$
This is because, $f_n(x,\cdot) \uparrow f(x,\cdot)$ as well, for measurable functions via Theorem 18.1. So, our integral exists. We also note it is measurable as a function of $x$, as the limits exist, each $\int_Y f_n(x,y) \nu(dy)$ is measurable $\xcal$ via our previous argument, and Theorem 13.4 gives us that the limit (or equivalently the supremum in this case, which might be better given the infinite values) is measurable as well. So, we just need to show equality of the double and iterated integral. We have:
$$
	\int_{X \times Y} f(x,y) \pi(d(x,y)) = 
	\lim_n \int_{X \times Y} f_n(x,y) \pi(d(x,y)) =
	\lim_n \int_X \left[\int_Y f_n(x,y) \nu(dy)\right] \mu(dx)
$$
Now, note that via $\int_Y f(x,y) \nu(dy) = \lim_n \int_Y f_n(x,y) \nu(dy)$, we can again make use of monotone convergence theorem to move the limit inside (as $f_1 \leq f_2 \leq \cdots$ and monotonicity + non-negativity of the $f_n$ imply monotonicity and non-negativity of the integrals) giving us:
$$
	= \int_X \lim_n \left[\int_Y f_n(x,y) \nu(dy)\right] \mu(dx) =
	\int_X \left[\int_Y f(x,y) \nu(dy)\right] \mu(dx)
$$

\paragraph{Integrable $f$} So now, we only make the assumption that $f$ is integrable with respect to $\pi$. Then, we have:
$$
	\int_{X \times Y} |f(x,y)| \pi(d(x,y)) = \int_X \left[\int_Y |f(x,y)| \nu(dy)\right] \mu(dx)
$$
By our previous non-negativity argument, and we have both sides are finite. Now, make the assumption:
$$
	\int_Y |f(x,y)| \nu(dy) < \infty
$$
For all $x$. Note, this will not necessarily be the case - we can have $f: \R^2 \to \R$ where we might imagine this happens, such as a point going to infinity. But, given this assumption, we have via integrability for each $x$:
$$
	\int_Y f(x,y) \nu(dy) =
	\int_Y f^+(x,y) \nu(dy) - \int_Y f-(x,y) \nu(dy)
$$
Each of these two summands are measurable $\xcal$ by our above arguments, nonnegative, and so we can take integrals across $\xcal$. Integrating out $\xcal$, we have:
$$
	\int_X \left[\int_Y f(x,y) \nu(dy)\right] \mu(dx) =
	\int_X \left[\int_Y f^+(x,y) \nu(dy) - \int_Y f^-(x,y) \nu(dy)\right] \mu(dx)
$$
...
$$
$$
	= \int_{X \times Y} f^+(x,y) \pi(d(x,y)) - \int_{X \times Y} f^-(x,y) \pi(d(x,y)) =
	\int_{X \times Y} f(x,y) \pi(d(x,y))
$$
So now, we don't assume that less than infinity expression for each $x$. Let:
$$
	A_0 = \left\{x : \int_Y |f(x,y)| \nu(dy) < \infty\right\}
$$
Note, we must have:
$$
	\mu(X - A_0) = 0
$$
If $f$ is integrable with respect to $\pi$. Why? We still have that:
$$
	\int_X \left[\int_Y |f(x,y)| \nu(dy)\right] \mu(dx) = \int_{X \times Y} |f(x,y)| \pi(d(x,y)) < \infty
$$
And so, as the inner integral, integrated across $X$, has an integral value that is \textit{finite}, it must be finite almost everywhere directly via Theorem 15.2(iii). Now, note that for $x \in A_0$, we have:
$$
	\int_Y f(x,y) \nu(dy) =
	\int_Y f^+(x,y) \nu(dy) - \int_Y f^-(x,y) \nu(dy)
$$
Holds. We find that $\int_Y f(x,y) \nu(dy)$ is thus measurable on $A_0$, and so 
$$
	\int_{X \times Y} f(x,y) \pi(d(x,y)) = \int_X \left[\int_Y f(x,y) \nu(dy)\right] \mu(dx)
$$
Follows if the inner integral on the right is given some arbitrary constant value on $X - A_0$ (this is via Theorem 15.2(v). And so, putting all of our above arguments together, we have:

\paragraph{Theorem 18.3 - Fubini's Theorem} Assume $(X, \xcal, \mu)$ and $(Y, \ycal, \nu)$ are $\sigma$-finite measure spaces, and $\pi$ is our unique measure on $\xcal \times \ycal$ such that $\pi(A \times B) = \mu(A) \cdot \nu(B)$ for measurable rectangles. For nonnegative $f$ the functions:
$$
	\int_Y f(x,y) \nu(dy) \quad\quad\quad
	\int_X f(x,y) \mu(dx)
$$
Are measurable $\xcal$ and $\ycal$, respectively, and we have:
$$
	\int_{X \times Y} f(x,y) \pi(d(x,y)) = \int_X \left[\int_Y f(x,y) \nu(dy)\right] \mu(dx)
$$
and
$$
	\int_{X \times Y} f(x,y) \pi(d(x,y)) = \int_Y \left[\int_X f(x,y) \mu(dx)\right] \nu(dy)
$$
Hold. If $f$ (not necessarily nonnegative) is integrable with respect to $\pi$, then the two functions are finite and measurable on:
$$
	A_0 = \left\{x : \int_Y |f(x,y)| \nu(dy) < \infty\right\} \quad\quad\quad
	B_0 = \left\{y : \int_X |f(x,y)| \mu(dx) < \infty\right\}
$$
Respectively, where $\mu(X - A_0) = \nu(Y - B_0) = 0$, and again the double/iterated integral equality holds. Proven in the above arguments. qed.

\paragraph{Example 18.2 - Integral Across a Disk} Let $D_r$ be the closed disk in the plane with center at the origin and radius $r$. We note:
$$
	\lambda_2(D_r) =
	\int_\R \1{D_r} d\lambda_2 =
	\int_{D_r} 1 d\lambda_2
$$
Noting that we can switch the closed disk with the open disk, as the indicators are equal almost everywhere. Let $T(\rho, \theta) = (\rho \cos \theta, \rho \sin \theta)$. We have $T$ on $(0, r) \times (0,2\pi)$ is the open disk, without the line. Note, the indicator of $T$ on this set is equal to the indicator of the open disk almost everywhere, so integral equalities work out. We also have the Jacobian of $T$ is $\rho$ as well. So, by Theorem 17.2, we have:
$$
	\int_{(0, r) \times (0,2\pi)} |\rho| 1 \circ T d\lambda_2 =
	\int_{\text{Open $D_r$ without the line}} 1 d\lambda_2 =
	\int_{D_r} 1 d\lambda_2 =
	\lambda_2(D_r)
$$
So, to find the measure of $D_r$, we just have to calculate the first integral. We do this via Fubini's Theorem. We note that $1 \1{(0, r) \times (0,2\pi)}$ is measurable and nonnegative on $\rcal^2 = \rcal \times \rcal$, where $\rcal$ is $\sigma$ finite. And so, Theorem 18.3 gives us:
$$
	\int_{(0, r) \times (0,2\pi)} |\rho| d\lambda_2 =
	\int_{(0, r) \times (0,2\pi)} \rho d\lambda_2 =
	\int_0^{2\pi} \left[\int_0^r \rho d\rho\right] d\theta
$$
Where we note we replace $d\lambda$ with our variables of integration, which were $\rho$ and $\theta$ (the inputs into the function on $\R^2$). We can evaluate this as:
$$
	=
	\int_0^{2\pi} \frac{r^2}{2} d\theta = \pi r^2
$$

\paragraph{Example 18.3 - Integral of Standard Normal Distribution} Define:
$$
...
$$
Via linearity, we find:
$$
	= \int_{-\infty}^\infty e^{-x^2} \left(\int_{-\infty}^\infty e^{-y^2} dy\right) dx
$$
Now, we note that we can consider the integrand as a function of $x$, say:
$$
	g(x) = e^{-x^2} \left(\int_{-\infty}^\infty e^{-y^2} dy\right)
$$
For fixed $x$, we can move the $e^{-x^2}$ inside:
$$
	= \int_{-\infty}^\infty e^{-x^2}e^{-y^2} dy =
	\int_{-\infty}^\infty e^{-(x^2+y^2)} dy
$$
Anyway. Now consider (without proving equality yet):
$$
	\int_{\R^2} e^{-(x^2+y^2)} d\lambda_2
$$
By directly applying Fubini's Theorem to this nonnegative function, for $\rcal^2$, where $\rcal$ is $\sigma$ finite, we have:
$$
	\int_{\R^2} e^{-(x^2+y^2)} d\lambda_2 =
	\int_{\R} \left[\int_{\R} e^{-(x^2+y^2)} dy\right] dx =
	\int_{\R} \left[\int_{\R} e^{-x^2}e^{-y^2} dy\right]dx =
	\int_{-\infty}^\infty e^{-x^2} \left(\int_{-\infty}^\infty e^{-y^2} dy\right) dx
$$
Where we note, just going this way, it is easier to conclude that the integrand as a function of $x$ is measurable, and we can move the $e^{-x^2}$ inside and out of the function via linearity for a fixed $x$. Anyway, the entire point is:
$$
	I^2 = \int_{\R^2} e^{-(x^2+y^2)} d\lambda_2
$$
Now, we will make use of Change of Variables. We note that for $T$ as defined above - $T(\rho, \theta) = (\rho \cos \theta, \rho \sin \theta)$ - we have that:
$$
	f(x,y) = e^{-(x^2+y^2)} \implies (f \circ T)(\rho, \theta) = e^{-\rho}
$$
And so, via Theorem 17.2, we have:
$$
	I^2 = \int_{(0, \infty) \times (0, 2\pi)} \rho e^{-\rho^2} d\lambda^2
$$
We again make use of Fubini's on $\rcal^2$ for our nonnegative function to find:
$$
	I^2 = \int_0^{2\pi} \left[\int_0^{\infty} \rho e^{-\rho^2} d\rho\right] d\theta
$$
Now how do we calculate the inner integral? This is just $u$ substitution, aka, change of variable in one dimension. We have $f(x) = \frac{1}{2}e^{-x}$, $T(\rho) = \rho^2$, and $(f \circ T)(\rho) = \frac{1}{2}e^{-\rho^2}$. Now note, the Jacobian of $T$ is $2\rho$. And so, Change of Variable tells us:
$$
	\int_0^{\infty} \rho e^{-\rho^2} d\rho =
	\int_0^{\infty} \frac{e^{-x}}{2} dx = -\frac{e^{-x}}{2}\bigg|_0^{\infty} = 1/2
$$
And so, we have:
$$
	I^2 = \pi \implies I = \sqrt{\pi}
$$
\paragraph{Example 18.4 - Limit of Integrals} We want to find the limit of an integral we had previously discussed:
$$
	\lim_{t \to \infty} \int_0^t \frac{\sin x}{x} dx
$$
First, examine:
$$
	\int_0^t e^{-ux} \sin(x) dx
$$
Recall Euler's formula: $e^{ix} = \cos(x) + i\sin(x)$. So, we have:
$$
	\int e^{ax}e^{ibx} dx = \int e^{(a + ib)x} dx = \frac{e^{(a+ib)x}}{a + ib}
$$
If we expand to a complex and real part, we get:
$$
	= \frac{e^{ax}}{a^2 + b^2}\left[[a\cos(bx) + b\sin(bx)] + i[a\sin(bx) - b\cos(bx)]\right]
$$
Now, recall that integration on the complex plane splits up the imaginary and real part, ie:
$$
	\int f(x) + ig(x) dx = \int f(x) dx + i \int g(x) dx
$$
So, the imaginary part give us:
$$
...
$$
	\int_0^t e^{-ux} \sin(x) dx =
	\left[\frac{e^{-ux}}{1 + u^2}\left[-u\sin(x) - \cos(x)\right]\right]_{x=0}^{x=t} =
	\left[\frac{e^{-ut}}{1 + u^2}\left[-u\sin(t) - \cos(t)\right]\right] +
	\frac{1}{1 + u^2}
$$
$$
	= \frac{1}{1 + u^2}\left[1 - e^{-ut}\left(u\sin(t) + \cos(t)\right)\right]
$$
You can also just verify the above w/o the complex analysis - ie, "pull the answer out of thin air" and then differentiate both sides with respect to $t$ to find:
$$
	e^{-ut} \sin(t) =
	\frac{1}{1 + u^2}\left[ue^{-ut}\left(u\sin(t) + \cos(t)\right) - e^{-ut}\left[u\cos(t) - \sin(t)\right]\right]
$$
$$
	= \frac{1}{1 + u^2}\left[u^2e^{-ut}\sin(t) + e^{-ut}\sin(t)\right] = e^{-ut} \sin(t)
$$
Plus validating equality at a point $t = 0$, in which case both sides are 0. Now, we note:
$$
	\int_0^\infty \left|e^{-ux}\sin(x)\right| du =
	|\sin x| \int_0^\infty e^{-ux} du
$$
Now, we do substitution (1 dimensional change of variables), let $v = -ux$, which has Jacobian $x$, and so:
$$
	\int_0^\infty e^{-ux} du = \frac{1}{x}\int_0^\infty xe^{-ux} du = \frac{1}{x}\int_{-\infty}^0 e^v dv = \frac{1}{x}e^v\bigg|_{-\infty}^0 = \frac{1}{x}
$$
Anyway, all of that is to say:
$$
	\int_0^\infty \left|e^{-ux}\sin(x)\right| du = \frac{|\sin(x)|}{x}
$$
Which tells us:
$$
	\int_0^t \left[\int_0^\infty \left|e^{-ux}\sin(x)\right| du\right] dx = \int_0^t \frac{|\sin(x)|}{x} \leq t < \infty
$$
Where the bounds come from noting $\frac{|\sin(x)|}{x} \leq 1$. And so, we can use Fubini's Theorem on the integration of $e^{-ux} \sin(x)$ over $(0, t) \times (0, \infty)$:
$$
	\int_0^t \frac{\sin(x)}{x} dx =
	\int_0^t \sin(x) \left[\int_0^{\infty} e^{-ux} du \right]dx =
	\int_0^\infty \left[\int_0^t e^{-ux}\sin(x) dx\right] du
$$
Fubini's is used to switch the order of integration. Now, from our above derivation:
$$
	= \int_0^\infty \frac{du}{1 + u^2} - \int_0^\infty \frac{e^{-ut}}{1+u^2}(u\sin(t) + \cos(t)) du
$$
Note the first entry has integral $\pi/2$, which we proved via change of variables in exercise 17.5 (plus a symmetry argument). For the second entry, notice as $t \to \infty$, the magnitude of $(u \sin(t) + \cos(t))$ oscillates between $[-u-1, u+1]$, while the magnitude of $\frac{e^{-ut}}{1+u^2}$ goes to zero. This tells us the value of the integral goes to 0 as well (note, we still have the infinite domain to deal with, but a change of variable $ut = s$ simplifies the whole thing). In total, we have:
$$
	\lim_{t \to \infty} \int_0^t \frac{\sin(x)}{x} dx = \frac{\pi}{2}
$$

\subsubsection{Integration by Parts} After proving the Lebesgue version, compare with \href{https://en.wikipedia.org/wiki/Integration_by_parts}{Integration by Parts Wiki}.
\\\\
...
$$
	...
	a \leq x \leq y \leq b
$$
By Theorem 12.5, we have existence of such measures if $F$ and $G$ are defined on all of $\R$. I think, we can extend such $F$ and $G$ to all of $\R$, just by taking straight lines outside of the intervals equal to the endpoint values $F(a), F(b), G(a), G(b)$, and then restricting our measure to $\rcal \cap [a,b]$. Note, this measure exists by Theorem 12.5, and as $[a,b]$ is $\sigma$ finite and the sub-intervals $I$ are a $\pi$ system such that $\sigma(I) = \rcal \cap [a,b]$ via Theorem 10.1, the measures $\mu$ and $\nu$ are unique via Theorem 10.3.
\\\\
In accordance with the convention of Stieltjes Integrals, write $dF(x)$ and $dG(x)$ in place of $\mu(dx)$ and $\nu(dx)$.

\paragraph{Theorem 18.4 - Integration by Parts} If $F$ and $G$ have no common points of discontinuity in $(a,b]$, then:
$$
	\int_{(a,b]} G(x) dF(x) =
	F(b)G(b) - F(a)G(a) - \int_{(a,b]} F(x) dG(x)
$$
In brief: $\int G dF = \Delta FG - \int F dG$.
\\\\
\textbf{Proof:} Note that replacing $F(x)$ by $F(x) - C$ leaves the above expression unchanged. I mean, assume we prove the statement for $F(x) - C$. Then, we note $\mu$ is the same - see section 12 to note this. And so, if we proved for $F(x) - C$, we would have the following expression is true:
$$
	\int_{(a,b]} G(x) dF(x) = (F(b) - C)G(b) - (F(a) - C)G(a) - \int_{(a,b]} F(x) dG(x) + \int_{(a,b]} C dG(x)
$$
$$
	= F(b)G(b) - F(a)G(a) - C(G(b) - G(a)) + \int_{(a,b]} F(x) dG(x) - C\nu(a,b]
$$
$$
	= F(b)G(b) - F(a)G(a) - \int_{(a,b]} F(x) dG(x) - C(G(b) - G(a)) + C(G(b) - G(a))
$$
$$
	\implies
	\int_{(a,b]} G(x) dF(x) =
	F(b)G(b) - F(a)G(a) - \int_{(a,b]} F(x) dG(x)
$$
So in fact, proving for within a constant of $F$ proves the statement. The same is similarly true if we consider $G(x) - C$. So, wlog, we will prove for $F(x) = F(x) - F(a)$ and $G(x) = G(x) - G(a)$, which implies $F(a) = 0$, $G(a) = 0$, and so:
$$
	\mu(a,x] = F(x) - F(a) = F(x)
	\quad\quad\quad
	\nu(a,x] = G(x) - G(a) = G(x)
$$
If $\pi = \mu \times \nu$ is the product measure in the plane, then by Fubini's Theorem:
$$
	\pi\left[(x,y) : a < y \leq x \leq b\right] =
	\int_{R^2} \1{(x,y) : a < y \leq x \leq b} d\pi
$$
$$
	= \int_\R\int_\R \1{(x,y) : a < y \leq x \leq b} \nu(dy)\mu(dx)
$$
Now, if we look at the inner integral for fixed $x$, it is just $\nu(a,x]$, and so the above equals:
$$
	= \int_\R \1{a < x \leq b}\nu(a,x] \mu(dx) =
	\int_{(a,b]} \nu(a,x] \mu(dx) =
	\int_{(a,b]} G(x) dF(x)
$$
Where the last step makes use of $\nu(a,x] = G(x)$, and notation. Similarly, we have:
$$
	\pi\left[(x,y) : a < x \leq y \leq b\right] =
	\int_{(a,b]} F(y) dG(y)
$$
Note that the two sets in the $\pi$ have their union as:
$$
	S = (a,b] \times (a,b]
$$
Note, it is not disjoint along the diagonal. However, we do note that the diagonal has measure $0$, as we assumed that $F$ and $G$ had no common points of discontinuity:
$$
	\pi\left[(x,y) : a < x = y \leq b\right] =
	\int_{(a,b]} \nu(\{x\}) \mu(dx) = 0
$$
Via the same argument as above. Thus, via inclusion-exclusion, we have:
$$
	\pi(S) = \pi(A) + \pi(B) - \pi(A \cap B) \implies
	\mu(a,b]\nu(a,b] = \pi(S) = \pi(A) + \pi(B)
$$
So, the left sides of our integral equalities add up to $\pi(S) = \mu(a,b]\nu(a,b] = F(b)G(b)$, and so:
$$
	\int_{(a,b]} G(x) dF(x) + \int_{(a,b]} F(y) dG(y) = F(b)G(b)
$$
Which proves the statement for $F(x) - C$ and $G(x) - C$ for the constants chosen, which proves in general for our given assumptions:
$$
	\int_{(a,b]} G(x) dF(x) =
	F(b)G(b) - F(a)G(a) - \int_{(a,b]} F(x) dG(x)
$$
And so we have proven integration by parts. qed.

\paragraph{Classical Integration by Parts} We now derive the classical integration by parts formula. Suppose that $\nu$ has a density $g$ with respect to Lebesgue measure and let $G(x) = c + \int_a^x g(t) dt$. In this case, $\nu(x,y) = G(y) - G(x) = \int_x^y g(t) dt$, which is the same as saying $\nu$ has density $g$. By Theorem 16.11, we have:
$$
	\int_{(a,b]} F(x) dG(x) = \int_{(a,b]} F(x) d\nu(x) = \int_{(a,b]} F(x)g(x) dx
$$
This holds for any $g$ integrable over $(a,b]$ (consider the positive and negative parts, remember densities can only be taken by $g$ positive, so we split it up to find the above holds for all $g$).
\\\\
Now, suppose further that $\mu$ has density $f$ with respect to the Lebesgue measure, and let $F(x) = c' + \int_a^x f(t) dt$. Then, we similarly find:
$$
	\int_{(a,b]} G(x) dF(x) = \int_{(a,b]} G(x) f(x) dx
$$
With our integration by parts formula, we thus have:
$$
	\int_{(a,b]} G(x) f(x) dx =
	F(b)G(b) - F(a)G(a) - \int_{(a,b]} F(x)g(x) dx
$$
This is the classical formula for integration by parts, which normally expresses it as:
$$
	\int_a^b u(x) v'(x) dx =
	u(b)v(b) - u(a)v(a) - \int_a^b u'(x) v(x) dx
$$

\subsubsection{Products of Higher Order} Suppose $(X, \xcal, \mu), (Y, \ycal, \nu)$, and $(Z, \zcal, \eta)$ are three $\sigma$-finite measure spaces. Note, identify $X \times Y \times Z$ with $(X \times Y) \times Z$. We can prove that:
$$
	\xcal \times \ycal \times \zcal = (\xcal \times \ycal) \times \zcal
$$
Where the first is the $\sigma$ field generated by sets of the form $A \times B \times C$, with $A, B, C$ in $\xcal, \ycal, \zcal$ respectively, and the second is the product $\sigma$ field as defined initially. This is done easily, using the same arguments in the first and second chapter, using $\lambda$-systems and using subset inequalities.
\\\\
Similarly, define $\mu \times \nu \times \eta$ on $\xcal \times \ycal \times \zcal$ as $(\mu \times \nu) \times \eta$, where the parenthesis describe how we form the measure as a product of product measures, each of which are well defined by our previous theorems. Clearly:
$$
	(\mu \times \nu \times \eta)(A \times B \times C) =
	((\mu \times \nu) \times \eta)(A \times B \times C) =
	(\mu \times \nu)(A \times B) \cdot \eta(C) =
	\mu(A)\nu(B)\eta(C)
$$
And it is the only measure that satisfies the above. The Fubini's Theorem formulas extend in the obvious way. Of course, we can treat higher order products in the same way, which leads to another construction of Lebesgue measure in $\R^k = \R \times \cdots \times \R$ as the product $\lambda \times \cdots \times \lambda$ on $\rcal^k = \rcal \times \cdots \times \rcal$.

\paragraph{Example 18.5 - Volume of a Sphere} Let $V_k$ be the volume of the sphere of radius $1$ in $\R^k$ - by Theorem 12.2, a sphere in $\R^k$ with radius $r$ has volume $r^kV_k$ (as a sphere of radius $k$ is the sphere of radius $1$ under the linear transformation $T = rI_k$, which has determinant $r^k$). 
\\\\
Let $A$ be the unit sphere in $\R^k$, and let $B = [(x_1, x_2) : x_1^2 + x_2^2 \leq 1]$, and let:
$$
	C(x_1, x_2) = \left[(x_3, \cdots, x_k) : \sum_{i=3}^k x_i^2 \leq 1 - x_1^2 - x_2^2\right]
$$
Clearly, we have:
$$
	\1{A}(x_1, \cdots, x_k) = \1{B}(x_1, x_2)\1{C(x_1, x_2)}(x_3, \cdots, x_k)
$$
We can make use of Fubini's Theorem:
$$
	\int_A dx_1 \cdots dx_k =
	\int_B \left(\int_{C(x_1, x_2)} dx_3 \cdots dx_k\right) dx_1dx_2
$$
Now, what is the value of the inner integral? Well, its coordinates which have a radius of maximum $\sqrt{1 - x_1^2 - x_2^2}$, and so its value is just $(1-x_1^2-x_2^2)^{(k-2)/2}V_{k-2}$. Thus, the above becomes:
$$
	= V_{k-2} \int_B (1-x_1^2-x_2^2)^{(k-2)/2} dx_1dx_2
$$
And for this one, we first use change of variables to polar coordinates:
$$
	= V_{k-2} \int\int_{0 < \theta < 2\pi, 0 < \rho < 1} (1-\rho^2)^{(k-2)/2} \rho d\rho d\theta
$$
Then, we can take out a $2\pi$:
$$
	= 2\pi V_{k-2} \int_{0 < \rho < 1} (1-\rho^2)^{(k-2)/2} \rho d\rho
$$
Then, we do change of variables again, $u = \rho^2 \implies du = 2\rho d\rho$ to find:
$$
	= \pi V_{k-2} \int_0^1 t^{(k-2)/2} dt = \frac{2\pi V_{k-2}}{k}
$$
So now, we have a relation that we can use inductively:
$$
	V_k = \frac{2\pi V_{k-2}}{k}
$$
Where the starting values are $V_0 = 1$ (the identity) and $V_1 = 2$ (the volume of the line from $-1$ to $2$). It follows by induction that for $i = 1, 2, \cdots$:
$$
	V_{2i-1} = \frac{2(2\pi)^{i-1}}{1 \times 3 \times 5 \times \cdots \times (2i - 1)}
	\quad\quad\quad
	V_{2i} = \frac{(2\pi)^i}{2 \times 4 \times \cdots \times (2i)}	
$$

\subsection{Problems}
\subsubsection{18.1 - All Rectangles in $\xcal \times \ycal$ are Measurable Rectangles} Show by Theorem 18.1 that if $A \times B$ is nonempty and lies in $\xcal \times \ycal$, then $A \in \xcal$ and $B \in \ycal$.
\\\\
As $A \times B$ is nonempty, there is an $(x, y) \in A \times B$. By Theorem 18.1:
$$
	A = [x : (x, y) \in A \times B] \in \xcal
	\quad\quad\quad
	B = [y : (x, y) \in A \times B] \in \ycal
$$

\subsubsection{18.2 - Diagonal is not Measurable} Suppose that $X = Y$ is uncountable and $\xcal = \ycal$ consists of the countable and co-countable sets. Show that the diagonal $E = [(x,y) : x = y]$ does not lie in $\xcal \times \ycal$, even though:
$$
	[y : (x,y) \in E] \in \ycal
	\quad\text{ and }\quad
	[x : (x,y) \in E] \in \xcal
...
For all $x$ and $y$.
\\\\
First, note this problem says to use problem 2.9, where we showed that, if $B \in \sigma(\acal)$, then there exists a countable subclass $\acal_B$ of $\acal$ such that $B \in \sigma(\acal_B)$.
\\\\
So, we have that if $E \in \xcal \times \ycal$, then $E$ is in the sigma algebra generated by the measurable rectangles $\acal$. So, $E$ must be in $\sigma(\acal_E)$, where $\acal_E \subseteq \acal$ is countable. We need to find a contradiction.
\\\\
We also note, that $\xcal \times \ycal$ is generated by sets of the form $\{x\} \times X$ and $X \times \{x\}$. Indeed, unions for $A \times X$ and $X \times A$, where $A$ is any countable set, and complements form any $A \times \emptyset$ or $\emptyset \times A$, where $A$ is any co-countable set. Finally, intersections of both types form any measurable rectangle $A \times B$, where $A$ and $B$ are either countable or co-countable.
\\\\
And so, $E$ must be in $\sigma(\acal_E)$, where we first define $S$ countable and $\acal_E$ contains sets of the form $\{x\} \times X$ or $X \times \{x\}$, and $x \in S$. Now, what is $\sigma(\acal_E)$? We will try to derive a contradiction via comparing $E$ and sets that must be contained in $\sigma(\acal_E)$.
\\\\
If $\pcal$ consists of $S^c$ and the singletons in $S$, then $\sigma(\acal_E)$ is the class of unions of sets in the partition $[P_1 \times P_2: P_1, P_2 \in \pcal]$. As for why that is. Note that the set we gave is a $\sigma$-algebra. The union of elements in the class of unions of sets in the partition is still a union of sets in the partition. The union of sets in the partition contains $X \times Y$, as we can take:
$$
	\left(\bigcup_{s \in S} s \times S^c\right) \cup
	\left(\bigcup_{s \in S} S^c \times s\right) \cup
	\left(\bigcup_{s_i, s_j \in S} s_i \times s_j\right) \cup
	\left(S^c \times S^c\right)
$$
Finally, the class contains complements, as first we have:
$$
	A = P_1 \times P_2 \implies
	A^c = \left(P_1^c \times P_2\right) \cup \left(P_1 \times P_2^c\right) \cup \left(P_1^c \times P_2^c\right) =
	\left(P_1 \times P_2^c\right) \cup \left(P_1^c \times P_2\right) 
$$
Then, note that:
$$
	\left(\bigcap P_1 \times P_2\right) =
	\left(\bigcap P_1 \times \bigcap P_2\right)
$$
Which, if nonempty, must be a set of the form $P_1 \times P_2$. And so, we have:
$$
	A = \bigcup P_1 \times P_2 \implies
	A^c = \bigcap (P_1 \times P_2)^c =
	\bigcap \left(P_1 \times P_2^c\right) \cup \left(P_1^c \times P_2\right)
$$
If $(x,y)$ is in the intersection, $(x,y)$ must be in one of the sets of the form:
$$
	\bigcap_{i \in I} (P_{1i}^j \times P_{2i}^k)
$$
Where only one of the $j/k$ is the complement. Above, we noted such a set was of the form $P_1 \times P_2$ if nonempty. And so, $A^c$ is just the union of sets of the form $P_1 \times P_2$. And we indeed have that unions of sets in the partition $[P_1 \times P_2: P_1, P_2 \in \pcal]$ is a $\sigma$ algebra. As $\sigma(\acal_E)$ is the smallest $\sigma$ algebra containing sets of the form $\{x\} \times X$ or $X \times \{x\}$ for $x \in S$, we have that $\sigma(\acal_E)$ must be contained within the unions of sets in the partition.
\\\\
Finally, we note that $E \in \sigma(\acal_E)$ is a contradiction, as $E$ is not of the form of unions of sets in the partition $[P_1 \times P_2: P_1, P_2 \in \pcal]$. Note, if $P_1$ or $P_2$ equaled $S^c$, we would contain an element not in the diagonal. However, if $E$ is the union of $P_1 \times P_2$ where $P_1 = P_2 = \{x\}$, note that $E$ would not contain the full diagonal.
\\\\
And so, we have that $E \in \sigma(\acal)$, where $\acal$ is the measurable rectangles, implies $E \in \sigma(\acal_E)$, which leads to a contradiction. And so, our assumption that $E \in \sigma(\acal)$ is incorrect. qed.

\subsubsection{18.3 Product of Complete Spaces Not Necessarily Complete} Let $(X, \xcal, \mu) = (Y, \ycal, \nu)$ be the completion of $(\R, \rcal, \lambda)$. Show that $(X \times Y, \xcal \times \ycal, \mu \times \nu)$ is not complete.
\\\\
First, recall that $(\Omega, \fcal, P)$ is complete if $A \subset B, B \in \fcal$ and $P(B) = 0$ implies $P(A) = 0$. At the end of section 3, we proved that any probability space can be enlarged and the measure can be extended to get a complete space. This was for only probability measures, but the argument can be extended to general measurable spaces. This was done in problem 10.5. $A \subseteq B$, $B \in \fcal$, and $\mu(B) = 0$ imply both $A \in \fcal$ and $\mu(A) = 0$.
\\\\
The problem notes to also make use of problem 18.1, done above, where we proved if $A \times B \in \xcal \times \ycal$ is nonnempty then $A \in \xcal$ and $B \in \ycal$.
\\\\
First, I want to consider whether or not $(\R, \rcal, \lambda)$ is complete. First, note that $((0,1], \rcal, \lambda)$ is not complete. Consider the Cantor Set - which is the closure of iteratively removing open middle thirds. It can also be described as the set of numbers in $(0,1]$ whose nonterminating expansions in base 3 do not contain the digit 1. We proved in problem 3.14 that the Cantor Set $C$ is Lebesgue measurable (ie, satisfies $\lambda^*(C \cap E) + \lambda^*(C^c \cap E) = \lambda^*(E)$), and so is in the set $\lcal$ of $\lambda^*$ measurable sets. We were able to directly conclude this from the fact that $\lambda^*(C) = 0$. Now, note that via monotonicity, every subset of $C$, call it $C'$, also satisfies $\lambda^*(C') = 0$, and is similarly Lebesgue measurable.
\\\\
Finally, note that in our construction, the borel set $\bcal \subseteq \lcal$, the Lebesgue measurable sets. As $|\bcal| < |2^C|$ (noted in problem 3.14), this implied that $2^C$ must contain a Lebesgue measure $0$ set that is not a Borel set. Finally, we note that $C$ itself is a Borel set, as it is \textit{closed}. And so, we have found a set of measure $0$, $C \in \bcal$, which has subsets that are not Borel sets. And so, $\bcal$ is not complete. Given that $C$ can be similarly taken as a set in $\rcal$, we have that $(\R, \rcal, \lambda)$ is not complete.
\\\\
Anyway. Based on the hint, we consider a set $\{a\} \times C \in \xcal \times \ycal$. We must have that:
$$
	(\mu \times \nu)(\{a\} \times C) = 0
$$
We will show that this set proves that $\xcal \times \ycal$ is not complete. Indeed, assume that $\xcal \times \ycal$ is complete. Then every subset of $\{a\} \times C$ is within $\xcal \times \ycal$ as well. This implies that every subset of $C$ is within $\ycal$. However, that is not a contradiction, as the completion of the borel sets would necessarily contain all of the subsets of the cantor set.
\\\\
Instead, I think we need to show that, for some set $B$ outside of the completion of $\rcal$, ie not contained in either $\rcal$ or its completion, that $\{x\} \times B \in \xcal \times \ycal$, and so $B \in \ycal$, which would be a contradiction. So, I think maybe use assumed completeness of $\xcal \times \ycal$ to prove that $\{x\} \times B \in \xcal \times \ycal$.
\\\\
Note, we can take the closure of $B$, and have that $B \subseteq \overline{B}$. We can also find that $B$ is bounded - if $B$ is unbounded, we must have that $B \cap (0,1]$ is bounded, but also outside of the closure of $\rcal$. More concretely, just let $B$ be a vitali set on $(0,1]$. As $B$ is bounded, we have $\nu(\overline{B}) = M < \infty$, and so we can conclude $(\mu \times \nu)(\{x\} \times \overline{B}) = 0 \times M = 0$ (actually, at the start of chapter 15, the conventions of infinite products says $0 \cdot \infty = 0$, so we might not need $B$ is bounded just closed). Note that as both $\{x\}$ and $\overline{B}$ are measurable, $\{x\} \times \overline{B} \in \xcal \times \ycal$. If $\xcal \times \ycal$ is complete, we have that $\{x\} \times B \in \xcal \times \ycal$, which implies $B \in \ycal$ which is the completion of $\rcal$, which is a contradiction. And so, we must have that $\xcal \times \ycal$ is not complete. qed.

\subsubsection{18.4 $\sigma$ finite is necessary in Theorem 18.2} Let $\mu$ be the Lebesgue measure on the line, and $\nu$ the counting measure on the line. Note that $\nu$ is not $\sigma$ finite (as a countable number of finite sets with finite measure cannot cover an uncountable space).
\\\\
Take $E = \left[(x,y) : x = y\right]$. Examine:
$$
	\pi'(E) = \int_X \nu\left[y : (x,y) \in E\right] \mu(dx) \quad\quad\quad
	\pi''(E) = \int_Y \mu\left[x : (x,y) \in E\right] \nu(dy)
$$
If both measures are countable, we would have $\pi'(E) = \pi''(E)$ for $E \in \xcal \times \ycal$. Note, $E \in \xcal \times \ycal$. This can be determined via the following. First, note that our measure spaces are $(\R, \rcal, \mu)$ and $(\R, \rcal, \nu)$ (note, the counting measure is well defined on the power sets, but we don't need that many sets). Then, note by the discussion at the start of the chapter, $\rcal \times \rcal = \rcal^2$. Then, Theorem 13.2 tells us that $f(x,y) = x - y$ is measurable as it is continuous. And so, $f^{-1}(\{0\}) \in \rcal^2$, which tells us $E = f^{-1}(\{0\})$ is measurable.
\\\\
And so, as $E \in \xcal \times \ycal$, Theorem 18.2, without the $\sigma$-finite assumption, would imply that $\pi'(E) = \pi''(E)$. We examine that now. First, we look at:
$$
	\nu\left[y : (x,y) \in E\right] = \nu\left[x\right] = 1 \implies
	\pi'(E) = \int_X 1 \mu(dx) = \infty
$$ 
$$
	\mu\left[x : (x,y) \in E\right] = \mu\left[y\right] = 0 \implies
	\pi''(E) = \int_Y 0 \nu(dy) = 0
$$
And so, in conclusion, we  must have the $\sigma$-finiteness assumption for Theorem 18.2, Fubini's Theorem, to hold.

\subsubsection{18.6 Area Under the Curve} Suppose that $f$ is nonnegative on a $\sigma$-finite measure space $(\Omega, \fcal, \mu)$. Ie, $f: \Omega \to \R$ is nonnegative. Also, the implication is that $f$ is measurable, as we are taking its integral value. Show that:
$$
	\int_\Omega f d\mu =
	(\mu \times \lambda)\left[(\omega, y) \in \Omega \times \R : 0 \leq y \leq f(\omega)\right]
$$
First prove that the set on the right is measurable. This will give the "area under the curve" intuition of the integral, as the integral value is the measure of all the values underneath the curve.
\\\\
Actually, I first prove the equality, under the assumption that the set is measurable, as it is really easy to do. We have as the Borel line is $\sigma$ finite, we can use Fubini's Theorem to find:
$$
	(\mu \times \lambda)\left[(\omega, y) \in \Omega \times \R : 0 \leq y \leq f(\omega)\right]
$$
$$
	=
	\int_{\Omega \times \R} \1{(\omega, y) \in \Omega \times \R : 0 \leq y \leq f(\omega)} (\mu \times \lambda)(d(x,y))
$$
$$
	=
	\int_\Omega \left[\int_\R \1{y \in \R : 0 \leq y \leq f(\omega)} \lambda(dy)\right] \mu(dx)
$$
$$
	\int_\Omega \lambda([0, f(\omega)]) \mu(dx) =
	\int_\Omega f(\omega) \mu(dx) =
	\int f d\mu
$$
So, this only follows is the set on the right is measurable. So, how to prove that? Note that:
$$
	h(\omega, y): \Omega \times \R \to \R
	\quad\quad\quad
	h(\omega, y) = f(\omega) - y
$$
$h$ is measurable. Clearly, $F(\omega, y) = f(\omega)$ is measurable, as $F^{-1}(B) = f^{-1}(B) \times \R$, where $B \in \rcal$, $f^{-1}(B) \in \fcal$, and measurable rectangles are in the product sigma algebra. Similarly, $G(\omega, y) = y$ is measurable. And so, $h = F - G$ is measurable via Theorem 13.3.
\\\\
Now, note:
$$
	h^{-1}([0, \infty]) = \left[(\omega, y) \in \Omega \times \R : y \leq f(\omega)\right]
$$
As $y \leq f(\omega)$ iff $f(\omega) - y \geq 0$. And so, the set above is measurable. Thus, we have:
$$
	\left[(\omega, y) \in \Omega \times \R : 0 \leq y \leq f(\omega)\right] =
	\left[(\omega, y) \in \Omega \times \R : y \leq f(\omega)\right] \cap
	\left[(\omega, y) \in \Omega \times \R : 0 \leq y\right]
$$
Is measurable, being the intersection of two measurable sets (the rightmost set being measurable as it is a measurable rectangle, $\Omega \times [0, \infty]$). Thus, our area under curve integral is justified. qed.

\subsubsection{18.8 Cavalieri's Principle} Suppose that $\nu[y: (x, y) \in E] = \nu[y: (x,y) \in F]$ for all $x$. Then show that:
$$
	(\mu \times \nu)(E) =
	(\mu \times \nu)(F)
$$
This is a general version of Cavalieri's principle.
\\\\
Just push it to the double integral. We have:
$$
	(\mu \times \nu)(E) =
	\int_{X \times Y} \1{E} \pi(d(x,y)) =
	\int_X \left[\int_Y \1{y : (x,y) \in E} \nu(dy)\right] \mu(dx)
$$
$$
	= 
	\int_X \nu[y: (x, y) \in E] \mu(dx) =
...
$$
$$
	= \int_X \left[\int_Y \1{y : (x,y) \in F} \nu(dy)\right] \mu(dx)
	= \int_{X \times Y} \1{F} \pi(d(x,y))
	= (\mu \times \nu)(F)
$$
Note, equality of the inner terms comes from our condition. The rest is just Fubini's Theorem. Note, the assumption that $\mu$ and $\nu$ are $\sigma$ finite is implicit in the question, as the existence of the product measure needs them to be $\sigma$ finite. qed.

\subsubsection{18.13 Distribution Function Subtraction Integral} 
Prove for distribution functions $F$ that:
$$
	\int_{-\infty}^\infty (F(x + c) - F(x)) dx = c
$$
Recall, a Random Variable is a measurable real function $X$ on a probability space $(\Omega, \fcal, P)$, where $P$ is a finite measure as $P(\Omega) = 1$. The \textit{distribution} of $X$ is the probability measure $\mu$ on $(\R^1, \rcal^1)$ defined by:
$$
	\mu(A) = P[X \in A] \quad\quad\quad
	\mu = PX^{-1}
$$
The \textit{distribution function of $X$} is defined by:
$$
	F(x) = \mu(-\infty, x]
$$
So, I will assume the existence of such a measure $\mu$, which is \textit{finite} (and thus $\sigma$ finite). As the lebesgue measure $\lambda$ is $\sigma$ finite, Theorem 18.2. gives us the existence of a measure $\lambda \times \mu$ on $\R \times \R$. This will be needed later.
\\\\
We thus have:
$$
	\int_\R \1{y \in (-\infty,x]} \mu(dy) = \mu(-\infty, x] = F(x)
$$
We can substitute that into our integral:
$$
	\int_{-\infty}^\infty (F(x + c) - F(x)) dx =
	\int_\R \left[\int_\R \1{y \in (x, x+c)} \mu(dy)\right] \lambda(dx)
$$
So now, note we can apply Fubini's Theorem, as our function is nonnegative. And so, we can switch the order of integration. Note, our function was $f(x,y) = \1{y \in (x, x+c)}$. If we fix $y$, then it becomes $f(x,y) = \1{x \in (y - c, y)}$, as $x = y - c \implies y = x + c$, and $x = y \implies y = x$. 
$$
	= \int_\R \left[\int_\R \1{x \in (y - c, y)} \lambda(dx)\right] \mu(dy)
$$
In this case, we we know the Lebesgue measure, the integral becomes easy to simplify:
$$
	= \int_\R c \mu(dy) = c\mu(\R) = c
$$
As $\mu(\R) = P(X \in \R) = P(\Omega) = 1$. Thus, we have found the expression. qed.


\subsubsection{18.13 Distribution Function Integral across Its Own Measure} Prove for continuous distribution functions that:
$$
	\int_{-\infty}^\infty F(x) dF(x) = 1/2
$$
First, what is this notation? It is the Stieltjes Integral, where $dF(x)$ is the same as $\mu(dx)$, where $\mu$ is the measure such that $\mu(A) = \Delta_A F$. In this case, I will assume that $F$ is derived from a random variable $X$, and $\mu(a,b] = F(b) - F(a)$. In Problem 14.3, we proved that $F$ is continuous, then:
$$
	P[F(X) \leq u] = u
$$
I don't know if we will need that, but we might. We will use Fubini's Theorem again - we have:
$$
	\int_{-\infty}^\infty F(x) dF(x) = 
	\int_\R \mu(-\infty,x] \mu(dx) =
	\int_\R \left[\int_\R \1{y \in (-\infty,x]} \mu(dy)\right] \mu(dx)
$$
Via Fubini's, expanding to a double integral gives us:
$$
	\int_{\R \times \R} \1{(x,y) | y \leq x} (\mu \times \mu)(d(x,y))
$$
Now note, we must have via symmetry (ie, just moving $x$ and $y$ around symbolically):
$$
	= \int_{\R \times \R} \1{(x,y) | x \leq y} (\mu \times \mu)(d(x,y))
$$
Let $z$ denote the common integral value. So, we have:
$$
	z + z =
	\int_{\R \times \R} \left[\1{(x, y) \in \R \times \R} +
	\1{x = y}\right] (\mu \times \mu)(d(x,y))
$$
Note, the diagonal is double counted. However, as $F$ is continuous, we have $\mu(\{x\}) = 0$, which implies $(\mu \times \mu)(\{(x,x)\}) = 0$. This can eb concluded again via Fubini's:
$$
	\int_\R \left[\int_\R \1{y = x} \mu(dy)\right] \mu(dx) =
	\int_\R 0 \mu(dx) = 0
$$
And so, as the remaining term is just $\1{\R}$, we have:
$$
	z + z = 1 \implies z = 1/2 \implies
	\int_{-\infty}^\infty F(x) dF(x) = 1/2
$$
Given that $F$ is a continuous distribution. qed.

\subsubsection{18.15 Integration By Parts Property} Suppose that a number $f_n$ is defined for each $n \geq n_0$ and put $F(x) = \sum_{n_0 \leq n \leq x} f_n$. Deduce from:
$$
	\int_{(a,b]} G(x)dF(x) =
...
That:
$$
	\sum_{n_0 \leq n \leq x} G(n)f_n = F(x)G(x) -
	\int_{n_0}^x F(t)g(t) dt
$$
If $G(y) - G(x) = \int_x^y g(t) dt$, which will hold if $G$ has continuous derivative $g$. First assume that the $f_n$ are nonnegative.
\\\\
So, we assume the $f_n$ are nonnegative. $F(x)$ is just a sum of the $f_n$ with index $n$ between $n_0$ and $x$. Note that $F(x)$ is nondecreasing and right continuous on the interval from $n_0$ to $x$. Indeed, right continuous, as if $x = n$, then $F(x)$ includes the $f_n$ value - and approaching $x$ from the left will be some value $F(x) - f_n$, whereas approaching from the right will ultimately match $F(x)$. Clearly $F(x)$ is nondecreasing. We have $\mu(x,y] = F(y) - F(x)$, and $\mu$ is the measure we take the integral with respect to. Examine:
$$
	\int_{[n_0,x]} G(t)dF(t) =
	\int_{[n_0,x]} G(t) \mu(dt)
$$
We will first try and solve it w/o integration by parts. Note that $\mu$ is a measure on $(\R, \rcal)$, so if we go to the definition, we can partition $[n_0,x]$ by $n_1 < \cdots < n_k$, where $n_i$ corresponds to an $f_{n_i}$. If we take the infimum definition of the integral, it equals:
$$
	\sum_{i=1}^k G(n_{i-1})\mu(n_{i-1},n_i] =
	\sum_{i=1}^k G(n_{i-1})(F(n_i) - F(n_{i-1})) =
	G(n_{k-1})F(n_k) - G(n_0)F(n_0)
$$
Via a telescoping sum. But that isn't exactly the LHS I want to see. Actually, we can instead note that $F(n_i) - F(n_{i-1}) = f_n$. In which case, we get:
$$
	\sum_{i=1}^k G(n_{i-1})f_{n_i}
$$
Which is closer to what we want. Anyway, I'm not getting the exact derivation, but it is something like what I said above. Infinite cases can be taken care of by a limit. Actually, if we think about supremum - think about adjacent $n_i < n_{i+1}$. Ie, there is no $n$ between them. If we think about the integral on this interval, we can decompose it again, like $n_i = t_1 < \cdots < t_k = n_{i+1}$ to find:
$$
	\int_{[n_i,n_{i+1}]} G(t) \mu(dt) =
	\sum_{i=0}^{k-1} G(i)\mu(t_i, t_{i+1}] =
	\sum_{i=0}^{k-1} G(i)(F(t_{i+1}) - F(t_i))
$$
But note, rot $i < k-1$, $F(t_{i+1}) = F(t_i)$, and so the integral becomes:
$$
	G(t_{k-1})(F(n_{i+1})-F(n_i))
$$
Where we note, the $G(t_{k-1}) \to G(n_{i+1})$. So, if we make all of that rigorous, ie split the integral across $[n_0,x]$ into intervals $(n_i,n_{i+1}]$, and individually calculate each subintegral, we can conclude that:
$$
	\int_{[n_0,x]} G(t)dF(t) = \sum_{n_0 < n \leq x} G(n)f_n
$$
Note the inequalities. Now, as for the other side of the equation, use integration by parts to find:
$$
	\int_{[n_0,x]} G(t)dF(t) = F(x)G(x) - F(n_0)G(n_0) -
	\int_{n_0}^x F(t)g(t) dt
$$
Where the final integral comes from the argument in the chapter. Now, we just bring the subtracted term to the other side to find:
$$
	\sum_{n_0 \leq n \leq x} G(n)f_n = F(x)G(x) -
	\int_{n_0}^x F(t)g(t) dt
$$

\subsubsection{18.20 Fubinis Extension on Probability Spaces} Suppose that $\mu$ is a probability measure on $(X, \xcal)$ and that, for each $x \in X$, $\nu_x$ is a probability measure on $(Y, \ycal)$. Suppose further that, for each $B$ in $\ycal$, $\nu_x(B)$ is, as a function of $x$, measurable $\xcal$. Regard the $\mu(A)$ as initial probabilities, and the $\nu_x(B)$ as transition probabilities.
\\\\
Recall, transition probabilities are $p_{ij}$, the probability to go from state $i$ to state $j$. $\mu(A)$ would give the probability of being in some state $x \in A$. If we let $m(x) = \nu_x(B)$, we have that $m$ is measurable. So $m(A)$ would give us what? The probability from going from $A$ to $B$? I guess, but $X$ and $Y$ are not necessarily the same state. But they could be - this is just more general.

\begin{enumerate}
	\item Show that, if $E \in \xcal \times \ycal$, then $\nu_x[y : (x, y) \in E]$ is measurable $\xcal$.
	\\\\
	So, for fixed $B$, $m(x) = \nu_x(B)$ is measurable. However, here, we have $m(x) = \nu_x(E_x)$, where $E_x = \{y : (x, y) \in E\}$. By Theorem 18.1, $E_x \in \ycal$, so this is well defined. Is this measurable?
	\\\\
	First consider $E = A \times B$, a measurable rectangle. Then, $E_x = B$, or $\emptyset$, based on whether $x \in A$. In this case, $m(x)$ is measurable, as for $R \in \rcal$:
	$$
		m^{-1}(R) = \left\{x : x \in A \text{ and } \nu_x[B] \in R\right\}
		\cup
		\left\{x : x \not\in A \text{ and } \nu_x[\emptyset] \in R\right\}
	$$
	$$
		= (A \cap m_1^{-1}(R)) \cup (A^c \cap m_2^{-1}(R)) 
	$$
	Where $m_1(x) = \nu_x(B)$ and $m_2(x) = \nu_x(\emptyset)$ are both measurable, and so $m$ is measurable.
	\\\\
	So, the class of $E$ for which $\nu_x[y : (x, y) \in E]$ contains the measurable rectangles. If this class is a $\lambda$-system, this would imply for all $E \in \xcal \times \ycal$, the function is measurable. Well, assume that $m_E$ is measurable. Does this imply $m_{E^c}$ is measurable? We have:
	$$
		m_E^{-1}(R) =
		\left\{x : \nu_x(E_x) \in R\right\}
		\quad\quad
		m_{E^c}^{-1}(R) =
		\left\{x : \nu_x(E^c_x) \in R\right\}
	$$
	We need to prove the right set is measurable. We have the left set is measurable by assumption. Compare $E_x$ and $E^c_x$, for fixed $x$. Note that as $E \cup E^c = X \times Y$, we must have $E_x \cup E^c_x = Y$. And $\nu_x(Y) = 1$, via being a probability measure. We have:
	$$
		\left\{x : \nu_x(Y) \in 2R\right\}
	$$
	Is a measurable set, by assumption (non-singular linear images are measurable in $\rcal$, Theorem 12.2). And so, I think we must have:
	$$
		\left\{x : \nu_x(E_x) \in R\right\} \cup \left\{x : \nu_x(E^c_x) \in R\right\} =
		\left\{x : \nu_x(E_x) + \nu_x(E^c_x) \in 2R\right\} 
	$$
	$$
		\left\{x : \nu_x(Y) \in 2R\right\}
	$$
	And so, we have $m_E^{-1}(R)$ and $m_E^{-1}(R) \cup m_{E^c}^{-1}(R)$ are measurable. I don't think that is enough. We also need the sets are disjoint.
	\\\\
	First start with disjoint unions, $m_{A_1}, m_{A_2}, \cdots$ are all measurable. Do we have $m_{A_1 \cup \cdots}$ is measurable? I think, it is clear:
	$$
		m_{A_1 \cup \cdots}^{-1}(R) =
		\left\{x : \nu_x((A_1 \cup \cdots)_x) \in R\right\} =
		\left\{x : \nu_x((A_1)_x) + \nu_x((A_2)_x) + \cdots \in R \right\}
	$$
	We can split the sum via being disjoint, and for each $x$, $\nu_x$ is a measure. Now, note that if that sum is in $R$, we have:
	$$
		m_{A_1}(x) + m_{A_2}(x) + \cdots \in R \implies
		m_{A_1 \cup \cdots}^{-1}(R) = \left[\sum_n m_{A_i}\right]^{-1}(R)
	$$
	And we finally note that the countable sum of nonnegative measurable functions is still measurable (Theorem 13.4). So, the final property to prove $\lambda$ system will be closed under proper differences. Note, I don't want to go through it, but the argument is the same as the one I laid out for the existence of the product measure between $\sigma$ finite spaces. And so, $m_E(x) = \nu_x[y : (x, y) \in E]$ is measurable $X$ for all $E \in \xcal \times \ycal$.
	
	\item Show that $\pi(E) = \int_X \nu_x[y: (x, y) \in E] \mu(dx)$ defines a probability measure on $\xcal \times \ycal$. If $\nu_x = \nu$ does not depend on $x$, this is just our product measure statement.
	\\\\
	Note, as $m(x) = \nu_x(E_x)$ is measurable, the integral is well defined. Also, the integral exists, as $m_x$ is nonnegative. As for probability measure, it is clear that $\pi(\emptyset) = 0$. The last part we need is just countable additivity. Assume $E_1, E_2, \cdots$ are disjoint sets in $\xcal \times \ycal$. We find:
	$$
		\pi\left(\bigcup_i E_i\right) =
		\int_X \nu_x\left[y: (x, y) \in \bigcup_i E_i\right] \mu(dx)
	$$
	Consider $x$ fixed. Note that as the $E_i$ are disjoint, we have:
	$$
		\left\{y: (x, y) \in \bigcup_i E_i\right\} =
		\bigcup_i \left\{y: (x, y) \in E_i\right\}
	$$
	This is because each $(x,y)$ in the union of $E_i$ must be contained in only one $E_i$ via disjointness. For this fixed $x$, we thus have via $\nu_x$ being a probability measure:
	$$
		\nu_x\left[y: (x, y) \in \bigcup_i E_i\right] =
		\sum_i \nu_x\left[y: (x, y) \in E_i\right]
	$$
	This is true for every $x$, as each $\nu_x$ is a probability measure, and so our function $m(x)$ can be re-written as:
	$$
		m(x) = \nu_x\left[y: (x, y) \in \bigcup_i E_i\right] =
		\sum_i \nu_x\left[y: (x, y) \in E_i\right]
	$$
	...
	$$
		\pi\left(\bigcup_i E_i\right) =
		\int_X \sum_i \nu_x\left[y: (x, y) \in E_i\right] \mu(dx)
	$$
	As each term in the sum above is $m_i(x)$, a nonnegative measurable function, we can make use of Theorem 16.2, Monotone Convergence Theorem to find:
	$$
		= \sum_i \int_X \nu_x\left[y: (x, y) \in E_i\right] \mu(dx) =
		\sum_i \pi(E_i)
	$$
	And so $\pi$ is indeed a measure. Finally, for probability measure, we note:
	$$
		\pi(X \times Y) =
		\int_X \nu_x[y: (x, y) \in X \times Y] \mu(dx) =
		\int_X 1 \mu(dx) = 1
	$$
	As for all $x$, the $\nu_x$ term equals $1$ as $\nu_x$ is a probability measure.
	
	\item Show that if $f$ is measurable $\xcal \times \ycal$ and nonnegative, then:
	$$
		\int_Y f(x,y) \nu_x(dy)
	$$
	Is measurable $\xcal$. Show further that:
	$$
		\int_{X \times Y} f(x,y)\pi(d(x,y)) =
		\int_X \left[\int_Y f(x,y) \nu_x(dy)\right] \mu(dx)
	$$
	...
	\\\\
	Consider $\int_Y f(x,y) \nu_x(dy)$ for fixed $x$. Note, as $f$ is measurable, Theorem 18.2.ii implies that $f(x, \cdot)$ is measurable $Y$. As $f$ is nonnegative, and $\nu_x$ is a probability measure on $Y$, the integral exists, and so the value is well defined for fixed $x$. We want to show that as a function of $x$, it is measurable.
	\\\\
	\textbf{Indicators} Consider first $f = \1{(x,y) \in E}$. In this case, note that for fixed $x$:
	$$
		\int_Y f(x,y) \nu_x(dy) =
		\int_Y \1{y \in E_x} \nu_x(dy) = \nu_x(E_x)
	$$
	We noted in part 1. that $\nu_x(E_x)$ is measurable $\xcal$. So, the integral is measurable $\xcal$ in this case, and:
	$$
		\int_X \left[\int_Y f(x,y) \nu_x(dy)\right] \mu(dx) =
		\int_X \nu_x[y: (x, y) \in E] \mu(dx) = \pi(E)
	$$
	Similarly, we have by definition:
	$$
		\int_{X \times Y} f(x,y)\pi(d(x,y)) =
		\int_{x \times Y} \1{E} \pi(d(x,y)) = \pi(E)
	$$
	So we have the extended Fubini's Theorem is true in the indicator case.
	\\\\
	\textbf{Simple Functions} Now, consider $f = \sum_i a_i\1{E_i}$, a nonnegative simple function. Note:
	$$
		\int_{X \times Y} f(x,y)\pi(d(x,y)) = \sum_i a_i\pi(E_i) 
	$$
	Via linearity of the integral. Now note for fixed $x$:
	$$
		\int_Y f(x,y) \nu_x(dy) = \int_Y \sum_{i=1} a_i \1{y : (x,y) \in E_i} \nu_x(dy)
		= \sum_{i=1} a_i\nu_x(E_{ix})
	$$
	Via the integral of a nonnegative simple function (Theorem 15.1.i). Thus linear sum of $\xcal$ measurable functions is measurable, and so:
	$$
		\int_X \left[\int_Y f(x,y) \nu_x(dy)\right] \mu(dx) =
		\int_X \sum_i a_i\nu_x[y: (x, y) \in E_i] \mu(dx) = \sum_i a_i\pi(E_i)
	$$
	Again via linearity. And so, we have proven Fubini's Theorem in the nonnegative simple function case.
	\\\\
	\textbf{Nonnegative Measurable Functions} A general measurable nonnegative $f$ is the limit from below of simple functions. Via MCT, we have:
	$$
		\int_{X \times Y} f(x,y)\pi(d(x,y)) = \lim_i \int_{X \times Y} f_i(x,y)\pi(d(x,y)) =
		\lim_i \int_X \left[\int_Y f_i(x,y) \nu_x(dy)\right] \mu(dx)
	$$
	Now note $f_i(x, \cdot) \uparrow f(x, \cdot)$, and so our integral exists for fixed $x$:
	$$
		\int_Y f(x,y) \nu_x(dy) = \lim_i \int_Y f_i(x,y) \nu_x(dy)
	$$
	It is also a measurable function in $x$, as Theorem 13.4 gives us that the limit of measurable functions is measurable as well. Now note:
	$$
		\int_Y f_i(x,y) \nu_x(dy) \uparrow \int_Y f(x,y) \nu_x(dy)
	$$
	Via monotonicity of the integral. So, as the inside is an increasing limit of measurable functions, we have:
	$$
		\lim_i \int_X \left[\int_Y f_i(x,y) \nu_x(dy)\right] \mu(dx) =
		\int_X \lim_i \left[\int_Y f_i(x,y) \nu_x(dy)\right] \mu(dx)
	$$
	$$
		=
		\int_X \left[\int_Y f(x,y) \nu_x(dy)\right] \mu(dx)
	$$
	\textbf{General Measurable $f$} So, the final part of the problem is to consider $f$'s that may be negative. I think we can just apply the same analysis as in Fubini's case, considering the set of $x$ the $A_0$ such that:
	$$
		\int_Y |f(x,y)| \nu_x(dy) < \infty
	$$
	If $\mu(X - A_0) = 0$, then everything should still apply.
	...
	First, note that:
	$$
		\pi(X \times B) = \int_X \nu_x\left[y : (x,y) \in X \times B\right] \mu(dx) =
		\int_X \nu_x(B) \mu(dx) = \nu(B)
	$$
	Now note, by Fubini's Theorem in the previous part, we have:
	$$
		\int_X\left[\int_Y f(y) \nu_x(dy)\right]\mu(dx) =
		\int_{X \times Y} f(y) \pi(d(x,y))
	$$
	Now note. We have, via the definition of the integral:
	$$
		\int_{X \times Y} f(y) \pi(d(x,y)) = \sup \sum_i \left[\inf_{(x,y) \in A_i} f(y)\right] \pi(A_i)
	$$
	Where $A_i$ is any finite decomposition of $X \times Y$ into measurable sets. We also note:
	$$
		\int_Y f(y) \nu(dy) =
		\sup \sum_j \left[\inf_{y \in B_j} f(y)\right] \nu(B_j)
	$$
	$$
		=
		\sup \sum_j \left[\inf_{(x,y) \in X \times B_j} f(y)\right] \pi(X \times B_j) \leq
		\int_{X \times Y} f(y) \pi(d(x,y))
	$$
	To show the inequality in the other direction, we need to bound each term on the $X \times Y$ supremum by a term in the $Y$ supremum. Consider:
	$$
		\int_{X \times Y} f(y) \pi(d(x,y)) = \sup \sum_i \left[\inf_{(x,y) \in A_i} f(y)\right] \pi(A_i)
	$$
	Consider a fixed finite decomposition $A_1, \cdots, A_n$. First note, Theorem 11.4 applies to $\pi$. The measurable rectangles are a semiring $\acal$ such that $\xcal \times \ycal = \sigma(\acal)$. Further, $\pi$ is finite, and so $\sigma$ finite on $\acal$. Thus, by Theorem 11.4.ii, we have that each $A_i$ can be approximated by (as we must have $\pi(A_i) < 1$) a finite $C_{i1} \times D_{i1}, \cdots, C_{in_i} \times D_{in_i}$ where:
	$$
		\pi\left(A_i \triangle \bigcup_{k=1}^{n_i} C_{ik} \times D_{ik}\right) < \epsilon
	$$
	We can further refine these measurable rectangles on any overlaps (as there are only a finite number of rectangles, take intersections and proper differences), to find that there is a partition of $X \times Y$ into measurable rectangles such that:
	$$
		\sum_i \left[\inf_{(x,y) \in A_i} f(y)\right] \pi(A_i) \leq
		\sum_i \left[\inf_{(x,y) \in C_i \times D_i} f(y)\right] \pi(C_i \times D_i)
	$$
	Now, we could refine the partition even further such that it would be of the form $\{C_1, \cdots, C_p\} \times \{D_1, \cdots, D_q\}$. In which case, we find that the above equals:
	$$
		=
		\sum_{j=1}^q \sum_{i=1}^p \left[\inf_{(x,y) \in C_i \times D_j} f(y)\right] \pi(C_i \times D_j)
	$$
	Now, note that the infimum value is the same for $X \times D_j$ and $C_i \times D_j$, and so the above equals:
	$$
		= \sum_{j=1}^q \sum_{i=1}^p \left[\inf_{(x,y) \in X \times D_j} f(y)\right] \pi(C_i \times D_j)
	$$
	$$
		= \sum_{j=1}^q \left[\inf_{(x,y) \in X \times D_j} f(y)\right] \sum_{i=1}^p \pi(C_i \times D_j)
	$$
	$$
		= \sum_{j=1}^q \left[\inf_{(x,y) \in X \times D_j} f(y)\right] \pi(X \times D_j)
	$$
	$$
		= \sum_{j=1}^q \left[\inf_{y \in D_j} f(y)\right] \nu(D_j) \leq \int_Y f(y) \nu(dy)
	$$
	Note, the only part of this that is non rigorous was our first inequality that:
	$$
		\sum_i \left[\inf_{(x,y) \in A_i} f(y)\right] \pi(A_i) \leq
		\sum_i \left[\inf_{(x,y) \in C_i \times D_i} f(y)\right] \pi(C_i \times D_i)
	$$
	However, to make it more rigorous. The issue is the rectangle overlaps - however, note that each overlap must be contained in the $n$ differences, each having a measure of $\epsilon$. So, we can bound this difference to be arbitrarily small, of total measure $n\epsilon$. This can be effectively brought to zero. So, for each finite decomposition $A_1, \cdots, A_n$, we can associate with it a decomposition of $Y$ for which the infimum sum is only $\epsilon$ larger. So, for each $\epsilon$, we find:
	$$
		\int_X\left[\int_Y f(y) \nu_x(dy)\right]\mu(dx) \leq \int_Y f(y) \nu(dy) + \epsilon
	$$
	Combined with:
	$$
		\int_Y f(y) \nu(dy) \leq \int_X\left[\int_Y f(y) \nu_x(dy)\right]\mu(dx)
	$$
	We must have that the two values are equal. qed.
		
\end{enumerate}

\section{Section 32 - The Radon-Nikodym Theorem}
\subsection{Notes}
If $f$ is a nonnegative function on a measure space $(\Omega, \fcal, \mu)$, then $\nu(A) = \int_A f d\mu$ defines another measure on $\fcal$. As noted in Chapter 16, $\nu$ has \textit{density} $f$ with respect to $\mu$. For each $A$, $\mu(A) = 0$ implies that $\nu(A) = 0$, as:
$$
	\int_A f d\mu = \int \1{A} f d\mu = 0
$$
As the integrand is $0$ almost everywhere (Theorem 15.2). The purpose of this section is to show the opposite direction - if $\nu(A)$ is zero when $\mu(A)$ is zero, then $\nu$ has a density with respect to $\mu$.

\subsubsection{Additive Set Functions} 
\paragraph{Additive Set Function} A function $\varphi$ from $\fcal$ to the reals for which:
$$
	\varphi\left(\bigcup_n A_n\right) = \sum_n \varphi(A_n)
$$
If $A_1, A_2, \cdots$ are disjoint. Note, it differs from a measure, as the values may be negative, but must also be finite. Note that $\varphi(\emptyset) = 0$ (via disjoint adding).

\paragraph{Lemma 1 - Additive Set Function Limits} If $E_u \uparrow E$ or $E_u \downarrow E$, then $\varphi(E_u) \to \varphi(E)$.
\\\\
\textbf{Proof:} If $E_u \uparrow E$, then:
$$
	\varphi(E) = \varphi\left(E_1 \cup \bigcup_{u=1}^\infty (E_{u+1} - E_u)\right)
	= \varphi(E_1) + \sum_{u=1}^\infty \varphi(E_{u+1} - E_u)
$$
Via disjointness. By the definition of an infinite sum:
$$
	= \lim_v \left[\varphi(E_1) + \sum_{u=1}^{v - 1} \varphi(E_{u+1} - E_u)\right] =
	\lim_v \varphi(E_v)
$$
Where we add the sets back together in the last step. Thus, we have proved the below case. From above, note that $E_u \downarrow E$ implies $E_u^c \uparrow E^c$, and hence:
$$
	\varphi(E_u) = \varphi(\Omega) - \varphi(E_u^c) \to \varphi(\Omega) - \varphi(E^c) = \varphi(E)
$$
Note, the from above case is simpler than the from above case for measures, as we automatically have each of the values is finite.

\subsubsection{The Hahn Decomposition}

\paragraph{Theorem 32.1 - The Hahn Decomposition} For any additive set function $\varphi$, there exist disjoint sets $A^+$ and $A^-$ such that $A^+ \cup A^- = \Omega$, $\varphi(E) \geq 0$ for all $E \subseteq A^+$, and $\varphi(E) \leq 0$ for all $E \subseteq A^-$.
\\\\
Note: If $\varphi(A) = \int_A f d\mu$, this is easy - take $A^+ = [f \geq 0]$, and $A^- = [f < 0]$. Clearly decompose $\Omega$ ($f$ assumed measurable) and any subsets contained within them must have positive/negative integrals, respectively.
\\\\
\textbf{Proof:} Let $\alpha = \sup\left[\varphi(A) : A \in \fcal\right]$. Suppose that there exists a set $A^+$ for which $\varphi(A^+) = \alpha$. Note, this also implies $\alpha$ is finite. Let $A^- = \Omega - A^+$. If $A \subseteq A^+$ and $\varphi(A) < 0$, then:
$$
	\varphi(A^+) = \alpha = \varphi(A) + \varphi(A^+ - A) \implies
	\varphi(A^+ - A) > \alpha
$$
Which is a contradiction. Hence, $A^+$ is a positive set. If $A \subseteq A^-$ and $\varphi(A) > 0$, then:
$$
	\varphi(A^+ \cup A) = \varphi(A^+) + \varphi(A) > \alpha
$$
Which is again a contradiction. So, we have proven it in the case that the supremum $\alpha$ can be obtained.
\\\\
So, if we prove it can always be attained, we have proven the theorem. Choose sets $A_n$ such that $\varphi(A_n) \to \alpha$ - this must exist by supremum definition. Let $A = \bigcup_n A_n \in \fcal$. For each $n$, consider the $2^n$ sets:
$$
	B_{ni} = \bigcap_{k=1}^n A_k'
$$
Where $A_k'$ is either $A_k$ or $A - A_k$. The collection $\bcal_n = \left[B_{ni}: 1 \leq i \leq 2^n\right]$ clearly partitions $A$ (note, we didn't do $A_k^c$, but $A - A_k$). Further, $\bcal_n$ refines $\bcal_{n-1}$ - each $B_{nj}$ is contained in exactly one of the $B_{n-1,i}$.
\\\\
Let $C_n$ be the union of those $B_{ni}$ in $\bcal_n$ for which $\varphi(B_{ni}) > 0$. Since $A_n$ is the union of all the sets of $\bcal_n$, it should be clear that $\varphi(A_n) \leq \varphi(C_n)$, as $\varphi(A_n)$ would have some subtractions. Since the $\bcal_1, \bcal_2, \cdots$ are finer and finer, $m < n$ implies that:
$$
	(C_m \cup \cdots \cup C_n) - (C_m \cup \cdots \cup C_{n-1})
$$
Is the union (perhaps empty) of certain of the sets $B_{ni}$. Note, adding $C_n$ could only add some partition of $B_{n-1,i}$ that was originally negative. As each of these $B_{ni}$ has positive $\varphi$, we note:
$$
	\varphi(C_m \cup \cdots \cup C_{n-1}) \leq \varphi(C_m \cup \cdots \cup C_n)
$$
And so, by induction:
$$
	\varphi(A_m) \leq \varphi(C_m) \leq \varphi(C_m \cup \cdots \cup C_n)
$$
If $D_m = \bigcup_{n=m}^\infty C_n$, then by Lemma 1, our limits, we have $\varphi(A_m) \leq \varphi(D_m)$. Define:
$$
	A^+ = \bigcap_{m=1}^\infty D_m
$$
So that $D_m \downarrow A^+$. By Lemma 1, $\alpha = \lim_m \varphi(A_m) \leq \lim_m \varphi(D_m) = \varphi(A^+)$. So, $A+$ does have the maximal value we need. qed.
