...
\begin{enumerate}
	...
	$$
	Note that the sets $(-x, \infty)$ and $(x, \infty)$ generate $\rcal^1$. Any finite interval of the form $(x, y]$ can be formed by a finite amount of intersections, unions, and complements of the given form - and those intervals generate $\rcal^1$. So note - we can take those finite amount of intersections, unions, and complements to find that:
	$$
		f^{-1}(a,b] \in \sigma(\fcal_0)
	$$
	This is true for every interval $(a,b]$. Now, we make the final push to prove:
	$$
		\fcal = \sigma(\fcal_0)
	$$
	Note that intervals $(-x,\infty)$ and $(x,\infty)$ generate $\rcal^1$. And so, by the pullback lemma below - we have that:
	$$
		\sigma(\fcal_0) \supseteq \sigma(f^{-1}(-x,\infty) \cup f^{-1}(x,\infty) | x > 0) = 
		f^{-1}(\rcal^1)
	$$
	We have that is true for every $f \in \lcal$. Thus, we have:
	$$
		\fcal = \sigma(\bigcup_{f \in \lcal} f^{-1}(\rcal^1)) \subseteq \sigma(\fcal_0)
	$$
	Note: this is kind of confusing. But, I think it is correct. For the first step - note that sets of the form $f^{-1}(\rcal^1)$ are what generate $\fcal$. And so, taking the $\sigma$ algebra over the generating set gives equality. Now, for the second step - $\subseteq$. We have that $f^{-1}(\rcal^1) \subseteq \sigma(\fcal_0)$. As this is a $\sigma$ algebra
