...
$$
And thus, we have that $\lambda_k$ is translation invariant. qed.
...
$$
	...
	TA^c = (TA)^c
$$
And so, the class $\gcal = \left[A : TA \in \rcal^k\right]$ is a $\sigma$ field. We now want to show that it is equal to $\rcal^k$, which comes from showing a generating set is contained in $\gcal$. Since $TA$ is open for open $A$ (if $T$ is nonsingular, $T$ is continuous, we can find open balls etc), it follows that $\gcal$ contains the open sets. The open sets generate $\rcal^k$, and so we can conclude:
$$
...
$$
For $A \in \rcal^k$, set $\mu_1(A) = \lambda_k(TA)$ and $\mu_2(A) = |\det T| \cdot \lambda_k(A)$. Then $\mu_1$ and $\mu_2$ are measures, and by Theorem 10.3 they will agree on $\rcal^k$ if they agree on the $\pi$-system of rational bounded rectangles (which, recall, generate $\rcal^k$, and $\R^k$ is $\sigma$ finite on this set). This setup is the same as how we prove Theorem 12.2. Note, $\mu_1$ and $\mu_2$ being measures comes directly from $\lambda_k$ being a measure.
\\\\
...
$$
If we prove equality for such $A$, and $B$ is a rational rectangle with $B = \bigcup_{i=1}^n A_i + x_i$, then we have:
$$
...
\begin{enumerate}
	...
	$$
	$$
		B_3 = [x : c < x_1 \leq c + x_2, 0 < x_2 \leq c] \cap B
	$$
	Note that $A = B_1 \cup B_2$, a disjoint union accounting for all cases of $0 < x_1 \leq c, 0 < x_2 \leq c$. Note that $TA = B_2 \cup B_3$.
\end{enumerate}
