...
\subsubsection{5.19} For integers $m$ and primes $p$, let $\alpha_p(m)$ be the exact power of $p$ in the prime factorization of $m$:
$$
...
$$
	P_n(A) = \frac{1}{n}\#\left[m : 1 \leq m \leq n, m \in A\right]
$$
Under each $P_n$, the $\alpha_p$ and $\delta_p$ are random variables (ie, they take $\omega$ into a real number). For distinct primes $p_1, \cdots, p_u$, there are a couple of things we want to show. First:
$$
...
We note:
$$
	P_n\left[\alpha_{p_i} = k_i, i \leq u\right] =
	P_n\left[\left\{\alpha_{p_i} \geq k_i, i \leq u\right\} - \left\{\alpha_{p_i} \geq k_i + 1, i \leq u\right\}\right]
$$
We note the set on the left completely contains the set on the right, and so as $P_n$ is a probability measure, we have:
$$
	= P_n[\alpha_{p_i} \geq k_i, i \leq u] - P_n[\alpha_{p_i} \geq k_i + 1, i \leq u] =
	\frac{1}{n}\floor{\frac{n}{p_1^{k_1} \cdots p_u^{k_u}}} - \frac{1}{n}\floor{\frac{n}{p_1^{k_1+1} \cdots p_u^{k_u+1}}}
$$
$$
	\to \frac{1}{p_1^{k_1} \cdots p_u^{k_u}} - \frac{1}{p_1^{k_1+1} \cdots p_u^{k_u+1}}
$$
We now make use of the inclusion-exclusion principle. Note: this is incorrect. Express the above sets as a union, and then simplify with the inclusion-exclusion principle.
